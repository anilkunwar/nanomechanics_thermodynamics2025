...
def render_sunburst_chart(labels, parents, values, cmap_name="viridis", label_size=12, 
                          width=900, height=700, theme=None, branchvalues="total"):
    """Sunburst with Hierarchical Symbols (★ Parent, ● Child, ■ Grandchild)"""
    ...
    # 1. Calculate Depth for Hierarchical Symbols
    ...
    depth_symbols = {0: '✦', 1: '★', 2: '●', 3: '■', 4: '▲'}

    # 3. Build Display Labels (Symbol + Name with spacing)
    display_labels = []
    for i, lab in enumerate(labels):
        d = depths[i]
        sym = depth_symbols.get(d, '•')
        display_labels.append(f"{sym}  {lab}")
    ...
    # 5. Color mapping based on root parent (Depth 1 category)
    ...
    fig.update_layout(
        ...
        title=dict(
            text="<b>Hierarchical Concept Map</b><br><i>★ = Category | ● = Concept | ■ = Sub-Concept</i>",
    ...
    # 7. Two-Column Custom Legend
    st.markdown("### 📊 Legend & Hierarchy Mapping")
    cols = st.columns(2)

    with cols[0]:
        st.markdown("**Hierarchy Symbols:**")
        st.markdown("- ✦ **Root Level** (All Concepts)")
        st.markdown("- ★ **Category Level** (e.g., Defect Engineering)")
        st.markdown("- ● **Concept Level** (e.g., Stacking Faults)")
        st.markdown("- ■ **Sub-Concept Level** (Detailed features)")
...
# ==========================================
# HIGH-QUALITY IMAGE EXPORT (Kaleido)
# ==========================================
def render_high_res_export(fig, filename="concept_graph"):
    """Provides buttons to download high-quality PNG, SVG, and PDF via Kaleido."""
    st.markdown("### 📥 High-Quality Export")
    st.caption("Requires 'kaleido' package: pip install kaleido")

    col1, col2, col3 = st.columns(3)

    try:
        png_bytes = fig.to_image(format="png", scale=3, width=1200, height=800)
        with col1:
            st.download_button(
                label="📄 PNG (High-Res)",
                data=png_bytes,
                file_name=f"{filename}_highres.png",
                mime="image/png"
            )
        ...
        with col2:
            st.download_button(
                label="📐 SVG (Vector)",
                data=svg_bytes,
                file_name=f"{filename}_vector.svg",
                mime="image/svg+xml"
            )
        ...
        st.error(f"Export failed. Ensure Kaleido is installed: {e}")
    ...
    if st.session_state.analysis_data is not None:
        ...
        with tabs[tab_idx]:
            ...
            with st.expander("Domain Hierarchy (Sunburst)"):
                ...
                # High-quality export for sunburst
                try:
                    sunburst_fig = go.Figure(go.Sunburst(
                        ids=[l[:20] for l in labels],
                        labels=labels,
                        parents=parents,
                        values=values,
                        branchvalues=bv_mode,
                    ...
                    render_high_res_export(sunburst_fig, "sunburst_hierarchy")
                except Exception:
                    pass
            ...
            with st.expander("Concept Radar"):
                ...
                # Note: Radar chart export requires capturing the fig object from render_radar_chart
        ...
        with tabs[tab_idx]:
            ...
            with st.expander("Bubble Chart (Importance)"):
                ...
                # High-quality export for bubble chart
                try:
                    bubble_data = []
                    category_map = abstract_concepts_to_categories(valid_concepts)
                    for concept in valid_concepts:
                    ...
                    bubble_df = pd.DataFrame(bubble_data)
                    bubble_fig = px.scatter(bubble_df, x="Degree", y="Frequency", 
                                            size="Distillation Efficiency", color="Category",
                                            title="Concept Importance Bubble Chart", size_max=50)
                    render_high_res_export(bubble_fig, "bubble_chart")
                except Exception:
                    pass
