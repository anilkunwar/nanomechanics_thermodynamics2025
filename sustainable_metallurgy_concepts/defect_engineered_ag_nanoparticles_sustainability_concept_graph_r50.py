...
====================================================================
v4.1 OOM-HARDENED & OPTIMIZED EDITION - AgNP SUSTAINABILITY FOCUSED
====================================================================
Multi-level reasoning concept graph for defect-engineered sustainable 
metallurgy of Ag nanoparticles.

Based on CoCrFeNi MPEA Quantitative Descriptor Graph v4.1
Structure and logic identical, only domain ontology and patterns replaced.
"""
...
# ==========================================
st.set_page_config(
    page_title="AgNP-Sustainability-ConceptGraph",
    page_icon="🔬",
    layout="wide",
...
class DomainOntology:
    """Comprehensive ontology for AgNP Sustainable Metallurgy."""
    def __init__(self):
...
class EnhancedConceptExtractor:
    ...
    def _build_extraction_patterns(self):
        # AgNP domain patterns
        self.alloy_patterns = [r'\bagnps\b', r'\bag\s+nanoparticles\b', r'\bsilver\s+nanoparticles\b', r'\bnano-silver\b']
        self.process_patterns = [r'\bpressureless\s+sintering\b', r'\bflux[- ]free\s+sintering\b', r'\bmass\s+transport\b', r'\batomic\s+diffusion\b']
        ...
        self.fluid_patterns = []
        self.ai_patterns = []
        self.micro_patterns = [r'\bfcc\s+phase\b', r'\bgrain\s+boundary\b', r'\bstacking\s+faults\b', r'\btwins\b']
        self.comp_patterns = []
        self.param_patterns = [
            r'\b(hardness|elongation|yield\s+strength|tensile\s+strength)\s*(?:of|is|=|:)?\s*(\d+(?:\.\d+)?)\s*(?:hv|%|gpa|mpa)\b',
            r'\b(temperature|sintering\s+temperature)\s*(?:of|is|=|:)?\s*(\d+(?:\.\d+)?)\s*(?:k|°c)\b'
        ]
        ...
        # Limit keyword regex to top 500 longest keywords
...
def extract_concepts_from_text(text: str) -> List[str]:
    ...
    noun_pattern = r'\b(?:[a-z]+(?:[-\s]?[a-z]+){0,2}[-\s]?)?(?:sintering|diffusion|defect|fault|twin|lattice|void|residue|conductivity|bonding|yield|stress|shock|budget|footprint|efficiency|circular|recycling|emission)\b'
    ...
    for concept in concepts:
        ...
        for pattern, category in AG_SUSTAINABILITY_CATEGORY_MAPPING.items():
            if re.search(pattern, concept, re.I):
                ...
                break
        if not matched:
            if any(re.search(p, concept, re.I) for p in [r'\bagnps', r'\bag\s+nanoparticles', r'\bsilver\s+nanoparticles']):
                concept_to_abstract[concept] = 'core_material'
            elif any(re.search(p, concept, re.I) for p in [r'\bsintering', r'\bdiffusion', r'\bflux']):
                concept_to_abstract[concept] = 'sintering_process'
            elif any(re.search(p, concept, re.I) for p in [r'\bdefect', r'\bfault', r'\btwin', r'\blattice']):
                concept_to_abstract[concept] = 'defect_engineering'
            elif any(re.search(p, concept, re.I) for p in [r'\bsustainable', r'\bcircular', r'\bemission', r'\bwaste']):
                concept_to_abstract[concept] = 'sustainability_impact'
            elif any(re.search(p, concept, re.I) for p in [r'\bthermal', r'\bstress', r'\byield', r'\bconductivity']):
                concept_to_abstract[concept] = 'thermal_mechanical_response'
            elif any(re.search(p, concept, re.I) for p in [r'\bvoid', r'\bporosity', r'\boxide']):
                concept_to_abstract[concept] = 'microstructural_defects'
            elif any(re.search(p, concept, re.I) for p in [r'\befficiency', r'\bfootprint', r'\brecycling']):
                concept_to_abstract[concept] = 'cleaner_production_metrics'
            else:
...
def build_hybrid_graph(all_concepts: List[List[str]], valid_concepts: List[str], concept_to_id: Dict[str, int], embed_model=None, config: Dict = None, ontology: DomainOntology = None) -> nx.Graph:
    if config is None: config = get_adaptive_config(3000)
    nx_graph = nx.Graph()
    for c in valid_concepts:
        definition = ontology.get_definition(c) if ontology else ''
        nx_graph.add_node(c, frequency=0, definition=definition, concept_type=ontology.get_concept_type(c).value if ontology else 'general')
    for concepts in all_concepts:
        valid_in_doc = [c for c in concepts if c in concept_to_id]
        for i in range(len(valid_in_doc)):
            for j in range(i + 1, len(valid_in_doc)):
...
def export_publication_figure(nx_graph, valid_concepts, concept_abstract_map, cmap_name="viridis", dpi=300, figsize=(14, 12), filename="agnp_graph_pub.png") -> bytes:
    try:
        ...
        plt.title("AgNP Sustainability Concept Graph", fontsize=14, fontweight='bold', pad=20)
        buf = io.BytesIO()
...
def generate_analysis_report(nx_graph, valid_concepts, concept_abstract_map, top_scores, distill_df, burst_df, drift_df, genealogy_df, bridge_df, motifs, val_metrics, df_filtered) -> str:
    report = []
    report.append("# AgNP Sustainability Concept Graph Analysis Report")
    ...
    report.append("")
    report.append("---")
    report.append("*Report generated by AgNP-Sustainability-ConceptGraph*")
    return "\n".join(report)
...
def get_ag_sintering_category_color(concept: str, cmap_colors: Optional[List[str]] = None) -> str:
    ...
    color_map = {
        ...
        'sintering_process': '#00BCD4',
        'defect_engineering': '#FF9800',
        'sustainability_impact': '#4CAF50',
        'thermal_mechanical_response': '#9C27B0',
        'microstructural_defects': '#795548',
        'cleaner_production_metrics': '#2196F3',
        'general': '#9E9E9E'
    }
    return color_map.get(category, '#9E9E9E')

# ==========================================
# RENDER FUNCTIONS – IDENTICAL TO COCRFENI
# ==========================================
def render_graph_pyvis(nx_graph, concept_abstract_map, physics_enabled=True, min_node_size=8, max_node_size=40, cmap_name="viridis", custom_labels=None, node_label_size=12, top_n_nodes=0, theme=None, physics_preset=None, show_edge_weights=False, edge_label_mode="hover", show_reasoning=False, use_abbreviated_labels=False, max_label_length=15, node_font_face="Inter, Segoe UI, Roboto, sans-serif", edge_label_size=10, edge_label_color=None, edge_label_position="middle", enable_node_highlight=True, show_definitions=True):
    ...
    try:
        ...
        try:
            st.download_button("Download Interactive Graph (HTML)", data=html_content.encode('utf-8'), file_name="agnp_graph.html", mime="text/html")
...
def render_sunburst_chart(labels, parents, values, cmap_name="viridis", label_size=11, width=800, height=600, theme=None, branchvalues="total", show_legend=True):
    ...
    fig.update_layout(title="<b>AgNP Sustainability Process-Material Response Domain Hierarchy</b><br><i>Size = concept frequency | Symbols show hierarchy depth</i>", font=dict(size=label_size, family="Arial"), paper_bgcolor="white", plot_bgcolor="white", width=width, height=height, margin=dict(t=80, b=20, l=20, r=20))
    st.plotly_chart(fig, width="stretch")
    if show_legend:
    ...
    if format_type == "GraphML":
        try:
            if include_metadata:
                ...
                nx_graph.graph['tool'] = 'AgNP-ConceptGraph'
            try: nx.write_graphml_lxml(nx_graph, "agnp_graph.graphml")
            except Exception: nx.write_graphml(nx_graph, "agnp_graph.graphml")
            with open("agnp_graph.graphml", "rb") as f: return f.read(), "application/graphml+xml", "agnp_graph.graphml"
        except Exception as e:
        ...
        if include_metadata:
            data['metadata'] = {'created': datetime.now().isoformat(), 'version': '4.0', 'tool': 'AgNP-ConceptGraph', 'node_count': len(nx_graph.nodes()), 'edge_count': len(nx_graph.edges()), 'inferred_edges': sum(1 for u, v, d in nx_graph.edges(data=True) if d.get('inferred', False)), 'categories': list(set(abstract_concepts_to_categories(list(nx_graph.nodes())).values()))}
        json_str = json.dumps(data, indent=2, default=str)
        return json_str.encode('utf-8'), "application/json", "agnp_graph_full.json"
        ...
        return csv_df.to_csv(index=False).encode('utf-8'), "text/csv", "agnp_edges_enhanced.csv"
        ...
        return csv_df.to_csv(index=False).encode('utf-8'), "text/csv", "agnp_nodes_enhanced.csv"
        ...
        try:
            if include_metadata:
                nx_graph.graph['created'] = datetime.now().isoformat()
                nx_graph.graph['version'] = '4.0'
            nx.write_gexf(nx_graph, "agnp_graph.gexf")
            with open("agnp_graph.gexf", "rb") as f: return f.read(), "application/xml", "agnp_graph.gexf"
        except Exception as e:
...
def render_sidebar():
    with st.sidebar:
        st.header("⚙️ Configuration v4.1 (OOM-Hardened) – AgNP")
        st.subheader("🎨 Theme")
        ...
        st.subheader("🔬 AgNP Sustainability Focus Areas")
        st.markdown("- **Core Materials**: AgNPs, SiC/GaN, polymer substrates")
        st.markdown("- **Sintering Processes**: Pressureless sintering, mass transport, diffusion")
        st.markdown("- **Defect Engineering**: Stacking faults (ISF/ESF), twins, thermodynamic catalysts")
        st.markdown("- **Sustainability Impact**: Cleaner production, circular economy, GHG emissions, net-zero")
        st.markdown("- **Thermal-Mechanical Response**: Thermal shock, residual stresses, yield, conductivity")
        st.markdown("- **Microstructural Defects**: Micro-voids, carbonaceous residues, lattice strain")
        st.markdown("- **Cleaner Production Metrics**: Energy efficiency, lifecycle assessment, waste minimization")
        st.subheader("🧠 NLP Reasoning Options")
        st.session_state['use_ontology'] = st.checkbox("Use ontology-based resolution", value=True, help="Maps synonyms like 'AgNPs', 'silver nanoparticles' to canonical concepts")
        ...
        st.session_state['use_relationship_extraction'] = st.checkbox("Extract cause-effect relationships", value=True, help="Identifies causal links between parameters and microstructure")
...
def render_reasoning_dashboard(nx_graph, valid_concepts, ontology, extractor):
    ...
    synonym_examples = [("silver nanoparticles", "agnps"), ("ag nanoparticles", "agnps"), ("pressureless sintering", "pressureless_sintering"), ("defect engineering", "defect_engineering"), ("stacking faults", "stacking_faults")]
    syn_data = []
...
def main():
    st.title("🔬 AgNP Sustainability Concept Graph")
    st.caption("Multi-level reasoning concept graph for defect-engineered sustainable metallurgy of Ag nanoparticles | Focus: Sintering, Defects, Sustainability")
    ...
    if st.session_state.analysis_data is not None:
        ...
        with tabs[tab_idx]:
            ...
            if st.button("Generate Publication Figure"):
                ...
                if pub_bytes: st.download_button("📥 Download Publication PNG", data=pub_bytes, file_name="agnp_graph_publication.png", mime="image/png")
            st.markdown("---")
            ...
            if st.button("Generate Markdown Report"):
                ...
                st.download_button("📄 Download Report (Markdown)", data=report.encode('utf-8'), file_name="agnp_sustainability_analysis_report.md", mime="text/markdown")
