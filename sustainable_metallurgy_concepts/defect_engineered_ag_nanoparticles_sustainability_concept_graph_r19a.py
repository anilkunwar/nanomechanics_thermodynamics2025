...
class ReasoningEnhancedGraphBuilder:
    ...
    def build_graph(self, all_concepts: List[List[str]], valid_concepts: List[str],
        ...
        for c in valid_concepts:
            ...
            definition = self.ontology.get_definition(c)   # <<< RETRIEVE DEFINITION
            nx_graph.add_node(c, 
                            frequency=freq,
                            concept_type=concept_type.value,
                            degree=0,
                            definition=definition)        # <<< STORE DEFINITION
...
def build_hybrid_graph(all_concepts: List[List[str]], valid_concepts: List[str],
                        ...
                        ontology=None) -> nx.Graph:   # <--- ADDED ontology parameter
    if config is None:
        config = get_adaptive_config(3000)
    nx_graph = nx.Graph()
    ...
    for c in valid_concepts:
        ...
        nx_graph.add_node(c, frequency=0, definition=definition)   # <--- STORE DEFINITION
    for concepts in all_concepts:
        valid_in_doc = [c for c in concepts if c in concept_to_id]
        for i in range(len(valid_in_doc)):
            for j in range(i + 1, len(valid_in_doc)):
...
def render_graph_pyvis(nx_graph, concept_abstract_map, physics_enabled=True,
                        ...
                        show_definitions=True):   # <--- ADDED PARAMETER
    """
    ...
    # --- NODE HIGHLIGHT & EDGE DESCRIPTION INJECTION (unchanged) ---
    ...
    (function() {
        var checkExist = setInterval(function() {
            if (typeof network !== 'undefined' && network !== null && network.body && network.body.data) {
                ...
                // ── Reset: restore node colors, hide panel ─────────────────
                function resetAll() {
                    var nodeRestores = [];
                ...
                // ── Build / update floating side panel ─────────────────────
                function showEdgeInfoPanel(nodeId, edgeList) {
                    ...
                    panel.style.cssText = [
                        'position:fixed','top:110px','right:24px','width:360px',
                        'max-height:560px','overflow-y:auto','z-index:9999',
                    ...
                    var html = '<div style="font-size:15px;font-weight:700;color:#D32F2F;margin-bottom:10px;'
                             + 'border-bottom:2px solid #FFD700;padding-bottom:8px;display:flex;'
                             + 'align-items:center;gap:8px;">'
                             + '<span style="font-size:18px;">🔗</span>'
                             + '<span style="color:#1e293b;word-break:break-word;">' + nodeName + '</span>'
                             + '<span style="color:#64748b;font-weight:400;font-size:12px;margin-left:auto;'
                             + 'white-space:nowrap;">(' + edgeList.length + ')</span></div>'
                    ...
                    edgeList.forEach(function(e, idx){
                        ...
                        var typeBadge = e.isInferred
                            ? '<span style="color:#8b5cf6;font-size:10px;font-weight:600;">(' + e.type + ') [INF]</span>'
                            : '<span style="color:#0ea5e9;font-size:10px;font-weight:600;">(' + e.type + ')</span>';
                        html += '<div style="padding:7px 10px;margin-bottom:5px;background:#f8fafc;'
                              + 'border-left:3px solid ' + typeColor + ';border-radius:5px;'
                              + 'font-size:12px;line-height:1.6;">'
                              + '<span style="color:#94a3b8;font-size:10px;display:inline-block;'
                              + 'width:18px;">' + (idx+1) + '.</span> '
                              + '<b style="color:#D32F2F;">w</b> '
                              + '<span style="color:#1e293b;font-weight:500;">' + e.from + '</span>'
                              + ' <span style="color:#94a3b8;">↔</span> '
                              + '<span style="color:#1e293b;font-weight:500;">' + e.to + '</span>'
                ...
                network.on("selectNode", function(params) {
                    ...
                    // 1) Gold ring on connected nodes
                    var nodeUpdates = [];
                    ...
                    // 2) Collect edge metadata → side panel
                    var edgeInfoList = [];
...
def render_sidebar():
    with st.sidebar:
        ...
        # --- NEW: Toggle for tooltip definitions ---
        st.session_state['show_definitions'] = st.toggle(
            "Show concept definitions in tooltips", value=True,   # <--- Default True
...
def main():
            ...
            with status:
                ...
                if use_ontology and use_inference:
                    ...
                    nx_graph = build_hybrid_graph(
                        ...
                        embed_model, config, ontology=ontology   # <--- PASS ONTOLOGY
                    )
    ...
    if st.session_state.analysis_data is not None:
        ...
        with tabs[tab_idx]:
            ...
            show_definitions = st.session_state.get('show_definitions', True)   # <--- NEW
            ...
            if viz_choice == "PyVis (Interactive)":
                render_graph_pyvis(
                    ...
                    show_definitions=show_definitions   # <--- PASS TO FUNCTION
                )
