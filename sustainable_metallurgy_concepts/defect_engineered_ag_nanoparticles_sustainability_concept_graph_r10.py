...
def render_sunburst_chart(labels, parents, values, cmap_name="viridis",
    ...
    """
    Sunburst with DISCRETE categorical colors and SYMBOL-based labels.
    Uses `ids` and `parents` for hierarchy matching to allow modified display labels.
    """
    ...
    # ── 2. Assign DISCRETE colors and SYMBOLS per category (Ring 1) ────
    category_nodes = [l for l, d in zip(labels, depths) if d == 1]
    
    # Symbol library for categories (add more if you have >15 categories)
    SYMBOL_LIBRARY = ['★', '●', '■', '▲', '◆', '⬟', '⬢', '◉', '◈', '◇', '○', '□', '△', '◊', '✦']
    cat_symbol_map = {cat: SYMBOL_LIBRARY[i % len(SYMBOL_LIBRARY)] for i, cat in enumerate(category_nodes)}
    ...
    # ── 3. Build Display Labels with Symbols ───────────────────────────
    display_labels = []
    node_colors = []
    
    for label, depth, parent in zip(labels, depths, parents):
        if depth == 0:
            display_labels.append("") # Root (center)
            node_colors.append(theme.get("plotly_paper", "#ffffff"))
        elif depth == 1:
            sym = cat_symbol_map.get(label, '★')
            display_labels.append(f"{sym} {label}" if show_labels else sym)
            node_colors.append(cat_color_map.get(label, "#9E9E9E"))
        else:
            # Child node: inherit parent's symbol and color
            parent_cat = parent if parent in cat_symbol_map else labels[0]
            sym = cat_symbol_map.get(parent_cat, '★')
            display_labels.append(f"{sym} {label}" if show_labels else sym)
            
            base_color = cat_color_map.get(parent_cat, "#9E9E9E")
    ...
    for label, parent, val, depth in zip(labels, parents, values, depths):
        if hover_info == "none":
            ...
            sym = cat_symbol_map.get(label, cat_symbol_map.get(parent, '★')) if depth > 0 else ''
            hover_texts.append(
                f"<b>{sym} {label}</b><br>"
    ...
    fig = go.Figure(go.Sunburst(
        ids=labels,             # Original names for hierarchy matching
        labels=display_labels,  # Symbol + Name for visual display
        parents=parents,        # Original parent names
        values=values,
    ...
    fig.update_layout(
        ...
        sunburstcolorway=discrete_colors,
    )
    st.plotly_chart(fig, use_container_width=True)

    # ── 7. Symbol & Color Legend ───────────────────────────────────────
    if st.session_state.get('sunburst_show_legend', True) and category_nodes:
        st.markdown("**Category Symbol & Color Legend:**")
        legend_html = "<div style='display: flex; flex-wrap: wrap; gap: 12px; margin-top: 10px;'>"
        for cat in category_nodes:
            sym = cat_symbol_map.get(cat, '★')
            color = cat_color_map.get(cat, "#999")
            
            # Handle rgba or rgb colors for background with opacity
            bg_color = color
            if bg_color.startswith("rgb("):
                bg_color = bg_color.replace("rgb(", "rgba(").replace(")", ",0.15)")
            ...
            <div style='display: flex; align-items: center; padding: 6px 12px; 
                ...
                <span style='font-size: 22px; color: {color}; margin-right: 8px; font-weight: bold;'>{sym}</span>
                <span style='font-size: 14px; font-weight: 600; color: {color};'>{cat}</span>
            </div>
            """
        legend_html += "</div>"
