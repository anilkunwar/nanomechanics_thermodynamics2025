...
def render_graph_pyvis(nx_graph, concept_abstract_map, physics_enabled=True,
                        ...
                        show_definitions=True):   # <--- show_definitions default True
    """
    ...
    if enable_node_highlight:
        # This JavaScript now extracts the node's title (including definition) and displays it in the side panel
        highlight_js = """
    <script>
    (function() {
        var checkExist = setInterval(function() {
            if (typeof network !== 'undefined' && network !== null && network.body && network.body.data) {
                ...
                function showEdgeInfoPanel(nodeId, connectedEdges) {
                    ...
                    // Show the full title (includes Type, Degree, Frequency, Definition)
                    if (nodeTitle) {
                    ...
                    edgeList.forEach(function(e, idx){
                        ...
                        var badge = e.inferred ? ' <span style="background:#8b5cf6;color:white;font-size:9px;padding:1px 4px;border-radius:3px;">INF</span>' : '';
                        html += '<div style="padding:5px 8px;margin:3px 0;background:#f8fafc;border-left:3px solid ' + typeColor + ';border-radius:4px;font-size:11px;">'
                             + (idx+1) + '. <b style="color:#D32F2F;">w</b> '
                             + e.from + ' ↔ ' + e.to + ' = <b style="color:#0ea5e9;">' + e.weight + '</b> '
                             + '<span style="color:#94a3b8;font-size:10px;">(' + e.type + ')</span>' + badge + '</div>';
                    });
