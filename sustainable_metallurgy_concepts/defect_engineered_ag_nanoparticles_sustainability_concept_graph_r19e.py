...
# -*- coding: utf-8 -*-
"""
AgNP-Sustainability-ConceptGraph: Advanced NLP-Enhanced Concept Graph Builder
============================================================================
Multi-level reasoning concept graph for defect-engineered sustainable 
metallurgy of Ag nanoparticles.
...
Run: streamlit run agnp_sustainability_concept_graph.py
...
# ==========================================
st.set_page_config(
    page_title="AgNP-Sustainability-ConceptGraph: Advanced NLP-Enhanced Explorer",
    page_icon="🔬",
    layout="wide",
...
class DomainOntology:
    """
    Comprehensive ontology for Ag nanoparticles sustainable metallurgy domain.
    Maps synonyms, hypernyms, and domain relationships for defect-engineered sintering.
    """
    ...
    def _build_ontology(self):
        ...
        # === MATERIALS ===
        self._add_concept("agnps", ConceptType.MATERIAL, 
            synonyms={"ag nanoparticles", "silver nanoparticles", "nano-silver", "silver", "ag",
                     "ag nps", "ag-nanoparticles", "silver nps"},
            definition="Silver nanoparticles for sustainable metallurgy")

        self._add_concept("silver", ConceptType.MATERIAL,
            synonyms={"ag", "silver metal", "silver powder", "silver particles"},
            hypernyms={"metal"},
            definition="Silver metal base material")

        self._add_concept("metal", ConceptType.MATERIAL,
            synonyms={"metals", "metallic material"},
            hyponyms={"silver", "agnps"})

        self._add_concept("sic", ConceptType.MATERIAL,
            synonyms={"silicon carbide", "sic substrate", "silicon carbide substrate"},
            definition="Silicon carbide semiconductor substrate")

        self._add_concept("gan", ConceptType.MATERIAL,
            synonyms={"gallium nitride", "gan substrate", "gallium nitride substrate"},
            definition="Gallium nitride wide-bandgap semiconductor")

        self._add_concept("polymer_substrate", ConceptType.MATERIAL,
            synonyms={"polymer substrates", "temperature-sensitive substrates", "organic substrate",
                     "polymer base", "flexible substrate"},
            definition="Temperature-sensitive polymer substrates")

        self._add_concept("fcc", ConceptType.MICROSTRUCTURE,
            synonyms={"face-centered cubic", "face centered cubic", "fcc crystal", "fcc structure"},
            definition="Face-centered cubic crystal structure of silver")

        # === SINTERING PROCESSES ===
        self._add_concept("pressureless_sintering", ConceptType.PROCESS,
            synonyms={"pressureless sintering", "sintering", "sintering process", "sintering onset",
                     "sintering temperature", "sintering mechanism", "sintering kinetics"},
            hypernyms={"sintering_process"},
            definition="Pressureless sintering of Ag nanoparticles")

        self._add_concept("sintering_process", ConceptType.PROCESS,
            synonyms={"sintering", "sinter", "sintering technique", "sintering method"},
            hyponyms={"pressureless_sintering", "flux_free_sintering"},
            definition="Sintering processes for metallurgy")

        self._add_concept("flux_free_sintering", ConceptType.PROCESS,
            synonyms={"flux-free sintering", "flux free", "flux-free metallurgy", "flux-free process"},
            hypernyms={"sintering_process"},
            definition="Flux-free sintering for cleaner production")

        self._add_concept("mass_transport", ConceptType.PROCESS,
            synonyms={"mass transport", "atomic transport", "material transport", "diffusion transport"},
            definition="Mass transport during sintering")

        self._add_concept("atomic_diffusion", ConceptType.PROCESS,
            synonyms={"atomic diffusion", "surface diffusion", "volume diffusion", "grain boundary diffusion",
                     "diffusion mechanism", "atomic contact"},
            definition="Atomic diffusion mechanisms")

        self._add_concept("power_module_assembly", ConceptType.PROCESS,
            synonyms={"power module", "module assembly", "industrial assembly", "scalable production",
                     "industrial-scale manufacturing"},
            definition="Power module assembly and manufacturing")

        # === DEFECT ENGINEERING & THERMODYNAMICS ===
        self._add_concept("defect_engineering", ConceptType.METHOD,
            synonyms={"defect engineering", "crystal defect engineering", "defect-mediated", 
                     "defect-mediated design", "intrinsic approach", "material-level design"},
            definition="Defect engineering for thermodynamic catalysis")

        self._add_concept("stacking_faults", ConceptType.MICROSTRUCTURE,
            synonyms={"stacking faults", "intrinsic stacking faults", "isf", "extrinsic stacking faults", 
                     "esf", "stacking fault", "planar defects"},
            definition="Stacking faults in Ag nanoparticles")

        self._add_concept("twins", ConceptType.MICROSTRUCTURE,
            synonyms={"twin", "deformation twins", "nanotwinning", "twin boundaries", "twinning",
                     "mechanical twinning", "growth twins"},
            definition="Twin structures in Ag nanoparticles")

        self._add_concept("lattice_distortion", ConceptType.PHENOMENON,
            synonyms={"lattice distortion", "lattice strain", "microstrain", "crystal strain",
                     "lattice deformation", "structural distortion"},
            definition="Lattice distortion and strain")

        self._add_concept("diffusion_highways", ConceptType.PHENOMENON,
            synonyms={"diffusion highways", "fast diffusion paths", "diffusion pathways",
                     "rapid diffusion channels", "enhanced diffusion"},
            definition="Diffusion highways created by defects")

        self._add_concept("thermodynamic_catalyst", ConceptType.MODEL,
            synonyms={"thermodynamic catalyst", "internal catalyst", "catalytic defect",
                     "defect catalyst", "thermodynamic driving force"},
            definition="Defects as thermodynamic catalysts")

        # === SUSTAINABILITY & ENVIRONMENT ===
        self._add_concept("cleaner_production", ConceptType.PROCESS,
            synonyms={"cleaner production", "sustainable metallurgy", "green metallurgy",
                     "sustainable manufacturing", "green manufacturing"},
            definition="Cleaner production and sustainable metallurgy")

        self._add_concept("circular_economy", ConceptType.MODEL,
            synonyms={"circular economy", "end-of-life recycling", "recycling", "recyclability",
                     "closed loop", "material recovery"},
            definition="Circular economy principles")

        self._add_concept("greenhouse_gas", ConceptType.PHENOMENON,
            synonyms={"greenhouse gas emissions", "scope 1 emissions", "scope 2 emissions", 
                     "ghg emissions", "carbon emissions", "co2 emissions"},
            definition="Greenhouse gas emissions")

        self._add_concept("net_zero", ConceptType.PROPERTY,
            synonyms={"net-zero targets", "net zero", "carbon neutral", "zero emission",
                     "zero-emission", "carbon neutrality"},
            definition="Net-zero emission targets")

        self._add_concept("e_waste", ConceptType.MATERIAL,
            synonyms={"e-waste", "electronic waste", "waste electronics", "end-of-life electronics"},
            definition="Electronic waste and end-of-life materials")

        self._add_concept("vocs", ConceptType.PHENOMENON,
            synonyms={"volatile organic compounds", "vocs", "organic volatiles", 
                     "toxic emissions", "hazardous emissions"},
            definition="Volatile organic compounds and toxic emissions")

        # === THERMAL-MECHANICAL RESPONSE ===
        self._add_concept("thermal_shock", ConceptType.PHENOMENON,
            synonyms={"thermal shock", "thermal cycling", "thermal stress", "thermal fatigue",
                     "temperature shock", "thermal degradation"},
            definition="Thermal shock and degradation")

        self._add_concept("residual_stress", ConceptType.PROPERTY,
            synonyms={"residual stresses", "internal stress", "thermal stress", "mechanical stress"},
            definition="Residual stresses in bonded structures")

        self._add_concept("mechanical_yield", ConceptType.PROPERTY,
            synonyms={"mechanical yield", "yield strength", "mechanical properties", 
                     "yield point", "strength"},
            definition="Mechanical yield and strength properties")

        self._add_concept("electrical_conductivity", ConceptType.PROPERTY,
            synonyms={"electrical conductivity", "conductivity", "electrical transport",
                     "electrical resistance", "resistivity"},
            definition="Electrical conductivity of sintered Ag")

        self._add_concept("thermal_conductivity", ConceptType.PROPERTY,
            synonyms={"thermal conductivity", "heat conductivity", "thermal transport"},
            definition="Thermal conductivity")

        self._add_concept("high_strength_bonding", ConceptType.PROPERTY,
            synonyms={"high-strength bonding", "dense bonding", "strong bonding", 
                     "robust bonding", "reliable bonding"},
            definition="High-strength bonding quality")

        self._add_concept("thermal_budget", ConceptType.PARAMETER,
            synonyms={"thermal budget", "mechanical budget", "energy budget", 
                     "processing budget", "energy consumption"},
            definition="Thermal and mechanical processing budget")

        # === MICROSTRUCTURAL DEFECTS ===
        self._add_concept("micro_voids", ConceptType.MICROSTRUCTURE,
            synonyms={"micro-voids", "voids", "porosity", "pores", "cavities", "micro-pores"},
            definition="Micro-voids and porosity defects")

        self._add_concept("carbonaceous_residue", ConceptType.MICROSTRUCTURE,
            synonyms={"carbonaceous residues", "organic residue", "carbon residue", 
                     "capping agent residue", "surfactant residue"},
            definition="Carbonaceous residues from capping agents")

        self._add_concept("oxidation", ConceptType.PHENOMENON,
            synonyms={"oxidation", "oxide formation", "surface oxidation", "ag oxidation"},
            definition="Oxidation of silver nanoparticles")
        ...
        self._add_concept("energy_efficiency", ConceptType.PROPERTY,
            synonyms={"energy efficiency", "energy savings", "low energy", "energy consumption",
                     "kilowatt-hour energy", "power consumption"},
            definition="Energy efficiency metrics")

        self._add_concept("lifecycle_assessment", ConceptType.METHOD,
            synonyms={"lifecycle assessment", "lca", "life cycle analysis", 
                     "environmental assessment"},
            definition="Lifecycle assessment methodology")

        self._add_concept("waste_minimization", ConceptType.PROPERTY,
            synonyms={"waste minimization", "waste reduction", "defect reduction",
                     "yield improvement", "process yield"},
            definition="Waste minimization and yield improvement")

        self._add_concept("carbon_footprint", ConceptType.PROPERTY,
            synonyms={"carbon footprint", "carbon impact", "environmental footprint",
                     "ecological footprint"},
            definition="Carbon footprint of manufacturing")
    ...
    def _build_causal_chains(self):
        ...
        causal_chains = [
            # Defect engineering drives sintering
            ("defect_engineering", RelationshipType.INFLUENCES, "pressureless_sintering", 0.95),
            ("defect_engineering", RelationshipType.CAUSES, "diffusion_highways", 0.95),
            ("defect_engineering", RelationshipType.INFLUENCES, "atomic_diffusion", 0.90),

            # Stacking faults and twins enable diffusion
            ("stacking_faults", RelationshipType.CAUSES, "diffusion_highways", 0.90),
            ("twins", RelationshipType.CAUSES, "diffusion_highways", 0.85),
            ("lattice_distortion", RelationshipType.CAUSES, "diffusion_highways", 0.80),
            ("diffusion_highways", RelationshipType.INFLUENCES, "atomic_diffusion", 0.95),
            ("diffusion_highways", RelationshipType.INFLUENCES, "mass_transport", 0.90),

            # Sintering process leads to bonding
            ("pressureless_sintering", RelationshipType.RESULTS_IN, "high_strength_bonding", 0.90),
            ("pressureless_sintering", RelationshipType.RESULTS_IN, "electrical_conductivity", 0.85),
            ("pressureless_sintering", RelationshipType.RESULTS_IN, "thermal_conductivity", 0.85),
            ("atomic_diffusion", RelationshipType.RESULTS_IN, "high_strength_bonding", 0.90),
            ("mass_transport", RelationshipType.RESULTS_IN, "high_strength_bonding", 0.85),

            # Thermal-mechanical response
            ("thermal_shock", RelationshipType.CAUSES, "residual_stress", 0.85),
            ("thermal_shock", RelationshipType.DEGRADES, "high_strength_bonding", 0.80),
            ("residual_stress", RelationshipType.INFLUENCES, "mechanical_yield", 0.85),
            ("thermal_budget", RelationshipType.INFLUENCES, "thermal_shock", 0.80),

            # Defects affect microstructure
            ("stacking_faults", RelationshipType.INFLUENCES, "micro_voids", 0.70),
            ("twins", RelationshipType.INFLUENCES, "grain_boundary", 0.75),
            ("carbonaceous_residue", RelationshipType.CAUSES, "micro_voids", 0.75),
            ("oxidation", RelationshipType.CAUSES, "micro_voids", 0.70),

            # Sustainability impacts
            ("cleaner_production", RelationshipType.INFLUENCES, "net_zero", 0.90),
            ("flux_free_sintering", RelationshipType.INFLUENCES, "cleaner_production", 0.95),
            ("cleaner_production", RelationshipType.REDUCES, "greenhouse_gas", 0.85),
            ("cleaner_production", RelationshipType.REDUCES, "vocs", 0.90),
            ("circular_economy", RelationshipType.REDUCES, "e_waste", 0.85),
            ("energy_efficiency", RelationshipType.INFLUENCES, "net_zero", 0.80),
            ("waste_minimization", RelationshipType.INFLUENCES, "circular_economy", 0.85),

            # Material hierarchy
            ("agnps", RelationshipType.HYPONYM, "silver", 1.0),
            ("silver", RelationshipType.HYPONYM, "metal", 1.0),
            ("pressureless_sintering", RelationshipType.HYPONYM, "sintering_process", 1.0),
            ("flux_free_sintering", RelationshipType.HYPONYM, "sintering_process", 1.0),

            # Process-material interaction
            ("pressureless_sintering", RelationshipType.CAUSES, "atomic_diffusion", 0.90),
            ("pressureless_sintering", RelationshipType.RESULTS_IN, "grain_boundary", 0.85),
            ("flux_free_sintering", RelationshipType.RESULTS_IN, "cleaner_production", 0.95),

            # Substrate interactions
            ("sic", RelationshipType.DEPENDS_ON, "pressureless_sintering", 0.80),
            ("gan", RelationshipType.DEPENDS_ON, "pressureless_sintering", 0.80),
            ("polymer_substrate", RelationshipType.DEPENDS_ON, "thermal_budget", 0.85),
        ]
...
class AdvancedConceptResolver:
    ...
    OPTIMIZED: Pre-computed ontology embeddings + batch resolve for 10-50x speedup.
    """
    ...
    def _context_disambiguation(self, text: str, context: str) -> Optional[str]:
        ...
        if 'defect' in text:
            ...
            elif is_sustainability and not is_defect:
...
class EnhancedConceptExtractor:
    ...
    def __init__(self, ontology: DomainOntology, resolver: AdvancedConceptResolver):
        ...
        self._compound_regex = re.compile(
            r'\b([a-z]+(?:[-\s][a-z]+){1,4})\s+(?:of|for|in|with|via|through|by|to|and|or|from)\s+([a-z]+(?:[-\s][a-z]+){0,3})\b',
            re.IGNORECASE
        )
    ...
    def _extract_noun_phrases(self, text: str) -> Set[str]:
        """Extract domain-relevant noun phrases using pre-compiled regex."""
...
# ==========================================
# --- DOMAIN KEYWORDS FOR AG NANOPARTICLES & SUSTAINABLE METALLURGY ---

AG_NANOPARTICLES_AND_SUBSTRATES = [
    "agnps", "ag nanoparticles", "silver nanoparticles", "nano-silver", "silver", "ag",
    "crystal lattice", "atomic architecture", "fcc", "face-centered cubic",
    "sic", "gan", "sic/gan", "semiconductor chips", "wide-bandgap semiconductors",
    "polymer substrates", "temperature-sensitive substrates",
    "organic capping agents", "reducing agents", "fluxes", "chemical additives",
    "carbonaceous residues", "micro-voids", "volatile organic compounds", "vocs"
]

SINTERING_AND_PRODUCTION_PROCESSES = [
    "pressureless sintering", "sintering onset", "sintering temperature", "sintering process",
    "mass transport", "atomic contact", "atomic diffusion", "surface diffusion",
    "power module assembly", "industrial-scale manufacturing", "scalable production",
    "post-sintering cleaning", "end-of-life recycling", "cleaner production",
    "sustainable metallurgy", "green metallurgy", "flux-free metallurgy",
    "material-level design", "intrinsic approach"
]

DEFECT_ENGINEERING_AND_THERMODYNAMICS = [
    "defect engineering", "crystal defects", "thermodynamic catalyst", "diffusion highways",
    "stacking faults", "intrinsic stacking faults", "isf", "extrinsic stacking faults", "esf",
    "twins", "deformation twins", "nanotwinning", "twin boundaries",
    "lattice distortion", "lattice strain", "microstrain", "planar defects",
    "internal catalysts", "atomic architecture", "crystal structure"
]

SUSTAINABILITY_AND_ENVIRONMENTAL_IMPACT = [
    "cleaner production", "sustainable metallurgy", "circular economy",
    "greenhouse gas emissions", "scope 1 emissions", "scope 2 emissions", "net-zero targets",
    "energy consumption", "thermal budget", "mechanical budget", "kilowatt-hour energy",
    "toxic emissions", "hazardous chemical additives", "resource wastage",
    "e-waste", "electronic waste", "critical raw materials", "zero-emission"
]

THERMAL_MECHANICAL_RESPONSE_AND_RELIABILITY = [
    "thermal shock", "residual stresses", "thermal degradation",
    "mechanical yield", "high-strength bonding", "dense bonding",
    "electrical conductivity", "thermal conductivity", "long-term reliability",
    "operational lifecycle", "mechanical properties", "yield strength",
    "thermal budget", "mechanical budget"
]

MICROSTRUCTURAL_DEFECTS = [
    "planar defects", "crystal defects", "defect density",
    "stacking faults", "twins", "twin boundaries", "grain boundaries",
    "micro-voids", "carbonaceous residues", "oxidation",
    "lattice strain", "microstrain", "lattice distortion", "isf", "esf"
]

CLEANER_PRODUCTION_METRICS = [
    "energy efficiency", "zero-emission", "flux-free process",
    "lifecycle assessment", "end-of-life", "recyclability",
    "yield improvement", "defect reduction", "waste minimization",
    "carbon footprint", "energy savings"
]

ALL_DOMAIN_KEYWORDS = (
    AG_NANOPARTICLES_AND_SUBSTRATES + SINTERING_AND_PRODUCTION_PROCESSES + 
    DEFECT_ENGINEERING_AND_THERMODYNAMICS + SUSTAINABILITY_AND_ENVIRONMENTAL_IMPACT +
    THERMAL_MECHANICAL_RESPONSE_AND_RELIABILITY + MICROSTRUCTURAL_DEFECTS +
    CLEANER_PRODUCTION_METRICS
)

AG_SINTERING_DEFECT_PATTERNS = [
    r'\b(?:agnps|ag\s+nanoparticles|silver\s+nanoparticles|nano-silver)\b',
    r'\b(?:pressureless\s+sintering|sintering\s+onset|sintering\s+temperature)\b',
    r'\b(?:defect\s+engineering|crystal\s+defects|intrinsic\s+approach|thermodynamic\s+catalyst)\b',
    r'\b(?:stacking\s+faults|intrinsic\s+stacking\s+faults|isf|extrinsic\s+stacking\s+faults|esf)\b',
    r'\b(?:twins|deformation\s+twins|nanotwinning|twin\s+boundaries)\b',
    r'\b(?:diffusion\s+highways|mass\s+transport|atomic\s+diffusion|surface\s+diffusion)\b',
    r'\b(?:sustainable\s+metallurgy|cleaner\s+production|circular\s+economy)\b',
    r'\b(?:greenhouse\s+gas\s+emissions|scope\s+[12]\s+emissions|net-zero\s+targets)\b',
    r'\b(?:thermal\s+budget|mechanical\s+budget|energy\s+consumption|thermal\s+shock)\b',
    r'\b(?:sic|gan|sic/gan|semiconductor\s+chips|polymer\s+substrates)\b',
    r'\b(?:volatile\s+organic\s+compounds|vocs|toxic\s+emissions|e-waste|electronic\s+waste)\b',
    r'\b(?:lattice\s+distortion|lattice\s+strain|microstrain|planar\s+defects)\b',
    r'\b(?:flux-free|zero-emission|end-of-life\s+recycling|material-level\s+design)\b'
]

AG_SUSTAINABILITY_CATEGORY_MAPPING = {
    r'agnps|ag\s+nanoparticles|silver\s+nanoparticles|nano-silver|sic|gan|polymer\s+substrates': 'core_material',
    r'pressureless\s+sintering|sintering\s+onset|mass\s+transport|atomic\s+diffusion|flux-free': 'sintering_process',
    r'defect\s+engineering|stacking\s+faults|twins|crystal\s+defects|isf|esf|intrinsic\s+approach': 'defect_engineering',
    r'sustainable\s+metallurgy|cleaner\s+production|circular\s+economy|greenhouse\s+gas|scope\s+[12]|net-zero|e-waste|vocs': 'sustainability_impact',
    r'thermal\s+shock|residual\s+stresses|thermal\s+degradation|mechanical\s+yield|electrical\s+conductivity|thermal\s+budget': 'thermal_mechanical_response',
    r'planar\s+defects|twin\s+boundaries|micro-voids|lattice\s+strain|lattice\s+distortion': 'microstructural_defects',
    r'energy\s+efficiency|zero-emission|lifecycle\s+assessment|recyclability|waste\s+minimization': 'cleaner_production_metrics'
}
...
def is_valid_ag_sintering_concept(concept: str) -> bool:
    concept_lower = concept.lower()
    has_domain = any(kw.lower() in concept_lower for kw in ALL_DOMAIN_KEYWORDS)
    has_pattern = any(re.search(p, concept, re.I) for p in AG_SINTERING_DEFECT_PATTERNS)
...
def normalize_ag_sintering_term(concept: str) -> str:
    concept = concept.lower().strip()
    # Material name normalizations
    concept = re.sub(r'\bag\s+nanoparticles\b', 'agnps', concept)
    concept = re.sub(r'\bsilver\s+nanoparticles\b', 'agnps', concept)
    concept = re.sub(r'\bnano-silver\b', 'agnps', concept)
    concept = re.sub(r'\bsilicon\s+carbide\b', 'sic', concept)
    concept = re.sub(r'\bgallium\s+nitride\b', 'gan', concept)
    # Sintering process normalizations
    concept = re.sub(r'\bpressureless\s+sintering\b', 'pressureless_sintering', concept)
    concept = re.sub(r'\bflux-free\s+sintering\b', 'flux_free_sintering', concept)
    concept = re.sub(r'\bflux\s+free\b', 'flux_free', concept)
    # Defect engineering normalizations
    concept = re.sub(r'\bdefect\s+engineering\b', 'defect_engineering', concept)
    concept = re.sub(r'\bstacking\s+faults\b', 'stacking_faults', concept)
    concept = re.sub(r'\bintrinsic\s+stacking\s+faults\b', 'isf', concept)
    concept = re.sub(r'\bextrinsic\s+stacking\s+faults\b', 'esf', concept)
    concept = re.sub(r'\bdeformation\s+twins\b', 'twins', concept)
    concept = re.sub(r'\btwin\s+boundaries\b', 'twin_boundaries', concept)
    concept = re.sub(r'\blattice\s+distortion\b', 'lattice_distortion', concept)
    concept = re.sub(r'\blattice\s+strain\b', 'lattice_strain', concept)
    concept = re.sub(r'\bdiffusion\s+highways\b', 'diffusion_highways', concept)
    # Sustainability normalizations
    concept = re.sub(r'\bcleaner\s+production\b', 'cleaner_production', concept)
    concept = re.sub(r'\bsustainable\s+metallurgy\b', 'sustainable_metallurgy', concept)
    concept = re.sub(r'\bgreen\s+metallurgy\b', 'green_metallurgy', concept)
    concept = re.sub(r'\bcircular\s+economy\b', 'circular_economy', concept)
    concept = re.sub(r'\bgreenhouse\s+gas\s+emissions\b', 'greenhouse_gas', concept)
    concept = re.sub(r'\bscope\s+1\s+emissions\b', 'scope_1', concept)
    concept = re.sub(r'\bscope\s+2\s+emissions\b', 'scope_2', concept)
    concept = re.sub(r'\bnet-zero\b', 'net_zero', concept)
    concept = re.sub(r'\be-waste\b', 'e_waste', concept)
    concept = re.sub(r'\belectronic\s+waste\b', 'e_waste', concept)
    concept = re.sub(r'\bvolatile\s+organic\s+compounds\b', 'vocs', concept)
    # Thermal-mechanical normalizations
    concept = re.sub(r'\bthermal\s+shock\b', 'thermal_shock', concept)
    concept = re.sub(r'\bresidual\s+stresses\b', 'residual_stress', concept)
    concept = re.sub(r'\bmechanical\s+yield\b', 'mechanical_yield', concept)
    concept = re.sub(r'\belectrical\s+conductivity\b', 'electrical_conductivity', concept)
    concept = re.sub(r'\bthermal\s+conductivity\b', 'thermal_conductivity', concept)
    concept = re.sub(r'\bhigh-strength\s+bonding\b', 'high_strength_bonding', concept)
    concept = re.sub(r'\bthermal\s+budget\b', 'thermal_budget', concept)
    concept = re.sub(r'\bmechanical\s+budget\b', 'mechanical_budget', concept)
    # Microstructural normalizations
    concept = re.sub(r'\bmicro-voids\b', 'micro_voids', concept)
    concept = re.sub(r'\bcarbonaceous\s+residues\b', 'carbonaceous_residue', concept)
    concept = re.sub(r'\bgrain\s+boundaries\b', 'grain_boundary', concept)
    # Metric normalizations
    concept = re.sub(r'\benergy\s+efficiency\b', 'energy_efficiency', concept)
    concept = re.sub(r'\blifecycle\s+assessment\b', 'lifecycle_assessment', concept)
    concept = re.sub(r'\bwaste\s+minimization\b', 'waste_minimization', concept)
    concept = re.sub(r'\bcarbon\s+footprint\b', 'carbon_footprint', concept)
    return concept

def extract_concepts_from_text(text: str) -> List[str]:
    concepts = set()
    text_lower = text.lower()
    for pattern in AG_SINTERING_DEFECT_PATTERNS:
    ...
    for m in matches:
        ...
        if is_valid_ag_sintering_concept(concept):
            concepts.add(concept)
...
def extract_concepts_from_abstracts(df: pd.DataFrame, text_columns: List[str]) -> Tuple[List[List[str]], List[Dict]]:
    ...
    for idx, row in df.iterrows():
        ...
        normalized = [normalize_ag_sintering_term(c) for c in concepts]
...
def normalize_and_filter_concepts(all_concepts: List[List[str]], config: Dict) -> Tuple[List[str], Dict[str, int], Dict[int, str], Dict[str, List[int]]]:
    ...
    for doc_idx, concepts in enumerate(all_concepts):
        seen_in_doc = set()
        for c in concepts:
            if c not in seen_in_doc and is_valid_ag_sintering_concept(c):
                concept_counts[c] += 1
...
def abstract_concepts_to_categories(concepts: List[str]) -> Dict[str, str]:
    ...
    for concept in concepts:
        matched = False
        for pattern, category in AG_SUSTAINABILITY_CATEGORY_MAPPING.items():
...
def build_hybrid_graph(all_concepts: List[List[str]], valid_concepts: List[str],
                        ...
                        ontology=None) -> nx.Graph:
    if config is None:
        config = get_adaptive_config(3000)
    nx_graph = nx.Graph()
    # Add nodes with definition (if ontology provided)
    for c in valid_concepts:
        definition = ""
        if ontology and c in ontology.concepts:
            definition = ontology.get_definition(c)
        nx_graph.add_node(c, frequency=0, definition=definition)
    for concepts in all_concepts:
        valid_in_doc = [c for c in concepts if c in concept_to_id]
        for i in range(len(valid_in_doc)):
            for j in range(i + 1, len(valid_in_doc)):
...
def export_publication_figure(nx_graph, valid_concepts, concept_abstract_map, 
                              ...
                              filename="agnp_graph_pub.png") -> bytes:
    ...
    try:
        ...
        nx.draw(nx_graph, pos, 
                ...
                width=0.5,
                alpha=0.9)
        plt.title("Ag Nanoparticles Sustainable Metallurgy Concept Graph", fontsize=14, fontweight='bold', pad=20)
        buf = io.BytesIO()
...
def generate_analysis_report(nx_graph, valid_concepts, concept_abstract_map, 
    ...
    report = []
    report.append("# Ag Nanoparticles Sustainable Metallurgy Concept Graph Analysis Report")
...
def render_graph_pyvis(nx_graph, concept_abstract_map, physics_enabled=True,
    ...
    If show_definitions is True, concept definitions are included in tooltips.
    Clicking a node shows a side panel with the full definition and connected edges.
    """
    ...
    for i, node in enumerate(nx_graph.nodes()):
        ...
        # --- BUILD TOOLTIP WITH DEFINITION (CONDITIONAL) ---
        tooltip_content = (
        ...
        # Conditionally add definition
        if show_definitions and definition:
            tooltip_content += (
                f"<br><span style='color:{theme['tooltip_text']};opacity:0.7;'>Definition:</span> "
                f"<i>{definition}</i>"
            )
        ...
        net.add_node(
            ...
            font=font_dict,
            title=tooltip_content,   # <--- tooltip includes definition
    ...
    # --- HTML TOOLTIP FIX: Enable HTML parsing in node titles ---
    # VisJS disabled HTML in titles for XSS protection. We re-enable it safely
    # by injecting htmlTitle() function that creates DOM elements from HTML strings.
    # This approach works with all vis-network versions (unlike CDN rollback).
    html_title_js = """
    <script type="text/javascript">
    // Enable HTML rendering in node tooltips (VisJS XSS-safe approach)
    ...
    // Transform all node title strings to htmlTitle() calls after network initialization
    (function() {
        var checkExist = setInterval(function() {
            if (typeof network !== 'undefined' && network !== null && 
                network.body && network.body.data && network.body.data.nodes) {
                ...
                for (var i = 0; i < allNodes.length; i++) {
                    ...
                    if (node.title && typeof node.title === 'string' && 
                        (node.title.indexOf('<') !== -1 || node.title.indexOf('&') !== -1)) {
                        updatedNodes.push({
                            id: node.id,
    ...
    """
    html_content = html_content.replace('</body>', html_title_js + '</body>')

    # --- NODE HIGHLIGHT & EDGE DESCRIPTION INJECTION (ENHANCED) ---
    if enable_node_highlight:
        # Retrieve display mode from session state
        edge_display_mode = st.session_state.get('edge_display_mode', 'Original Names')
        use_short_names_js = "true" if edge_display_mode == "Short Names (N1, N2...)" else "false"

        # Updated JavaScript with extraction and beautified edge list
        highlight_js = """
    <script>
    (function() {
        var useShortNames = """ + use_short_names_js + """;
        ...
        var checkExist = setInterval(function() {
            if (typeof network !== 'undefined' && network !== null && network.body && network.body.data) {
                ...
                function showEdgeInfoPanel(nodeId, connectedEdges) {
                    ...
                    panel.style.cssText = [
                        'position:fixed','top:110px','right:24px','width:380px',
                        'max-height:600px','overflow-y:auto','z-index:9999',
                    ...
                    var html = '<div style="font-size:16px;font-weight:700;color:#D32F2F;border-bottom:2px solid #FFD700;padding-bottom:8px;margin-bottom:8px;">'
                        + '🔗 ' + nodeLabel + ' <span style="font-size:12px;font-weight:400;color:#666;">(' + connectedEdges.length + ' edges)</span></div>';

                    if (nodeTitle) {
                        html += '<div style="margin:6px 0 12px 0;padding:10px 12px;background:rgba(0,0,0,0.04);border-radius:8px;font-size:13px;line-height:1.6;color:#334155;max-height:200px;overflow-y:auto;border-left:3px solid #FFD700;">'
                            + nodeTitle + '</div>';
                    }

                    html += '<div style="font-size:12px;color:#64748b;margin-top:6px;border-top:1px solid #eee;padding-top:8px;">Connected Edges:</div>';
                    var edgeList = [];
                    ...
                    connectedEdges.forEach(function(eId) {
                        ...
                        var fromOrig = fromNode && fromNode.title ? extractOriginalName(fromNode.title) : fromLabel;
                        var toOrig   = toNode   && toNode.title   ? extractOriginalName(toNode.title)   : toLabel;
                        ...
                        edgeList.push({
                            ...
                            fromShort: fromLabel, fromOrig: fromOrig,
                            toShort: toLabel, toOrig: toOrig,
                    ...
                    edgeList.forEach(function(e, idx){
                        ...
                        var badge = e.inferred ? ' <span style="background:#8b5cf6;color:white;font-size:9px;padding:2px 6px;border-radius:4px;margin-left:4px;vertical-align:middle;">INF</span>' : '';

                        var displayFrom = useShortNames ? e.fromShort : e.fromOrig;
                        var displayTo   = useShortNames ? e.toShort   : e.toOrig;

                        html += '<div style="padding:8px 12px;margin:4px 0;background:#f8fafc;border-left:3px solid ' + typeColor + ';border-radius:6px;box-shadow:0 1px 2px rgba(0,0,0,0.05);">';
                        html += '<div style="font-size:13px;font-weight:600;color:#1e293b;">' + displayFrom + ' ↔ ' + displayTo + '</div>';
                        html += '<div style="font-size:11px;color:#64748b;margin-top:3px;">Weight: <b style="color:#0ea5e9;">' + e.weight + '</b> | Type: ' + e.type + badge + '</div>';
                        html += '</div>';
                    });
    ...
    # Render Legend if abbreviated labels
...
def render_sidebar():
    with st.sidebar:
        ...
        st.subheader("Ag Nanoparticles & Sustainable Metallurgy Focus Areas")
        st.markdown("- **Core Materials**: AgNPs, SiC/GaN semiconductor chips, polymer substrates")
        st.markdown("- **Sintering Processes**: Pressureless sintering, mass transport, atomic/surface diffusion")
        st.markdown("- **Defect Engineering**: Stacking faults (ISF/ESF), twins, thermodynamic catalysts, intrinsic approach")
        st.markdown("- **Sustainability Impact**: Cleaner production, circular economy, Scope 1 & 2 GHG emissions, net-zero targets")
        st.markdown("- **Thermal-Mechanical Response**: Thermal shock, residual stresses, mechanical yield, thermal/electrical conductivity")
        st.markdown("- **Microstructural Defects**: Planar defects, twin boundaries, micro-voids, lattice strain/distortion")
        st.markdown("- **Cleaner Production Metrics**: Energy efficiency, zero-emission, lifecycle assessment, e-waste minimization")
        ...
        # --- Toggle for tooltip definitions ---
        st.session_state['show_definitions'] = st.toggle(
            "Show concept definitions in tooltips", value=True,
            help="When enabled, hover tooltips display the definition of each concept."
        )
        ...
        # --- NEW: Side Panel Display Mode ---
        st.subheader("Side Panel Display")
        st.session_state['edge_display_mode'] = st.selectbox(
            "Edge list node names:",
            ["Original Names", "Short Names (N1, N2...)"],
            index=0,
            help="Choose whether to show full concept names or abbreviated N1, N2... labels in the node selection side panel."
        )
...
def render_reasoning_dashboard(nx_graph, valid_concepts, ontology, extractor):
    ...
    st.subheader("🔍 Defect Engineering & Sustainability Reasoning Insights")
    ...
    synonym_examples = [
        ...
        ("defect engineering", "defect_engineering"),
        ("stacking faults", "stacking_faults"),
    ]
    syn_data = []
...
def main():
    ...
    if st.button("Build AgNP Sustainability Concept Graph with Reasoning", type="primary", use_container_width=True):
        ...
        try:
            with status:
                ...
                if use_ontology and use_inference:
                    ...
                    # Use legacy graph builder, passing ontology
                    nx_graph = build_hybrid_graph(
                        all_concepts, valid_concepts, concept_to_id, 
                        embed_model, config, ontology=ontology
                    )
        ...
        with tabs[tab_idx]:
            ...
            if top_scores.empty:
                ...
                st.download_button("Download Scores (CSV)", data=csv_scores,
                                  file_name="agnp_research_directions.csv", mime="text/csv")
        ...
        with tabs[tab_idx]:
            ...
            if st.button("Generate Publication Figure"):
                ...
                if pub_bytes:
                    st.download_button("Download Publication PNG", data=pub_bytes,
                                     file_name="agnp_graph_publication.png", mime="image/png")
            ...
            if st.button("Generate Markdown Report"):
                ...
                st.download_button("Download Report (Markdown)", data=report.encode('utf-8'),
                                  file_name="agnp_sustainability_analysis_report.md", mime="text/markdown")
            ...
            st.download_button("Download Concept List (CSV)", data=csv_concepts,
                              file_name="agnp_concepts.csv", mime="text/csv")
