...
class AdvancedConceptResolver:
    ...
    def __init__(self, ontology: DomainOntology, embed_model):
        ...
        # Precompute ontology embedding matrix for O(1) batch matching
    ...
    def _precompute_ontology_embeddings(self):
        """Precompute and cache the embedding matrix for all ontology concepts."""
        concepts = []
        embeddings = []
        for canonical, node in self.ontology.concepts.items():
        ...
        self.ontology_embedding_matrix = np.array(embeddings)

    def _embedding_match(self, text: str, context: str = "") -> Optional[str]:
        """Find most similar concept using batch matrix multiplication."""
        try:
            ...
            # Single batch operation replaces thousands of Python loops
    ...
    def _context_disambiguation(self, text: str, context: str) -> Optional[str]:
        ...
        if 'defect' in text:
            if is_defect and not is_sustainability:
                return "stacking_faults"  # crystal defect
            elif is_sustainability and not is_defect:
                return "waste_minimization"  # process defect/waste
            elif is_sintering:
                return "micro_voids"  # sintering defect

        # "Diffusion" disambiguation
        if 'diffusion' in text:
            if is_sintering:
                return "atomic_diffusion"  # sintering diffusion
            elif is_defect:
                return "diffusion_highways"  # defect-enhanced diffusion
...
class ReasoningEnhancedGraphBuilder:
    ...
    def _add_semantic_edges(self, nx_graph: nx.Graph, valid_concepts: List[str],
        ...
        """Add edges based on embedding similarity using vectorized NumPy operations."""
        try:
            ...
            rows, cols = np.where(upper_tri > sim_thresh)
            for r, c in zip(rows, cols):
                c1, c2 = valid_concepts[r], valid_concepts[c]
                if not nx_graph.has_edge(c1, c2):
                    if nx_graph.degree(c1) < 3 or nx_graph.degree(c2) < 3:
                        nx_graph.add_edge(c1, c2,
                                          weight=sim_matrix[r, c] * 2,
                                          cooccurrence=0,
                                          semantic=sim_matrix[r, c],
                                          edge_type='semantic',
                                          inferred=False)
        except Exception as e:
...
@st.cache_data(show_spinner=False)
def cached_encode_texts(texts_tuple, model_id):
    """Cache text embeddings to avoid redundant GPU/CPU inference."""
    embed_model = load_embedding_model()
    return embed_model.encode(list(texts_tuple), show_progress_bar=False, batch_size=64)
...
def cluster_similar_concepts(valid_concepts: List[str], embed_model, similarity_threshold: float = 0.75):
    ...
    try:
        embeddings = cached_encode_texts(tuple(valid_concepts), id(embed_model))
    ...
    for i, c in enumerate(valid_concepts):
        ...
        if freq > 1 and doc_corpus[i].strip():
            try:
                ...
                concept_embeddings = cached_encode_texts(tuple(words), id(embed_model))
    ...
    if embed_model and len(valid_concepts) >= 10:
        try:
            ...
            for r, c in zip(rows, cols):
                ...
                if not nx_graph.has_edge(c1, c2):
                    if nx_graph.degree(c1) < 3 or nx_graph.degree(c2) < 3:
                        nx_graph.add_edge(c1, c2, weight=sim_matrix[r, c] * 2,
                                         cooccurrence=0, semantic=sim_matrix[r, c],
                                         edge_type='semantic')
        except Exception as e:
            st.warning(f"Semantic edge addition skipped: {e}")
...
def compute_research_direction_scores(model, node_features, final_emb, nx_graph,
    ...
    emb_np = cached_encode_texts(tuple(valid_concepts), id(embed_model))
...
def detect_semantic_drift(df_filtered: pd.DataFrame, valid_concepts: List[str],
    ...
    for concept in valid_concepts:
        ...
        if len(early_texts) < 3 or len(late_texts) < 3:
            continue
        try:
            early_emb = cached_encode_texts(tuple(early_texts), id(embed_model))
            late_emb = cached_encode_texts(tuple(late_texts), id(embed_model))
        ...
        try:
            with status:
                ...
                if use_ontology and extractor is not None:
                    # Use enhanced extraction with caching
                    for idx, row in df_filtered.iterrows():
                        text = " ".join([str(row[col]) for col in selected_text_cols 
                                        if col in row and pd.notna(row[col])])
                        # Cache key based on text content + extraction settings
                        cw = st.session_state.get('context_window', 50)
                        cache_key = f"{use_ontology}_{use_embedding}_{use_inference}_{cw}_{compute_text_hash(text)}"
                        if cache_key not in st.session_state:
                            st.session_state[cache_key] = extractor.extract_from_text(text, idx)
                        concepts = st.session_state[cache_key]
                    ...
                    # Use legacy extraction with caching
                    legacy_cache_key = f"legacy_{compute_text_hash(str(selected_text_cols))}_{len(df_filtered)}"
                    if legacy_cache_key not in st.session_state:
                        st.session_state[legacy_cache_key] = extract_concepts_from_abstracts(df_filtered, selected_text_cols)
                    all_concepts, all_metrics = st.session_state[legacy_cache_key]
