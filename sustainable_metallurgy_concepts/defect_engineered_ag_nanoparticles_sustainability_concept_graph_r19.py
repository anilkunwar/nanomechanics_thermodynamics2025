...
class ReasoningEnhancedGraphBuilder:
    ...
    def build_graph(self, all_concepts: List[List[str]], valid_concepts: List[str],
        ...
        for c in valid_concepts:
            ...
            definition = self.ontology.get_definition(c) if c in self.ontology.concepts else ""
            nx_graph.add_node(c, 
                            frequency=freq,
                            concept_type=concept_type.value,
                            degree=0,
                            definition=definition)   # <--- ADDED DEFINITION
...
def build_hybrid_graph(all_concepts: List[List[str]], valid_concepts: List[str],
    ...
    if config is None:
        config = get_adaptive_config(3000)
    nx_graph = nx.Graph()
    # Add nodes with definition (if available from ontology, otherwise empty)
    ontology = st.session_state.get('ontology')  # Might not be defined if not using ontology
    for c in valid_concepts:
        definition = ""
...
def render_graph_pyvis(nx_graph, concept_abstract_map, physics_enabled=True,
                        ...
                        show_definitions=False):   # <--- NEW PARAMETER
    """
    ...
    for i, node in enumerate(nx_graph.nodes()):
        ...
        # Retrieve definition from node attributes
...
def render_sidebar():
    with st.sidebar:
        ...
        st.session_state['show_definitions'] = st.toggle(
            "Show concept definitions in tooltips", value=False,
...
def main():
    ...
    if st.session_state.analysis_data is not None:
        ...
        with tabs[tab_idx]:
            ...
            show_definitions = st.session_state.get('show_definitions', False)   # <--- NEW
            ...
            if viz_choice == "PyVis (Interactive)":
                render_graph_pyvis(
                    ...
                    show_definitions=show_definitions   # <--- NEW
                )
