...
def render_sunburst_chart(labels, parents, values, cmap_name="viridis",
    ...
    and full customization. Each slice gets its own hue from a continuous
    ...
    # ═══════════════════════════════════════════════════════
    #  COLORFUL PER-NODE COLORING (the key fix)
    # ═══════════════════════════════════════════════════════
    # Instead of one color per category, generate a unique color
    # for EVERY node by sampling a continuous colormap.
    # This produces the rainbow/multi-hued effect like image 1.
    # ──

    n_nodes = len(labels)
    # Pick colormap: use passed color_continuous_scale name, or cmap_name,
    # or default to "Spectral" for maximum color variety
    cmap_to_use = color_continuous_scale or cmap_name or "Spectral"

    try:
        # Sample n_nodes distinct colors from the colormap
        import matplotlib.pyplot as plt
        cmap_obj = plt.cm.get_cmap(cmap_to_use)
        # Evenly spaced samples across the full colormap range
        ...
        # Fallback: use Plotly's qualitative palettes for variety
        try:
            from plotly.express import colors as px_colors
            # Build a long diverse palette by cycling through multiple qualitative sets
            qual_palettes = [
                ...
                px_colors.qualitative.Alphabet,
            ]
            long_palette = []
            ...
            # Ultimate fallback: matplotlib tab20
    ...
    with st.sidebar:
        ...
        with col_slider1:
            st.session_state['top_n_graph'] = st.slider(
                "Max nodes", 10, 500, 200, step=5, disabled=all_graph,
