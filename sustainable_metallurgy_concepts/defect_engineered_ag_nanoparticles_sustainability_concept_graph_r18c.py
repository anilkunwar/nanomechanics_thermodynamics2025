        ...
        var checkExist = setInterval(function() {
            if (typeof network !== 'undefined' && network !== null && network.body && network.body.data) {
                ...
                function showEdgeInfoPanel(nodeId, edgeList) {
                    ...
                    // Extract full tooltip content (definition, type, etc.)
                    var nodeTooltipHtml = "";
                    var nodeDefinition = "";
                    var nodeType = "";
                    var nodeFreq = "";
                    ...
                    if (nodeData && nodeData.title) {
                        nodeTooltipHtml = nodeData.title;
                        // Parse the HTML tooltip to extract structured fields
                        var tmpDiv = document.createElement("div");
                        tmpDiv.innerHTML = nodeTooltipHtml;
                    ...
                    // ── Concept Metadata Section (Type, Definition) ──
                    if (nodeType || nodeDefinition || nodeFreq || nodeDegree) {
                        html += '<div style="margin:8px 0;padding:10px;background:rgba(0,0,0,0.04);border-radius:6px;font-size:12px;line-height:1.5;color:#334155;">';
                        if (nodeType) {
                            html += '<div style="margin-bottom:4px;"><span style="color:#64748b;font-weight:500;">Type:</span> <span style="color:#1e293b;font-weight:600;">' + nodeType + '</span></div>';
                        }
                        if (nodeDegree) {
                            html += '<div style="margin-bottom:4px;"><span style="color:#64748b;font-weight:500;">Degree:</span> <span style="color:#1e293b;">' + nodeDegree + '</span></div>';
                        }
                        if (nodeFreq) {
                            html += '<div style="margin-bottom:4px;"><span style="color:#64748b;font-weight:500;">Frequency:</span> <span style="color:#1e293b;">' + nodeFreq + '</span></div>';
                        }
                        if (nodeDefinition) {
                            html += '<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(0,0,0,0.08);"><span style="color:#64748b;font-weight:500;">Definition:</span> <i style="color:#0f172a;">' + nodeDefinition + '</i></div>';
                        }
                        html += '</div>';
                    }

                    html += '<div style="font-size:10.5px;color:#64748b;margin-bottom:10px;'
                ...
                network.on("selectNode", function(params) {
                    ...
                    // 1) Gold ring on connected nodes (ONLY nodesDS.update, NEVER edgesDS)
                    var nodeUpdates = [];
                    ...
                    // 2) Collect edge metadata (read-only from edgesDS) → side panel
                    var edgeInfoList = [];
