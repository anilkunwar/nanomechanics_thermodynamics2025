...
def render_sunburst_chart(labels, parents, values, cmap_name="viridis", label_size=14, 
    ...
    for i, parent in enumerate(unique_parents):
        with cols[i % len(cols)]:
            sym = parent_symbol_map[parent]
            col = parent_color_map[parent]
            st.markdown(f"""
            <div style='padding:10px; border-radius:8px; background-color:{col}15; 
                        border-left:5px solid {col}; margin-bottom:10px;'>
                <span style='font-size:20px; color:{col}; margin-right:8px;'>{sym}</span>
                <b style='color:{col}; font-size:14px;'>{parent}</b>
            </div>
