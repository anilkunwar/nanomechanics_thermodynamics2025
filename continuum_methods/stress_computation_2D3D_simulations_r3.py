# 2D vs 3D Stress — multi-field visualization (based on user's working baseline)
...
# ------------------- 3D Solver (extended from your baseline to return full tensors) -------------------
@st.cache_data
def compute_stress_3d_full(eta, eps0, theta, phi, dx):
    ...
    # principal stresses on the central slice (cheap and directly comparable to 2D)
    slice_idx = N//2
    sxx_s = sxx[:, :, slice_idx]; syy_s = syy[:, :, slice_idx]; szz_s = szz[:, :, slice_idx]
    sxy_s = sxy[:, :, slice_idx]; sxz_s = sxz[:, :, slice_idx]; syz_s = syz[:, :, slice_idx]

    # build 3x3 per-voxel for central slice and compute eigenvalues
    princ1 = np.zeros((N,N)); princ2 = np.zeros((N,N)); princ3 = np.zeros((N,N))
    for (i,j) in np.ndindex(N,N):
        S = np.array([[sxx_s[i,j], sxy_s[i,j], sxz_s[i,j]],
                      [sxy_s[i,j], syy_s[i,j], syz_s[i,j]],
                      [sxz_s[i,j], syz_s[i,j], szz_s[i,j]]])
    ...
    eps_total_slice = eps_total[:, :, :, :, slice_idx]  # careful indexing: eps_total[comp_i,comp_j, x,y,z]
    # rearrange to per-voxel 3x3
    for (i,j) in np.ndindex(N,N):
        E = np.array([[eps_total[0,0,i,j,slice_idx], eps_total[0,1,i,j,slice_idx], eps_total[0,2,i,j,slice_idx]],
                      [eps_total[1,0,i,j,slice_idx], eps_total[1,1,i,j,slice_idx], eps_total[1,2,i,j,slice_idx]],
                      [eps_total[2,0,i,j,slice_idx], eps_total[2,1,i,j,slice_idx], eps_total[2,2,i,j,slice_idx]]])
...
# ------------------- 2D Plane-Strain Solver (stable, extended to return components) -------------------
@st.cache_data
def compute_stress_2d_full(eta2d, eps0, theta, dx):
    ...
    # plane-strain isotropic constants (consistent with your baseline)
    Cp11 = 124e9 - 93.4e9**2 / 124e9   # kept from your baseline
...
FIELDS_3D = {
    ...
    'Principal ε1 (central slice, ×1e-3)': ('ep1_slice', False),
}
FIELDS_2D = {
    'von Mises': ('vm', True),
    'Hydrostatic (mean) stress': ('hydro', True),
    ...
    'Principal ε1 (2D, ×1e-3)': ('ep1', False),
}
...
if st.button("Run 2D vs 3D Comparison", type="primary"):
    ...
    # central slice from 3D outputs (Pa)
    arr3 = out3[key3] if key3.endswith('_slice') else out3[key3][:, :, N//2]
    arr2 = out2[key2]

    # convert units: stresses -> GPa; strains -> x1e-3
    if is_stress3 and is_stress2:
        arr3_disp = arr3 / 1e9
        arr2_disp = arr2 / 1e9
        units = "GPa"
    else:
        # either strains or mixed; show strains scaled by 1e3
        # if user accidentally selects strains for one and stresses for the other, we still show raw arrays but label carefully
        arr3_disp = arr3 * (1e3 if not is_stress3 else 1e-9)
        arr2_disp = arr2 * (1e3 if not is_stress2 else 1e-9)
        units = "GPa (stress)/×10^-3 (strain) — check selection"
    ...
    with col1:
        st.metric(f"3D Max ({field3d})", f"{max3:.4g} {units}")
    ...
    with col2:
        st.metric(f"2D Max ({field2d})", f"{max2:.4g} {units}")
    ...
    with col3:
        st.metric("Max |3D−2D|", f"{maxdiff:.4g} {units}")
    ...
    st.success(f"Done. Max diff = {maxdiff:.4g} {units}")

    # small CSV download of central slice comparison
    import pandas as pd
    flat_eta = eta_2d.flatten()
    flat3 = arr3_disp.flatten()
    flat2 = arr2_disp.flatten()
    df = pd.DataFrame({'eta': flat_eta, '3D': flat3, '2D': flat2, 'diff': (flat3-flat2)})
    st.download_button("Download slice comparison CSV", df.to_csv(index=False), file_name="slice_compare.csv")

    # show a few stats
    st.write("### Quick stats")
    st.write(pd.DataFrame({
        'quantity': ['3D max', '2D max', 'max abs diff'],
        'value': [f"{max3:.6g} {units}", f"{max2:.6g} {units}", f"{maxdiff:.6g} {units}"]
    }))
