# 2D vs 3D — Three comparison modes (Option 1/2/3) with dropdown
...
# ------------------- 2D plane-strain solver (robust) -------------------
@st.cache_data
def compute_stress_2d_plane(eta2d, eps0, theta, dx):
    ...
    # plane-strain out-of-plane
    ezz = -nu_plane/(1-nu_plane) * (exx + eyy)
...
if st.button("Run comparison", type="primary"):
    ...
    sxx3 = sol3['sxx']; syy3 = sol3['syy']; szz3 = sol3['szz']; sxy3 = sol3['sxy']
    slice_idx = N//2
    # central-slice von Mises (Pa)
    vm3_slice = np.sqrt(0.5*((sxx3-syy3)**2 + (syy3-szz3)**2 + (szz3-sxx3)**2 + 6*(sxy3**2)))
    ...
    if mode.startswith("Option 1"):
        ...
        vm3_full = np.sqrt(0.5*((sxx3-syy3)**2 + (syy3-szz3)**2 + (szz3-sxx3)**2 + 6*(sxy3**2)))
        ...
        with st.spinner("Running 2D plane-strain solver..."):
            sol2 = compute_stress_2d_plane(eta_2d, eps0, theta, dx_nm)
        vm2 = np.sqrt(sol2['sxx']**2 + sol2['syy']**2 - sol2['sxx']*sol2['syy'] + 3*sol2['sxy']**2)
