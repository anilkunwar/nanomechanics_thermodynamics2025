...
Four fields exported • Custom shapes • Real eigenstrain values • Anisotropic elasticity • Multiple variants
""")
# =============================================
# Material & Grid
# =============================================
...
# =============================================
# NEW: Multiple Variants for {111} planes
# =============================================
st.sidebar.header("Multiple Variants ({111} planes)")
variant_options = [
    "Variant 1 (0°)",
    "Variant 2 (60°)",
    "Variant 3 (120°)",
...
thetas = [np.deg2rad(theta_map[v]) for v in selected_variants]
num_variants = len(selected_variants)
beta = 1.0  # Coupling coefficient for multi-variant interaction
if num_variants > 1:
    st.sidebar.info(f"Multiple variants active: Coupling β = {beta:.1f}")
# =============================================
...
init_etas = [create_initial_eta(shape) for _ in selected_variants]
init_eta_display = np.max(init_etas, axis=0) if num_variants > 1 else init_etas[0]
# Enhanced figure with better styling
fig0, ax0 = plt.subplots(figsize=(8, 6))
im0 = ax0.imshow(init_eta_display, extent=extent, cmap=eta_cmap, origin='lower',
                 vmin=colorbar_limits['eta'][0], vmax=colorbar_limits['eta'][1])
if show_contours:
    ax0.contour(X, Y, init_eta_display, levels=[contour_level], colors=contour_color,
...
with st.sidebar.expander("About Plasticity & Model Physics"):
    ...
    **Phase-field approach** models defect evolution through:
    ...
    - **Anisotropic elasticity**: Using cubic constants for Ag
    - **Multiple variants**: Up to 4 {111} plane variants with coupling
    """)
# =============================================
# Numba-safe Multi-Variant Allen-Cahn
# =============================================
@jit(nopython=True, parallel=True)
def evolve_phase_fields(etas, kappa, beta, dt, dx, N):
    num_v = etas.shape[0]
    etas_new = etas.copy()
    dx2 = dx * dx
    for v in prange(num_v):
        eta = etas[v]
        eta_new = etas_new[v]
        sum_other = np.zeros((N, N))
        for w in prange(num_v):
            if w != v:
                sum_other += etas[w]**2
        for i in prange(1, N-1):
            for j in prange(1, N-1):
                lap = (eta[i+1,j] + eta[i-1,j] + eta[i,j+1] + eta[i,j-1] - 4*eta[i,j]) / dx2
                dF = 2*eta[i,j]*(1-eta[i,j])*(eta[i,j]-0.5) + beta * eta[i,j] * sum_other[i,j]
    ...
    return etas_new
# =============================================
# FFT Stress Solver (upgraded with anisotropy)
# =============================================
@st.cache_data
def compute_stress_fields(etas, eps0, thetas):
    """
    Full 2D plane-strain anisotropic elasticity with rotated eigenstrain
    Valid for any in-plane {111} orientation, cubic anisotropy
    """
    ...
    # Total eigenstrain from all variants
    eps_xx_star = np.zeros((N, N))
    eps_yy_star = np.zeros((N, N))
    eps_xy_star = np.zeros((N, N))
    for v, theta in enumerate(thetas):
        eta_v = etas[v]
        ...
        eps_xx_star += eps_star[0,0] * eta_v
        eps_yy_star += eps_star[1,1] * eta_v
        eps_xy_star += eps_star[0,1] * eta_v
...
if st.button("🚀 Run Phase-Field Evolution", type="primary"):
    with st.spinner("Running crystallographically accurate simulation..."):
        etas = np.array([init_eta for init_eta in init_etas])
        history = []
        for step in range(steps + 1):
            if step > 0:
                etas = evolve_phase_fields(etas, kappa, beta, dt=0.004, dx=dx, N=N)
            if step % save_every == 0 or step == steps:
                sm, sh, vm = compute_stress_fields(etas, eps0, thetas)
                history.append((etas.copy(), sm.copy(), sh.copy(), vm.copy()))
...
if 'history' in st.session_state:
    ...
    etas, sigma_mag, sigma_hydro, von_mises = st.session_state.history[frame]
    eta_display = np.max(etas, axis=0) if num_variants > 1 else etas[0]
    # Display current field statistics
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.metric("η Range", f"{eta_display.min():.3f} - {eta_display.max():.3f}")
    with col2:
    ...
    fields = [eta_display, sigma_mag, sigma_hydro, von_mises]
    ...
    for ax, field, cmap, title, cbar_label, field_key in zip(
        ...
        if show_contours:
            ax.contour(x, y, eta_display, levels=[contour_level], colors=contour_color,
                      linewidths=line_width, alpha=contour_alpha)
    ...
    st.pyplot(fig)
    # =============================================
    # Download (all 4 fields) - updated for variants
    # =============================================
    buffer = BytesIO()
    with zipfile.ZipFile(buffer, "w", zipfile.ZIP_DEFLATED) as zf:
        for i, (es, sm, sh, vm) in enumerate(st.session_state.history):
            # Save combined eta for simplicity
            e_combined = np.max(es, axis=0)
            df = pd.DataFrame({
                'eta': e_combined.flatten(order='F'), 'stress_magnitude': sm.flatten(order='F'),
...
<ImageData WholeExtent="0 {N-1} 0 {N-1} 0 0" Origin="{extent[0]} {extent[2]} 0" Spacing="{dx} {dx} 1">
  <Piece Extent="0 {N-1} 0 {N-1} 0 0">
    <PointData>
      <DataArray type="Float32" Name="eta" format="ascii">{flat(e_combined)}</DataArray>
    ...
    st.download_button(
        ...
        buffer,
        f"Ag_NP_{defect_type}_MultiVariant_Simulation.zip",
        "application/zip"
    )
# =============================================
...
with st.expander("🔍 Theoretical Soundness Analysis"):
    ...
    - **Anisotropic elasticity**: Cubic constants C11=124 GPa, C12=93.4 GPa, C44=46.1 GPa
    - **Multiple variants**: Up to 4 {111} projections with coupling
   
    ### ⚠️ **Limitations & Improvements:**
    - **2D approximation**: Real defects are 3D; consider 3D FFT
    - **Fixed crystal axes**: Anisotropy aligned to lab frame
    ...
    - **Anisotropy**: Better captures FCC behavior
    - **Variants**: Allows multiple {111} planes
   
    **Phase-field plasticity** bridges continuum mechanics with crystal defects!
    """)
st.caption("🔬 Crystallographically Accurate • ISF/ESF/Twin distinct • Anisotropic • Multi-Variant •  2025")
