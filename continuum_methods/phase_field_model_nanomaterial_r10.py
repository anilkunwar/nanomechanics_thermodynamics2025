...
def create_3d_isosurface_plot(eta, sigma, np_mask, iso_level_eta, iso_level_stress,
    ...
    return fig
def create_vtu_file(eta, sigma, np_mask, N, dx, extent):
    """Create VTU file for ParaView"""
    # Flatten in Fortran order (column-major for VTK)
...
{flat(np.stack([X,Y,Z], axis=3).reshape(-1,3))}
</DataArray>
</Points>
</Piece>
</StructuredGrid>
