...
def compute_stress_fields(eta, eps0, theta):
    """
    Full 2D plane-strain isotropic elasticity with rotated eigenstrain
    Valid for any in-plane {111} orientation
    """
    # === Physical eigenstrain for one Shockley layer in {111} local frame ===
    gamma = eps0  # Use the user-input eps0 as the magnitude for shear
    delta = 0.02  # small normal dilatation (realistic for Ag)

    # Local eigenstrain in fault coordinate system (n = [cosθ, sinθ], s = [-sinθ, cosθ])
    ct, st = np.cos(theta), np.sin(theta)
    n = np.array([ct, st])      # normal to fault plane
    s = np.array([-st, ct])     # shear direction (Shockley partial glide)

    # Transformation strain tensor in local coordinates
    eps_local = delta * np.outer(n, n) + gamma * (np.outer(n, s) + np.outer(s, n)) / 2

    # Rotate to lab frame: ε_lab = R ε_local R^T
    R = np.array([[ct, -st],
                  [st, ct]])
    ...
    # === FFT-based plane-strain isotropic elasticity (Voigt average Ag) ===
    mu = 22.75e9  # Pa
    nu = 0.375
    lam = 2 * mu * nu / (1 - 2 * nu)
    ...
    K2 = KX**2 + KY**2
    K2[0, 0] = 1e-12  # avoid div by zero

    # Compute tau = C : eps*
    trace_star = eps_xx_star + eps_yy_star
    tau_xx = lam * trace_star + 2 * mu * eps_xx_star
    tau_yy = lam * trace_star + 2 * mu * eps_yy_star
    tau_xy = 2 * mu * eps_xy_star
    ...
    K_tau_K = KX * S_hat_x + KY * S_hat_y

    U_hat = np.zeros_like(K2, dtype=complex)
    mask = K2 > 0
    U_hat[mask] = -1j * K_tau_K[mask] / (K2[mask] * (lam + 2 * mu))

    u_hat_x = np.zeros_like(K2, dtype=complex)
    u_hat_y = np.zeros_like(K2, dtype=complex)
    u_hat_x[mask] = -1j * S_hat_x[mask] / (mu * K2[mask]) - (lam + mu) * KX[mask] * U_hat[mask] / (mu * K2[mask])
    u_hat_y[mask] = -1j * S_hat_y[mask] / (mu * K2[mask]) - (lam + mu) * KY[mask] * U_hat[mask] / (mu * K2[mask])
    ...
    # Elastic stress
    trace_el = exx + eyy - trace_star
    sxx = lam * trace_el + 2 * mu * (exx - eps_xx_star)
    syy = lam * trace_el + 2 * mu * (eyy - eps_yy_star)
    sxy = 2 * mu * (exy - eps_xy_star)

    # Stress fields
    sigma_mag = np.sqrt(sxx**2 + syy**2 + 2 * sxy**2) / 1e9  # GPa

    # Hydrostatic stress (2D approximation to match original)
    # sigma_hydro = (sxx + syy) / 3 / 1e9  # GPa
    sigma_hydro = (sxx + syy) / 2 / 1e9  # GPa

    # von Mises with plane strain correction
    szz = nu * (sxx + syy)
    von_mises = np.sqrt(0.5 * ((sxx - syy)**2 + (syy - szz)**2 + (szz - sxx)**2 + 6 * sxy**2)) / 1e9  # GPa
