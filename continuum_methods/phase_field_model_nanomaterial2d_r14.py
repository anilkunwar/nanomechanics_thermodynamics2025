...
Four fields exported • Custom shapes • Real eigenstrain values • **Crystallographic Orientation Control**
""")
...
# =============================================
# Sidebar – Enhanced with orientation selector
# =============================================
...
st.sidebar.header("🧭 Crystallographic Orientation")
orientation = st.sidebar.selectbox(
    "Habit Plane Orientation",
    ["Horizontal (111)", "Vertical (111)", "Diagonal 45° (1¯10)", "Diagonal 135° (110)", "Custom Angle"]
)

# Custom angle input
if orientation == "Custom Angle":
    custom_angle = st.sidebar.slider("Rotation Angle (degrees)", 0, 180, 45, 5)
else:
    angle_map = {
        "Horizontal (111)": 0,
        "Vertical (111)": 90,
        "Diagonal 45° (1¯10)": 45,
        "Diagonal 135° (110)": 135
    }
    custom_angle = angle_map[orientation]

st.sidebar.info(f"**Selected**: {orientation}\n**Rotation**: {custom_angle}°")
...
# =============================================
# NEW: Rotation functions for habit plane
# =============================================
def rotate_coordinates(X, Y, angle_degrees):
    """Rotate coordinate system by specified angle"""
    angle_rad = np.radians(angle_degrees)
    X_rot = X * np.cos(angle_rad) - Y * np.sin(angle_rad)
    Y_rot = X * np.sin(angle_rad) + Y * np.cos(angle_rad)
    return X_rot, Y_rot

def create_rotated_initial_eta(shape, angle_degrees):
    """Create initial defect with specified orientation"""
    # Rotate coordinates
    X_rot, Y_rot = rotate_coordinates(X, Y, angle_degrees)
    
    eta = np.zeros((N, N))
    cx, cy = 0, 0  # Center of rotated coordinates
    
    # Define dimensions in rotated coordinate system
    if shape in ["Rectangle", "Horizontal Fault"]:
        w, h = (24*dx, 12*dx)  # Convert to physical units
    else:
        w, h = (16*dx, 16*dx)
    
    if shape == "Square":
        mask = (np.abs(X_rot - cx) < w) & (np.abs(Y_rot - cy) < h)
        eta[mask] = init_amplitude
    elif shape == "Horizontal Fault":
        # Horizontal in rotated coordinates
        mask = (np.abs(Y_rot - cy) < 4*dx) & (np.abs(X_rot - cx) < w)
        eta[mask] = init_amplitude
    elif shape == "Vertical Fault":
        # Vertical in rotated coordinates  
        mask = (np.abs(X_rot - cx) < 4*dx) & (np.abs(Y_rot - cy) < w)
        eta[mask] = init_amplitude
    elif shape == "Rectangle":
        mask = (np.abs(X_rot - cx) < w) & (np.abs(Y_rot - cy) < h)
        eta[mask] = init_amplitude
    elif shape == "Ellipse":
        mask = ((X_rot/(w*1.5))**2 + (Y_rot/(h*1.5))**2) <= 1
        eta[mask] = init_amplitude
...
# =============================================
# Visualization Controls (unchanged)
# =============================================
...
# =============================================
# Initial Defect – Now with orientation
# =============================================
st.subheader("🎯 Initial Defect Configuration")
init_eta = create_rotated_initial_eta(shape, custom_angle)
...
ax0.set_title(f"Initial η – {defect_type} ({shape})\nε* = {eps0:.3f}, κ = {kappa:.2f}, Orientation: {custom_angle}°",
...
with st.sidebar.expander("About Plasticity & Model Physics"):
    st.markdown(f"""
    **Crystallographic Orientation**: {orientation} ({custom_angle}°)
    
    **Habit Planes in FCC Crystals**:
    - **{defect_type}** typically forms on **{{111}}** planes
    - **Horizontal/Vertical**: Different {defect_type} plane orientations
    - **Diagonal**: Common slip directions like **⟨110⟩**
...
if 'history' in st.session_state:
    ...
    # Auto-update suggested max values based on current frame data
    current_max_values = {
        'sigma_mag': float(np.ceil(sigma_mag.max())),
        'sigma_hydro_min': float(np.floor(sigma_hydro.min())),
        'sigma_hydro_max': float(np.ceil(sigma_hydro.max())),
        'von_mises': float(np.ceil(von_mises.max()))
    }
...
<ImageData WholeExtent="0 {N-1} 0 {N-1} 0 0" Origin="{extent[0]} {extent[2]} 0" Spacing="{dx} {dx} 1">
  <Piece Extent="0 {N-1} 0 {N-1} 0 0">
    ...
    st.download_button(
        ...
        buffer,
        f"Ag_NP_{defect_type}_{orientation.replace(' ', '_')}_{custom_angle}deg_Simulation.zip",
        "application/zip"
    )
...
with st.expander("🔍 Theoretical Soundness Analysis"):
    ...
    - **Selected Orientation**: {orientation} ({custom_angle}°)
    - **Habit Plane**: {{111}} family for {defect_type}
    - **Slip Directions**: ⟨110⟩ type for FCC crystals
    ...
    - **Crystallographic control**: Realistic habit plane orientations
    ...
    - **Crystallographic orientation**: Realistic habit plane control
   
    **Phase-field plasticity** bridges continuum mechanics with crystal defects!
    """)

st.caption("🔬 Crystallographically Accurate • ISF/ESF/Twin distinct • Orientation Control • 2025")
