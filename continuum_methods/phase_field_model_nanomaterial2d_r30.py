...
def create_3d_revolution_surface(simulations, frames, config, style_params):
    """Create 3D revolution surface visualization by revolving 2D profiles"""
    st.subheader("🗻 3D Revolution Surface Comparison")
    ...
    for idx, (sim, frame) in enumerate(zip(simulations, frames)):
        ...
        # Get 2D data and create radial profile
        eta, stress_fields = sim['history'][frame]
        stress_data = stress_fields[stress_key]
        ...
        r_vals = np.arange(0, r_max)
        profile = np.zeros_like(r_vals, dtype=float)
        for i, r in enumerate(r_vals):
            if r == 0:
                profile[i] = stress_data[center, center]
            else:
                theta = np.linspace(0, 2*np.pi, 100)
                x_samp = center + r * np.cos(theta)
                y_samp = center + r * np.sin(theta)
                # Interpolate
                f = interpolate.RectBivariateSpline(np.arange(N), np.arange(N), stress_data)
                profile[i] = np.mean(f.ev(y_samp, x_samp)) # Note: y,x for row,col
       
        # Revolve profile to 3D surface
        phi = np.linspace(0, 2*np.pi, 50) # Azimuthal
        R, Phi = np.meshgrid(r_vals, phi)
        X = R * np.cos(Phi)
        Y = R * np.sin(Phi)
        Z = np.tile(profile, (len(phi), 1)) # Revolve along z? Wait, for sphere, adjust.
        # For spherical NP approximation, use Z = np.sqrt(r_max**2 - R**2) or similar; here simple revolution.
       
        # Plot surface
        cmap_name = sim['params']['sigma_cmap']
        cmap = plt.cm.get_cmap(COLORMAPS.get(cmap_name, 'viridis'))
        surf = ax.plot_surface(X, Y, Z.T, cmap=cmap, linewidth=0, antialiased=True)
       
        ax.set_title(f"{sim['params']['defect_type']} - {sim['params']['orientation']}")
        ax.set_xlabel("X (nm)")
        ax.set_ylabel("Y (nm)")
        ax.set_zlabel(f"{config['stress_component']} (GPa)")
       
        fig.colorbar(surf, ax=ax, shrink=0.5, aspect=5)
   
    # Apply styling (note: 3D axes may need manual tweaks)
    fig = EnhancedFigureStyler.apply_advanced_styling(fig, fig.axes, style_params)
...
def create_enhanced_comparison_plot(simulations, frames, config, style_params):
    ...
    if config['type'] == "Side-by-Side Heatmaps":
        ...
        return create_3d_revolution_surface(simulations, frames, config, style_params)
    else:
