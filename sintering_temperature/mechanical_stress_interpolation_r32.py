...
class EnhancedSolutionLoader:
    ...
    def _standardize_data(self, data, file_path):
        ...
        standardized = {
            ...
            'metadata': {
                'filename': os.path.basename(file_path),
                'size': os.path.getsize(file_path),  # Include size in standardized metadata
...
class DefectRadarVisualizer:
    ...
    def create_sunburst_defect_chart(self, defect_comparison, stress_component='sigma_hydro',
        ...
        """Create a sunburst-style chart for defect stress patterns with customizable line properties"""
        fig = go.Figure()
    ...
    def create_multi_component_radar(self, defect_comparison, defect_type='Twin',
                                   ...
                                   line_width=3, line_style='solid', habit_plane_style='dashdot'):
        """Create radar chart showing multiple stress components for a single defect type with line customization"""
        fig = go.Figure()
        ...
        for comp_name in ['sigma_hydro', 'von_mises', 'sigma_mag']:
            if comp_name in target_data['stresses']:
                ...
                fill_color = self._apply_opacity_to_color(color, fill_opacity)
                # Custom line styles for different components
                component_styles = {
                    'sigma_hydro': 'solid',
                    'von_mises': 'dash',
                    'sigma_mag': 'dot'
                ...
                fig.add_trace(go.Scatterpolar(
                    ...
                    line=dict(
                        color=color,
                        width=line_width,
                        dash=component_styles.get(comp_name, 'solid')  # Different style per component
                    ),
    ...
    def create_normalized_stress_radar(self, defect_comparison,
        ...
        for defect_key, data in defect_comparison.items():
            ...
            # Add trace with customizable line style
            fig.add_trace(go.Scatterpolar(
                r=stresses_closed,
                theta=angles_closed,
                fill='toself',
    ...
    def create_advanced_sunburst_chart(self, defect_comparison, stress_component='sigma_hydro',
                                      ...
                                      radial_range_factor=1.2, line_width=3, line_style='solid'):
        """
    ...
    def create_interactive_3d_defect_sunburst(self, defect_comparison,
        ...
        for defect_key, data in defect_comparison.items():
            ...
            x_grid = np.linspace(-global_max_stress, global_max_stress, 50)
            y_grid = np.linspace(-global_max_stress, global_max_stress, 50)
            X, Y = np.meshgrid(x_grid, y_grid)
            # Calculate Z values for the surface - distance from origin
            Z_surface = np.sqrt(X**2 + Y**2) * 0
            # Add surface for visual context
            surface_color = color.replace('rgb', 'rgba').replace(')', f', {surface_opacity})')
            fig.add_trace(go.Surface(
                x=X, y=Y, z=Z_surface + z_offset,
                opacity=surface_opacity,
                colorscale=[[0, surface_color], [1, surface_color]],
                showscale=False,
                name=f'{defect_type} Surface',
                hoverinfo='skip'
            ))
            # Add stress line with smooth interpolation
            ...
            fig.add_trace(go.Scatter3d(
                ...
                marker=dict(
                    ...
                    colorscale='Viridis',
                    opacity=0.9,
                    symbol='circle'  # Use valid symbol for 3D
                ),
            ...
            # Add connecting lines to origin for visual reference
            fig.add_trace(go.Scatter3d(
                x=[0, x[0]],
                y=[0, y[0]],
                z=[z_offset, z_offset],
        ...
        # Add habit plane marker with valid symbol
        fig.add_trace(go.Scatter3d(
            x=[0],
            y=[0],
            z=[max_z/2],
            ...
            marker=dict(
                size=15,
                color='gold',
                symbol='diamond',  # Use valid symbol for 3D
                line=dict(width=2, color='#D4AF37')
            ),
            text=['HABIT PLANE'],
            textposition='top center',
            textfont=dict(size=12, color='black'),
            name='Habit Plane Marker',
            hovertemplate=f'Habit Plane ({self.habit_angle}°)<br>Z-Position: {max_z/2:.1f}<extra></extra>',
            showlegend=True
        ))
    ...
    def create_defect_comparison_dashboard(self, defect_comparison,
        ...
        fig.add_trace(
            go.Scatterpolar(
                ...
                theta=[self.habit_angle],
                mode='markers+text',
                marker=dict(size=15, color='gold', symbol='diamond'),  # Use valid symbol
                text=['HABIT'],
                textposition='top center',
    ...
    with st.sidebar:
        ...
        if st.button("📂 Load Solutions", use_container_width=True):
            ...
            if st.session_state.solutions:
                ...
                # Display loaded files with proper metadata handling
                with st.expander(f"📁 Loaded Solutions ({len(st.session_state.solutions)})", expanded=False):
                    for sol in st.session_state.solutions:
                        ...
                        filename = sol['metadata'].get('filename', 'Unknown file')
                        size = sol['metadata'].get('size', 0)
                        size_kb = size // 1024 if size else 0
                        st.write(f"• {filename} ({size_kb} KB)")
            else:
        ...
        with tab2:
            ...
            if 'defect_comparison' in st.session_state:
                ...
                with col2:
                    ...
                    st.markdown("##### **Advanced Line Options**")
                    line_smoothing = st.slider("Line Smoothing", 0.0, 1.0, 0.5, 0.05,
                ...
                with col3d1:
                    background_color = st.color_picker("3D Background Color", "#f8f9fa")
                    surface_opacity = st.slider("Surface Opacity", 0.0, 1.0, 0.8, 0.05)
                with col3d2:
                    st.markdown("##### **Camera Position**")
                    camera_x = st.slider("Camera X Position", -5.0, 5.0, 1.5, 0.1)
                    camera_y = st.slider("Camera Y Position", -5.0, 5.0, 1.5, 0.1)
                    camera_z = st.slider("Camera Z Position", -5.0, 5.0, 0.8, 0.1)
                    camera_position = {
        ...
        with tab4:
            ...
            if 'defect_comparison' in st.session_state:
                ...
                if analysis_type == "Stress Gradient Analysis":
                    ...
                    ```
                    For tensile stress (σ_hydro > 0): D/D₀ = exp(Ωσ_hydro / kT)
                    For compressive stress (σ_hydro ≤ 0): D/D₀ = 1 (no enhancement)
                    
                    Where:
                    - Ω = atomic volume (1.56e-29 m³ for Ag)
                    - σ_hydro = hydrostatic stress
                    - k = Boltzmann constant (1.38e-23 J/K)
                    - T = temperature (650 K typical sintering temp for Ag)
                    ```
                    """)
                    # Calculate diffusion enhancement with physics-correct formula
                    ...
                    for defect_key, data in defect_comparison.items():
                        defect_type = data.get('defect_type', 'Unknown')
                        angles = np.array(data['angles'])
                        stresses_gpa = np.array(data['stresses']['sigma_hydro'])  # in GPa
                        stresses_pa = stresses_gpa * 1e9  # convert to Pa
                        
                        # Physics-correct enhancement: only for tensile stress (σ_hydro > 0)
                        diffusion_ratio = np.ones_like(stresses_pa)  # default = 1 (no enhancement)
                        tensile_mask = stresses_pa > 0
                        diffusion_ratio[tensile_mask] = np.exp(
                            Omega * stresses_pa[tensile_mask] / (k_B * T)
                        )
                        ...
                        fig_diff.add_trace(go.Scatterpolar(
                            ...
                            hovertemplate='Defect: ' + defect_type +
                                         ...
                                         'Hydrostatic Stress: %{customdata:.4f} GPa<br>' +
                                         'Diffusion Enhancement: %{r:.2f}x<extra></extra>',
                            customdata=np.append(stresses_gpa, stresses_gpa[0])
                        ))
                    # Add habit plane reference
                    max_enhancement = max(
                        max(trace.r if hasattr(trace, 'r') else [0]) 
                        for trace in fig_diff.data
                    )
                    fig_diff.add_trace(go.Scatterpolar(
                        r=[0, max_enhancement * 1.1],
                        theta=[54.7, 54.7],
                        mode='lines',
                    ...
                    fig_diff.update_layout(
                        title="Diffusion Coefficient Enhancement Factor (Tensile Stress Only)",
                        polar=dict(
                            radialaxis=dict(
                                title="D / D₀ Enhancement Ratio",
                                type="log",  # log scale due to exponential nature
                                range=[0, np.log10(max_enhancement * 1.2)] if max_enhancement > 1 else [0, 1]
                            ),
                            angularaxis=dict(rotation=90, direction="clockwise")
                        ),
                        width=800,
                    ...
                    compared to bulk material when under tensile stress. Compressive stress provides no enhancement. This dramatic
                    enhancement enables low-temperature sintering of nanocrystalline silver.
                    </div>
                    """, unsafe_allow_html=True)
                st.markdown("""
                ---
