...
class DefectRadarVisualizer:
    ...
    def __init__(self, habit_angle=54.7):
        ...
        self.colormaps = {
            ...
            # Custom colormaps for Plotly (list format)
            'deep_thermal': [
    ...
    def rgb_str_to_tuple(self, rgb_str):
        """Convert RGB string 'rgb(r,g,b)' to normalized tuple (r,g,b) in [0,1]"""
        try:
            # Extract numbers from string like 'rgb(255, 0, 0)'
            ...
            print(f"Error converting color {rgb_str}: {e}")
    ...
    def create_enhanced_defect_radar(self, defect_data, title="Defect Stress Radar",
            ...
            for comp in max_values:
                if max_values[comp]:
                    try:
                        ...
                        pass
            if radial_max > 0:
                radial_range = [max(0, radial_min * 0.9), radial_max * 1.1]
            else:
                radial_range = [0, 1]
        ...
        for defect_name, data in defect_data.items():
            ...
            # Filter to vicinity
            mask = (angles_array >= center_angle - vicinity_range) & \
                   (angles_array <= center_angle + vicinity_range)
            
            if not np.any(mask):
                continue
                
            vic_angles = angles_array[mask]
            ...
            for comp_idx, (comp_name, comp_values) in enumerate(stresses.items()):
                ...
                try:
                    comp_array = np.array(comp_values, dtype=float)
                except:
                    continue
                    
                vic_stresses = comp_array[mask]
                
                if len(vic_stresses) == 0:
                    continue
                
                # Normalize if requested
                if normalization == 'max' and np.max(vic_stresses) > 0:
                    display_values = vic_stresses / np.max(vic_stresses)
                elif normalization == 'minmax' and (np.max(vic_stresses) - np.min(vic_stresses)) > 0:
                    display_values = (vic_stresses - np.min(vic_stresses)) / \
                                   (np.max(vic_stresses) - np.min(vic_stresses))
        ...
        for i, comp in enumerate(stress_components):
            for j, defect in enumerate(defect_types):
                ...
                if defect in defect_data and comp in defect_data[defect]['stresses']:
                    ...
                    if 'angles' not in data or 'stresses' not in data:
                        continue
                        
                    angles = data['angles']
                    stresses = data['stresses'][comp]
                    
                    # Convert to arrays
                    try:
                        angles_array = np.array(angles, dtype=float)
                        stresses_array = np.array(stresses, dtype=float)
                    except:
                        continue
                    ...
                    # Filter to vicinity
                    mask = (angles_array >= self.habit_angle - vicinity_range) & \
                           (angles_array <= self.habit_angle + vicinity_range)
                    
                    if np.any(mask):
                        vic_angles = angles_array[mask]
                        vic_stresses = stresses_array[mask]
                        ...
                        fig.add_trace(
                            go.Scatterpolar(
                                ...
                                marker=dict(
                                    size=4,
                                    color=vic_stresses,
                                    colorscale=self.colormaps[colormap] if colormap in self.colormaps else 'viridis',
                                    showscale=False
                                ),
                                fill='toself',
    ...
    def create_sunburst_defect_chart(self, defect_data, max_layers=3,
        ...
        for defect_idx, (defect_name, data) in enumerate(defect_data.items()):
            ...
            if 'stresses' in data and 'sigma_hydro' in data['stresses']:
                try:
                    avg_stress = np.mean(data['stresses']['sigma_hydro']) if len(data['stresses']['sigma_hydro']) > 0 else 0
                except:
                    avg_stress = 0
            else:
                avg_stress = 0
            ...
            if max_layers > 1:
                for comp_idx, (comp_name, comp_data) in enumerate(data.get('stresses', {}).items()):
                    ...
                    if comp_data:
                        try:
                            comp_value = np.mean(comp_data) if len(comp_data) > 0 else 0
                        except:
                            comp_value = 0
                    else:
                        comp_value = 0
    ...
    def get_colormap_preview(self, selected_colormaps=None):
        """
        Generate colormap preview visualization - FIXED VERSION
        """
        ...
        if len(selected_colormaps) == 1:
            axes = [axes]
        elif len(selected_colormaps) == 0:
            return plt.figure()  # Return empty figure
        
        for ax, cmap_name in zip(axes, selected_colormaps):
            if cmap_name in self.colormaps:
                cmap_value = self.colormaps[cmap_name]
                
                if isinstance(cmap_value, list):
                    # Custom colormap defined as list for Plotly
                    # For matplotlib preview, create a simple gradient
                    # Extract colors from the list
                    colors = []
                    for item in cmap_value:
                        if isinstance(item, list) and len(item) == 2:
                            _, color_str = item
                            # Convert RGB string to matplotlib color
                            try:
                                rgb_tuple = self.rgb_str_to_tuple(color_str)
    ...
    with tab1:
        ...
        if defect_data:
            ...
            font_settings = {
                ...
                'grid_color': grid_color + "4D",  # Add transparency
                'bg_color': bg_color + "1A"  # Add transparency
            }
                    ...
                    if 'stresses' in data and 'sigma_hydro' in data['stresses']:
                        ...
                        if stresses:
                            try:
                                ...
                                st.metric(f"{defect_name} σ_h", "N/A", "Error")
        ...
        with col_cust2:
            ...
            if preview_colormaps:
                try:
                    ...
                    st.error(f"Error generating colormap preview: {e}")
                    # Fallback: show a simple message
                    st.info("Colormap preview requires matplotlib. Using default preview.")
                    # Create a simple text-based preview
                    for cmap in preview_colormaps:
                        if cmap in st.session_state.visualizer.colormaps:
                            st.write(f"**{cmap}**: Available")
            ...
            if defect_data:
                ...
                with col_btn1:
                    if st.button("🖼️ Export Image", use_container_width=True):
                        st.info(f"To export as {export_format}, use Plotly's write_image method in a production environment.")
                        st.code(f"""
                        # Example code for export:
                        import plotly.io as pio
                        pio.write_image(fig_export, 'defect_radar.{export_format.lower()}')
                        """)
