...
class NumericalSolutionsScanner:
    ...
    def __init__(self, base_dir=None):
        """
        Args:
            base_dir: Base directory containing numerical solutions (optional; uses env var if not provided)
        """
        # Use environment variable if set, else fallback to default or provided base_dir
...
class SpatialLocalityAttentionInterpolator:
    ...
    def __init__(self, input_dim=15, num_heads=4, d_model=32, output_dim=3, 
                 sigma_spatial=0.2, sigma_param=0.2, use_gaussian=True, base_dir=None):
        """
        Args:
            ...
            base_dir: Optional base directory for the scanner (passed to NumericalSolutionsScanner)
        """
        ...
        # Initialize directory scanner with optional base_dir
        self.directory_scanner = NumericalSolutionsScanner(base_dir=base_dir)
...
def create_attention_interface():
    ...
    if 'interpolator' not in st.session_state:
        # Try to get directory from environment variable
        env_dir = os.getenv('SIM_DIR')
        if env_dir:
            st.info(f"📁 Using SIM_DIR environment variable: {env_dir}")
    ...
    with st.sidebar.expander("⚙️ Model Parameters", expanded=False):
        ...
        if not dir_exists:
            ...
            with st.expander("💡 How to set up"):
                st.markdown("""
                **Option 1: Create directory manually**
                ```bash
                # In your terminal
                mkdir numerical_solutions
                ```
                
                **Option 2: Set environment variable**
                ```bash
                # Linux/Mac
                export SIM_DIR="/path/to/your/simulations"
                
                # Windows (Command Prompt)
                set SIM_DIR="C:\\path\\to\\your\\simulations"
                
                # Windows (PowerShell)
                $env:SIM_DIR = "C:\\path\\to\\your\\simulations"
                ```
                
                **Option 3: Use Streamlit secrets**
                Create `.streamlit/secrets.toml`:
                ```toml
                SIM_DIR = "/path/to/your/simulations"
                ```
                """)
        
        # Manual directory override
        manual_dir = st.text_input(
            "Override directory path (optional):",
            placeholder="/path/to/simulation/files",
            help="Leave empty to use environment variable or default 'numerical_solutions'"
        )
        
        if st.button("🔄 Update Directory Path"):
            if manual_dir:
                new_dir = Path(manual_dir)
                if new_dir.exists():
                    st.session_state.interpolator = SpatialLocalityAttentionInterpolator(
                        num_heads=num_heads,
                    ...
                    st.success(f"✅ Updated directory to: {new_dir.resolve()}")
                    # Clear cache
                    st.session_state.interpolator.directory_scanner.clear_cache()
                    st.rerun()
                else:
                    st.error(f"❌ Directory not found: {new_dir}")
            else:
                ...
                st.session_state.interpolator = SpatialLocalityAttentionInterpolator(
                    ...
                    sigma_param=sigma_param,
                    use_gaussian=use_gaussian,
                    base_dir=None  # Will use env var or default
                )
                st.success("✅ Using environment variable or default directory")
                st.session_state.interpolator.directory_scanner.clear_cache()
                st.rerun()
    ...
    with tab1:
        ...
        # Show directory info at the top
        scanner = st.session_state.interpolator.directory_scanner
        current_dir = str(scanner.base_dir.resolve())
        dir_exists = scanner.base_dir.exists()
        ...
        if not dir_exists:
            ...
            st.info("""
            **To fix this:**
            1. Create a `numerical_solutions` folder in the same directory as this script, OR
            2. Set the `SIM_DIR` environment variable to point to your simulation files, OR
            3. Use the directory override option in the sidebar
            """)
            
            # Create directory button
            if st.button("📁 Create numerical_solutions directory"):
                try:
                    scanner.base_dir.mkdir(parents=True, exist_ok=True)
                    st.success(f"✅ Created directory: {scanner.base_dir}")
                    st.info("Now add your simulation files to this directory and refresh.")
                    st.rerun()
                except Exception as e:
                    st.error(f"Failed to create directory: {e}")
            
            # Continue with upload option anyway
            st.markdown("---")
            st.info("You can still upload files manually below:")
        ...
        if dir_exists:
            ...
            data_source = "📤 Upload files manually"
                ...
                if total_files == 0:
                    ...
                    **Example file structure:**
                    ```python
                    {
                        ...
                        'history': [
                            (eta_array, {'sigma_hydro': ..., 'sigma_mag': ..., 'von_mises': ...})
                        ],
                        'metadata': {
                            'run_time': 120.5,
                            'frames': 100,
            ...
            if not structure:
                ...
                def display_tree(node, path="", depth=0):
                    ...
                    if node["type"] == "directory":
                        st.markdown(f"{indent}📁 **{Path(path).name if path else scanner.base_dir.name}**")
...
with st.expander("🔬 Theoretical Analysis: Spatial-Attention Interpolation", expanded=False):
    ...
    1. **Data Collection**:
       ...
       - Store in directory (default: `numerical_solutions`) or set `SIM_DIR` environment variable
