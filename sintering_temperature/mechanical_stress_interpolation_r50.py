...
def main():
    ...
    if st.session_state.interpolation_result:
        ...
        stress_viz_tab, = st.tabs(["🎨 Stress Visualization"])
        ...
        with stress_viz_tab:
            ...
            if st.button("Show All Stress Components Comparison", key="show_all_stress_components"):
                fig_all, axes = plt.subplots(1, 3, figsize=(18, 6), dpi=300)
                ...
                for idx, (comp, title) in enumerate(zip(components, titles)):
                    if comp in result['fields']:
                        ax = axes[idx]
                        im = ax.imshow(result['fields'][comp], cmap=cmap_name, aspect='equal')
