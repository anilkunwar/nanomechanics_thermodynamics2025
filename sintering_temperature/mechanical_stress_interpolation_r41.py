...
def main():
    ...
    with st.sidebar:
        ...
        # Target angle input - NUMBER INPUT INSTEAD OF SLIDER
    ...
    if st.session_state.interpolation_result:
        ...
        with tab5:
            ...
            **Publication Quality Features:**
            ...
            if pub_component in result['fields']:
                ...
                st.markdown("#### 📈 Publication Quality Heatmap")
                ...
                with col_save2:
                    filename_base = f"stress_{pub_component}_theta_{result['target_angle']:.1f}_{result['target_params']['defect_type']}"
                ...
                if st.button("Create Multi-Panel Figure", use_container_width=True):
                    ...
                    components = ['von_mises', 'sigma_hydro', 'sigma_mag', 'sigma_hydro']  # Last one for difference
                    ...
                    for idx, comp in enumerate(components):
                        ...
                        if idx < 3:
                            ...
                            im = ax.imshow(field, cmap=pub_cmap, aspect='equal', interpolation='bilinear', origin='lower')
                            plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
                            ax.set_title(f"{comp.replace('_', ' ').title()}", fontsize=16, fontweight='bold')
                        else:
                            # Difference plot (if ground truth selected)
                            if st.session_state.selected_ground_truth is not None:
                                gt_field = result['source_fields'][st.session_state.selected_ground_truth][pub_component]
                                diff_field = result['fields'][pub_component] - gt_field
                                max_diff = np.max(np.abs(diff_field))
                                im = ax.imshow(diff_field, cmap='RdBu_r', vmin=-max_diff, vmax=max_diff,
                                              aspect='equal', interpolation='bilinear', origin='lower')
                                plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04, label='Difference (GPa)')
                                ax.set_title(f"Difference from Ground Truth", fontsize=16, fontweight='bold')
                            else:
                                ax.text(0.5, 0.5, "Select ground truth\nfor difference plot",
                                       ha='center', va='center', fontsize=14, fontweight='bold')
                                ax.set_axis_off()
                    ...
                    plt.suptitle(f"Theory-Informed Interpolation: θ = {result['target_angle']:.1f}°, {result['target_params']['defect_type']}",
        ...
        with tab6:
            ...
            if export_format == "JSON (Full Results)":
                ...
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    fig_path = os.path.join(tmpdir, f"publication_figure.png")
        ...
        3. **Set Angular Bracketing Parameters** (kernel sigma, attention balance)
