...
def create_attention_interface():
    ...
    with tab6:
        ...
        if not has_prediction:
            ...
            # ALWAYS VISIBLE DIRECT DOWNLOAD BUTTONS (like CSV download)
            st.subheader("⬇️ Direct Download Options")
          
            dl_col1, dl_col2, dl_col3 = st.columns(3)
          
            # PKL download - ALWAYS VISIBLE
            with dl_col1:
                st.markdown("**PKL Format**")
                
                # Function to prepare PKL data
                def prepare_pkl_data():
                    try:
                        ...
                        pkl_buffer.seek(0)
                        return pkl_buffer.getvalue()
                    except Exception as e:
                        st.error(f"❌ Error preparing PKL: {str(e)}")
                        return None
                
                # Create PKL data
                pkl_data = prepare_pkl_data()
                
                # Download button (always visible if data is available)
                if pkl_data:
                    st.download_button(
                        label="📥 Download PKL",
                        data=pkl_data,
                        ...
                        mime="application/octet-stream",
                        key="download_pkl_direct",
                        use_container_width=True,
                        type="secondary"
                    )
                    st.success("✅ PKL file ready for download!")
                else:
                    st.error("❌ Could not prepare PKL file")
              
            # PT download - ALWAYS VISIBLE
            with dl_col2:
                st.markdown("**PT Format**")
                
                # Function to prepare PT data
                def prepare_pt_data():
                    try:
                        save_data = st.session_state.prediction_results_manager.prepare_prediction_data_for_saving(
                            st.session_state.prediction_results,
                        ...
                        torch.save(save_data, pt_buffer)
                        pt_buffer.seek(0)
                        return pt_buffer.getvalue()
                    except Exception as e:
                        st.error(f"❌ Error preparing PT: {str(e)}")
                        return None
                
                # Create PT data
                pt_data = prepare_pt_data()
                ...
                if pt_data:
                    st.download_button(
                        ...
                        mime="application/octet-stream",
                        key="download_pt_direct",
                        use_container_width=True,
                        type="secondary"
                    )
                    st.success("✅ PT file ready for download!")
                else:
                    st.error("❌ Could not prepare PT file")
              
            # ZIP download - ALWAYS VISIBLE
            with dl_col3:
                st.markdown("**ZIP Archive**")
                
                # Function to prepare ZIP data
                def prepare_zip_data():
                    try:
                ...
                if zip_data:
                    ...
                    st.error("❌ Could not prepare ZIP file")
