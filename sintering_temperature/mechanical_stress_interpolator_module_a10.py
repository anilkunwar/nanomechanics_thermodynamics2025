...
def create_attention_interface():
    ...
    # Main interface tabs for steps 1-7
    ...
    with tab2:
        if len(st.session_state.source_simulations) < 2:
            st.warning("⚠️ Please load at least 2 source simulations in Tab 1 first")
        else:
    ...
    # Tab 3: Perform Machine Learning Interpolation
    with tab3:
        if len(st.session_state.source_simulations) < 2:
            st.warning("⚠️ Please load at least 2 source simulations in Tab 1 first")
        elif 'target_params' not in st.session_state:
            st.warning("⚠️ Please configure target parameters in Tab 2 first")
        else:
            col1, col2 = st.columns(2)
            ...
            if st.button("🚀 Train & Predict", type="primary"):
                with st.spinner("Training attention model and predicting..."):
                    try:
                        ...
                        st.session_state.source_params = source_params
                        st.session_state.source_stress = source_stress
            ...
            if 'prediction_results' in st.session_state:
                ...
                model_filename = st.text_input("Model Filename", "trained_model")
    ...
    with tab4:
        if 'prediction_results' not in st.session_state:
            st.warning("⚠️ Please perform ML interpolation in Tab 3 first")
        else:
            ...
            if st.button("💾 Save Results"):
                save_format = st.session_state.save_format.lower().split()[0]
                ...
                saved = st.session_state.solutions_manager.save_simulation(
                    save_data, base_filename, format_type=save_format
                )
                if saved:
    ...
    with tab5:
        if 'prediction_results' not in st.session_state:
            ...
            viz_tab, new_pred_tab = st.tabs(["Visualization", "New Prediction"])
            ...
            with new_pred_tab:
                st.subheader("New Single Target Prediction")
                if 'source_params' not in st.session_state or 'source_stress' not in st.session_state:
                    st.warning("Source data not available for prediction.")
                else:
                    col1, col2 = st.columns(2)
                 
                    with col1:
                        new_defect = st.selectbox(
                            "New Defect Type",
                            ["ISF", "ESF", "Twin"],
                            index=0
                        )
                    ...
                    with col2:
                        ...
                        new_orientation_mode = st.radio(
                            "New Orientation Mode",
                            ["Predefined", "Custom Angle"],
                            horizontal=True
                        )
                     
                        if new_orientation_mode == "Predefined":
                            ...
                            new_theta = np.deg2rad(angle_map.get(new_orientation, 0))
                            ...
                            new_theta = np.deg2rad(new_angle)
                         
                            new_orientation = st.session_state.interpolator.get_orientation_from_angle(new_angle)
                     
                    if st.button("Predict for New Target"):
                        new_params = {
                            'defect_type': new_defect,
                            'shape': new_shape,
                            'eps0': new_eps0,
                            'kappa': new_kappa,
                        ...
                        new_vector, _ = st.session_state.interpolator.compute_parameter_vector(
                            {'params': new_params}
                        )
                        new_param = torch.from_numpy(new_vector).float().unsqueeze(0)
                     
                        new_stress, new_weights = st.session_state.interpolator.predict(
                            new_param, st.session_state.source_params, st.session_state.source_stress
                        )
                     
                        st.session_state.new_prediction = {
                            'stress_fields': {
                                'sigma_hydro': new_stress[0],
                                'sigma_mag': new_stress[1],
                                'von_mises': new_stress[2]
                            },
                            'attention_weights': new_weights,
                            'target_params': new_params
                        }
                     
                        st.success("New prediction complete!")
                     
                    if 'new_prediction' in st.session_state:
                        new_results = st.session_state.new_prediction
                        new_stress_fields = new_results.get('stress_fields', {})
                        new_component = st.selectbox(
                            "Select Component for New Prediction",
                            ['von_mises', 'sigma_hydro', 'sigma_mag']
                        )
                     
                        if new_component in new_stress_fields:
                            new_data = new_stress_fields[new_component]
                            fig_new = px.imshow(
                                new_data,
    ...
    # Tab 6: Download Options
    with tab6:
        if 'prediction_results' not in st.session_state:
            st.warning("⚠️ Please perform ML interpolation in Tab 3 first")
        else:
            # Conditional downloads based on save_format
            ...
            if st.session_state.save_format == "PKL only":
                with dl_col1:
            ...
            elif st.session_state.save_format == "PT only":
                with dl_col1:
            ...
            elif st.session_state.save_format == "Both PKL & PT":
                with dl_col1:
                    st.markdown("**Both PKL & PT (as ZIP)**")
                    both_zip_buffer = BytesIO()
                    with zipfile.ZipFile(both_zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
                        # Add PKL
                        pkl_buffer = BytesIO()
                        pickle.dump(save_data, pkl_buffer, protocol=pickle.HIGHEST_PROTOCOL)
                        pkl_buffer.seek(0)
                        ...
                        # Add README
                        readme_content = f"""# PKL and PT Prediction Results
Generated: {datetime.now().isoformat()}
Base filename: {base_filename}
"""
                        zip_file.writestr(f"README_{base_filename}.txt", readme_content)
                    both_zip_buffer.seek(0)
                    st.download_button(
                        label="📦 Download Both (ZIP)",
                        data=both_zip_buffer.getvalue(),
                        file_name=f"{base_filename}_both.zip",
            ...
            elif st.session_state.save_format == "Archive (ZIP)":
                with dl_col1:
                    st.markdown("**Full Archive (ZIP)**")
                    ...
                    st.download_button(
                        ...
                        file_name=f"{base_filename}_archive.zip",
                        mime="application/zip",
                        key="download_zip",
                        use_container_width=True,
                        type="primary"
            ...
            st.success(f"✅ {st.session_state.save_format} ready for download!")
 
    # Tab 7: Time Frame Analysis
    with tab7:
        if len(st.session_state.source_simulations) < 1:
            st.warning("⚠️ Please load source simulations in Tab 1 first")
        else:
