...
# =============================================
# PHASE-FIELD SIMULATION WITH 12.8 nm DOMAIN
# =============================================
class PhaseFieldSimulation:
    """Phase-field simulation with 12.8 nm x 12.8 nm domain centered at 0"""
    
    def __init__(self, N=128, dx=0.1):
        """
        ...
        self.N = N
        self.dx = dx
        self.L = N * dx  # Total domain length = 12.8 nm
        self.extent = [-self.L/2, self.L/2, -self.L/2, self.L/2]
        
        # Create coordinate grids
        self.x = np.linspace(-self.L/2, self.L/2, N, endpoint=False)
        self.y = np.linspace(-self.L/2, self.L/2, N, endpoint=False)
        self.X, self.Y = np.meshgrid(self.x, self.y)
        
        # Elastic constants (for Silver, in GPa)
        self.C11 = 124.0  # GPa
        self.C12 = 93.4   # GPa
        self.C44 = 46.1   # GPa
        
        # Initialize fields
        self.phi = np.zeros((N, N))  # Order parameter
        self.stress_fields = {
            'sigma_xx': np.zeros((N, N)),
            'sigma_yy': np.zeros((N, N)),
            'sigma_zz': np.zeros((N, N)),
            'tau_xy': np.zeros((N, N))
        }
        
    def create_defect_shape(self, shape_type='Square', theta=0.0, defect_size=2.0):
        """
        Create different defect shapes in the domain
        
        Parameters:
        -----------
        shape_type : str
            Type of defect shape ('Square', 'Horizontal Fault', 'Vertical Fault', 'Rectangle')
        theta : float
            Rotation angle in radians
        defect_size : float
            Size of defect in nm
        """
        # Reset phi field
        ...
        Y_rot = -self.X * np.sin(theta) + self.Y * np.cos(theta)
        
        if shape_type == 'Square':
            # Square defect centered at origin
            mask = (np.abs(X_rot) < defect_size/2) & (np.abs(Y_rot) < defect_size/2)
            self.phi[mask] = 1.0
            
        elif shape_type == 'Horizontal Fault':
            # Horizontal line defect
            mask = (np.abs(Y_rot) < defect_size/4) & (np.abs(X_rot) < self.L/3)
            self.phi[mask] = 1.0
            
        elif shape_type == 'Vertical Fault':
            # Vertical line defect
            mask = (np.abs(X_rot) < defect_size/4) & (np.abs(Y_rot) < self.L/3)
            self.phi[mask] = 1.0
            
        elif shape_type == 'Rectangle':
            # Rectangular defect
            mask = (np.abs(X_rot) < defect_size) & (np.abs(Y_rot) < defect_size/2)
            self.phi[mask] = 1.0
    ...
    def compute_stress_fields(self, eps0=2.12, kappa=0.6, defect_type='Twin'):
        """
        Compute stress fields using eigenstrain approach
        ...
        defect_type : str
            Type of defect ('Twin', 'ISF', 'ESF', 'No Defect')
        """
        # Get eigenstrain from physics parameters
        eps0 = PhysicsParameters.get_eigenstrain(defect_type)
        
        # Define eigenstrain tensor based on defect type
        if defect_type == 'Twin':
            # Shear eigenstrain for twinning
            epsilon_star = np.array([
                [0, eps0, 0],
                [eps0, 0, 0],
                [0, 0, 0]
            ])
        elif defect_type in ['ISF', 'ESF']:
            # Smaller shear for stacking faults
            epsilon_star = np.array([
                [0, eps0/3, 0],
                [eps0/3, 0, 0],
                [0, 0, 0]
            ...
            epsilon_star = np.zeros((3, 3))
        
        # Compute stress using Fourier transform method
        ...
        K2 = KX**2 + KY**2
        K2[0, 0] = 1.0  # Set DC component to avoid division by zero
        
        # Compute Green's function in Fourier space
        # Simplified anisotropic elasticity
        C1111 = self.C11
        C1122 = self.C12
        C1212 = self.C44
        
        # For 2D plane strain
        G_inv = (C1111 * KX**4 + 2 * (C1122 + 2*C1212) * KX**2 * KY**2 + C1111 * KY**4)
        G_inv[0, 0] = 1.0  # Avoid division by zero
        
        # Compute stress in Fourier space
        sigma_xx_hat = -1j * KX * phi_hat * epsilon_star[0, 0] / G_inv
        sigma_yy_hat = -1j * KY * phi_hat * epsilon_star[1, 1] / G_inv
        tau_xy_hat = -0.5j * (KX + KY) * phi_hat * epsilon_star[0, 1] / G_inv
        
        # Inverse transform to real space
        sigma_xx = np.real(np.fft.ifft2(sigma_xx_hat))
        sigma_yy = np.real(np.fft.ifft2(sigma_yy_hat))
        tau_xy = np.real(np.fft.ifft2(tau_xy_hat))
        
        # Plane strain assumption: sigma_zz = nu*(sigma_xx + sigma_yy)
        nu = self.C12 / (self.C11 + self.C12)  # Poisson's ratio
        sigma_zz = nu * (sigma_xx + sigma_yy)
        
        # Apply scaling based on kappa parameter
        scale_factor = kappa * 10.0  # Convert to GPa scale
        sigma_xx *= scale_factor
        sigma_yy *= scale_factor
        sigma_zz *= scale_factor
        tau_xy *= scale_factor
        ...
        self.stress_fields = {
            ...
            'sigma_zz': sigma_zz,
            'tau_xy': tau_xy,
            'von_mises': self.compute_von_mises(sigma_xx, sigma_yy, sigma_zz, tau_xy),
            'sigma_hydro': (sigma_xx + sigma_yy + sigma_zz) / 3.0
        }
        
        return self.stress_fields
    
    def compute_von_mises(self, sxx, syy, szz, txy):
    ...
    def run_simulation(self, params):
        ...
        params : dict
            ...
            - shape: defect shape
            - eps0: eigenstrain
            - kappa: elasticity parameter
        """
        # Extract parameters
        theta = params.get('theta', 0.0)
        defect_type = params.get('defect_type', 'Twin')
        shape = params.get('shape', 'Square')
        eps0 = params.get('eps0', PhysicsParameters.get_eigenstrain(defect_type))
        kappa = params.get('kappa', 0.6)
        
        # Create defect shape
        self.create_defect_shape(shape_type=shape, theta=theta, defect_size=3.0)
        
        # Compute stress fields
        stress_fields = self.compute_stress_fields(eps0=eps0, kappa=kappa, defect_type=defect_type)
        
        # Create history record
        history_record = {
            'phi': self.phi.copy(),
            'stresses': stress_fields.copy(),
            'params': params.copy(),
            'step': 0,
        ...
        return {
            ...
            'metadata': {
                ...
                'domain_size': f'{self.L} nm x {self.L} nm',
                'grid_points': self.N,
                'grid_spacing': f'{self.dx} nm'
            }
        }
    
    def visualize_domain(self, figsize=(10, 8)):
        """Visualize the simulation domain"""
        fig, axes = plt.subplots(1, 3, figsize=figsize)
        
        # Domain grid
        ax1 = axes[0]
        ax1.scatter(self.X.flatten(), self.Y.flatten(), s=1, alpha=0.3)
        ax1.set_xlabel('X (nm)')
        ax1.set_ylabel('Y (nm)')
        ax1.set_title(f'Domain: {self.L} nm x {self.L} nm')
        ...
        # Coordinate axes
        ax2 = axes[1]
        ax2.quiver(0, 0, 1, 0, scale=10, color='r', label='X')
        ax2.quiver(0, 0, 0, 1, scale=10, color='b', label='Y')
        ax2.set_xlim([-2, 2])
        ax2.set_ylim([-2, 2])
        ax2.set_aspect('equal')
        ax2.set_title('Coordinate System')
        ax2.legend()
        ...
        ax3.add_patch(rect)
        ax3.plot(0, 0, 'ko', markersize=10, label='Center (0,0)')
        ax3.set_xlim([-self.L/2-1, self.L/2+1])
        ax3.set_ylim([-self.L/2-1, self.L/2+1])
        ax3.set_aspect('equal')
        ax3.set_title('Domain Boundaries')
        ax3.legend()
...
class SimulationGenerator:
    """Generate new phase-field simulations with 12.8 nm domain"""
    
    def __init__(self):
        self.simulation = PhaseFieldSimulation(N=128, dx=0.1)
        
    def generate_simulation(self, params):
        """
        Generate a new simulation with given parameters
        ...
        """
        return self.simulation.run_simulation(params)
    
    def save_simulation(self, simulation_result, filename=None):
        """
        Save simulation result to file
        """
        if filename is None:
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            theta = np.degrees(simulation_result['params'].get('theta', 0))
            defect_type = simulation_result['params'].get('defect_type', 'Twin')
            filename = f"simulation_theta_{theta:.1f}_{defect_type}_{timestamp}.pkl"
    ...
    def generate_multiple_simulations(self, angle_range=(0, 180), num_simulations=10):
        """
        Generate multiple simulations across a range of angles
        ...
        """
        simulations = []
        angles = np.linspace(angle_range[0], angle_range[1], num_simulations)
        ...
        for angle in angles:
            params = {
                ...
                'defect_type': 'Twin',
                'shape': 'Square',
                'eps0': PhysicsParameters.get_eigenstrain('Twin'),
                'kappa': 0.6
            }
            
            simulation = self.generate_simulation(params)
...
# =============================================
# ENHANCED HEAT MAP VISUALIZER WITH 12.8 nm DOMAIN
# =============================================
class HeatMapVisualizer:
    """Enhanced heat map visualizer with diffusion visualization and 12.8 nm domain"""
    def __init__(self, domain_size=12.8):
        self.domain_size = domain_size
        self.extent = [-domain_size/2, domain_size/2, -domain_size/2, domain_size/2]
    ...
    def create_diffusion_heatmap(self, sigma_hydro_field, title="Diffusion Enhancement Map",
        ...
        if log_scale:
            ...
            im = ax.imshow(log_data, cmap=cmap, vmin=vmin, vmax=vmax,
                          extent=self.extent, aspect=aspect_ratio, 
    ...
    def create_interactive_diffusion_heatmap(self, sigma_hydro_field, title="Diffusion Enhancement Map",
        ...
        try:
            ...
            # Create physical coordinates for 12.8 nm domain
            n_rows, n_cols = sigma_hydro_field.shape
            x_phys = np.linspace(-self.domain_size/2, self.domain_size/2, n_cols)
            y_phys = np.linspace(-self.domain_size/2, self.domain_size/2, n_rows)
            ...
            for i in range(plot_data.shape[0]):
                ...
                for j in range(plot_data.shape[1]):
                    base_text = f"Position: ({x_phys[j]:.2f}, {y_phys[i]:.2f}) nm<br>"
    ...
    def create_diffusion_3d_surface_plot(self, sigma_hydro_field, title="3D Diffusion Surface",
        ...
        # Create physical coordinate meshgrid for 12.8 nm domain
        n_rows, n_cols = sigma_hydro_field.shape
        x_phys = np.linspace(-self.domain_size/2, self.domain_size/2, n_cols)
        y_phys = np.linspace(-self.domain_size/2, self.domain_size/2, n_rows)
        ...
        try:
            ...
            for i in range(plot_data.shape[0]):
                ...
                for j in range(plot_data.shape[1]):
                    if log_scale:
                        value = D_ratio[i, j]
                        base_text = f"Position: ({X[i, j]:.2f}, {Y[i, j]:.2f}) nm<br>"
                        base_text += f"σₕ: {sigma_hydro_field[i, j]:.4f} GPa<br>"
                        base_text += f"D/D₀: {value:.4f}<br>"
                        base_text += f"log₁₀(D/D₀): {plot_data[i, j]:.3f}"
                    else:
    ...
    def create_diffusion_gradient_map(self, D_ratio_field, title="Diffusion Gradient Analysis",
        ...
        ax1 = axes[0, 0]
        im1 = ax1.imshow(grad_magnitude, cmap='hot', extent=self.extent,
        ...
        X_phys = np.linspace(-self.domain_size/2, self.domain_size/2, D_ratio_field.shape[1])[::stride]
        Y_phys = np.linspace(-self.domain_size/2, self.domain_size/2, D_ratio_field.shape[0])[::stride]
        X_phys_grid, Y_phys_grid = np.meshgrid(X_phys, Y_phys)
        
        U = grad_x[::stride, ::stride]
        V = grad_y[::stride, ::stride]
        ax2.quiver(X_phys_grid, Y_phys_grid, U, V, grad_magnitude[::stride, ::stride], cmap='hot',
        ...
        ax2.set_aspect('equal')
        ax2.set_xlim([-self.domain_size/2, self.domain_size/2])
        ax2.set_ylim([-self.domain_size/2, self.domain_size/2])
        
        # 3. D/D₀ field
        ax3 = axes[1, 0]
        im3 = ax3.imshow(D_ratio_field, cmap='RdBu_r', extent=self.extent,
                        aspect='equal', interpolation='bilinear', origin='lower',
        ...
        im4 = ax4.imshow(laplacian, cmap='coolwarm', extent=self.extent,
                        aspect='equal', interpolation='bilinear', origin='lower')
...
# =============================================
# MAIN APPLICATION WITH ENHANCED SIMULATION
# =============================================
def main():
    ...
    if 'heatmap_visualizer' not in st.session_state: st.session_state.heatmap_visualizer = HeatMapVisualizer(domain_size=12.8)
    ...
    if 'simulation_generator' not in st.session_state: st.session_state.simulation_generator = SimulationGenerator()
    if 'phase_field_sim' not in st.session_state: st.session_state.phase_field_sim = PhaseFieldSimulation(N=128, dx=0.1)
    ...
    with st.sidebar:
        ...
        st.markdown('<h2 class="section-header">🔄 Generate New Simulation</h2>', unsafe_allow_html=True)
        if st.button("🔄 Generate Simulation", type="primary", use_container_width=True):
            with st.spinner("Generating new simulation..."):
                params = {
                ...
                simulation_result = st.session_state.simulation_generator.generate_simulation(params)
                filepath = st.session_state.simulation_generator.save_simulation(simulation_result)
                
                if filepath:
                    st.success(f"Simulation saved to {os.path.basename(filepath)}")
                    st.session_state.solutions.append(simulation_result)
        
        st.divider()
        
        if st.button("🧠 Perform Interpolation", type="primary", use_container_width=True):
            if not st.session_state.solutions:
                st.error("Please load solutions first or generate new simulations!")
            else:
    ...
    # Main Content Area
    st.markdown('<h2 class="section-header">📐 12.8 nm Domain Visualization</h2>', unsafe_allow_html=True)
    
    # Domain Visualization
    col1, col2 = st.columns(2)
    
    with col1:
        st.markdown("#### Domain Specification")
        st.markdown(f"""
        **Domain Size:** {st.session_state.phase_field_sim.L} nm × {st.session_state.phase_field_sim.L} nm
        
        **Grid Parameters:**
        - Grid points: {st.session_state.phase_field_sim.N} × {st.session_state.phase_field_sim.N}
        - Grid spacing: {st.session_state.phase_field_sim.dx} nm
        - Total points: {st.session_state.phase_field_sim.N ** 2:,}
        
        **Physical Extent:**
        - X-range: ±{st.session_state.phase_field_sim.L/2} nm
        - Y-range: ±{st.session_state.phase_field_sim.L/2} nm
        - Center: (0, 0) nm
        """)
    
    with col2:
        if st.button("Visualize Domain", use_container_width=True):
            fig_domain = st.session_state.phase_field_sim.visualize_domain()
    ...
    if st.session_state.interpolation_result:
        ...
        # Create tabs
        diffusion_viz_tab, stress_tab, physics_tab = st.tabs(["🔬 Diffusion Visualization", "📈 Stress Analysis", "⚛️ Physics"])
        ...
        with diffusion_viz_tab:
            ...
            if diffusion_viz_type == 'Diffusion Enhancement (D/D₀)':
                ...
                im = ax.imshow(C_ratio, cmap='plasma', extent=st.session_state.heatmap_visualizer.extent,
                              aspect='equal', interpolation='bilinear', origin='lower',
                              norm=LogNorm(vmin=0.1, vmax=10))
            ...
            if st.button("Compare Materials", use_container_width=True):
                ...
                for i, (mat, title) in enumerate(zip(materials, titles)):
                    ...
                    im = axes[i].imshow(plot_data, cmap=cmap_name, vmin=vmin, vmax=vmax,
                                      extent=st.session_state.heatmap_visualizer.extent,
                                      aspect='equal', interpolation='bilinear', origin='lower')
                    ...
                    axes[i].set_xlabel('X Position (nm)')
                    axes[i].set_ylabel('Y Position (nm)')
        ...
        with stress_tab:
            st.markdown('<h2 class="section-header">📈 Stress Field Analysis</h2>', unsafe_allow_html=True)
            
            if 'von_mises' in result['fields']:
                # Create stress visualization
                fig_stress, axes = plt.subplots(2, 2, figsize=(14, 12), dpi=300)
                
                # Von Mises stress
                ax1 = axes[0, 0]
                im1 = ax1.imshow(result['fields']['von_mises'], cmap='hot', 
                                extent=st.session_state.heatmap_visualizer.extent,
                                aspect='equal', interpolation='bilinear', origin='lower')
                plt.colorbar(im1, ax=ax1, label='Von Mises Stress (GPa)')
                ax1.set_title('Von Mises Stress', fontsize=16, fontweight='bold')
                ax1.set_xlabel('X Position (nm)')
                ax1.set_ylabel('Y Position (nm)')
                
                # Hydrostatic stress
                ax2 = axes[0, 1]
                im2 = ax2.imshow(result['fields']['sigma_hydro'], cmap='RdBu_r',
                                extent=st.session_state.heatmap_visualizer.extent,
                                aspect='equal', interpolation='bilinear', origin='lower')
                plt.colorbar(im2, ax=ax2, label='Hydrostatic Stress (GPa)')
                ax2.set_title('Hydrostatic Stress', fontsize=16, fontweight='bold')
                ax2.set_xlabel('X Position (nm)')
                ax2.set_ylabel('Y Position (nm)')
                
                # Stress magnitude
                ax3 = axes[1, 0]
                im3 = ax3.imshow(result['fields']['sigma_mag'], cmap='viridis',
                                extent=st.session_state.heatmap_visualizer.extent,
                                aspect='equal', interpolation='bilinear', origin='lower')
                plt.colorbar(im3, ax=ax3, label='Stress Magnitude (GPa)')
                ax3.set_title('Stress Magnitude', fontsize=16, fontweight='bold')
                ax3.set_xlabel('X Position (nm)')
                ax3.set_ylabel('Y Position (nm)')
                ...
                stats_text = (
                    f"Von Mises Stress:\n"
                    f"  Max: {result['statistics']['von_mises']['max']:.2f} GPa\n"
                    f"  Mean: {result['statistics']['von_mises']['mean']:.2f} GPa\n\n"
                    f"Hydrosstatic Stress:\n"
                    f"  Max tension: {result['statistics']['sigma_hydro']['max_tension']:.2f} GPa\n"
                    f"  Max compression: {result['statistics']['sigma_hydro']['max_compression']:.2f} GPa\n\n"
                    f"Stress Magnitude:\n"
                    f"  Max: {result['statistics']['sigma_mag']['max']:.2f} GPa\n"
                    f"  Mean: {result['statistics']['sigma_mag']['mean']:.2f} GPa"
                )
                
                ax4.text(0.1, 0.5, stats_text, transform=ax4.transAxes,
                        fontsize=12, verticalalignment='center',
                        bbox=dict(boxstyle='round', facecolor='white', alpha=0.9))
                ax4.set_title('Stress Statistics', fontsize=16, fontweight='bold')
                
                plt.suptitle(f'Stress Field Analysis - θ = {result["target_angle"]:.1f}°, {result["target_params"]["defect_type"]}',
                           fontsize=18, fontweight='bold', y=0.95)
                plt.tight_layout()
                st.pyplot(fig_stress)
        
        with physics_tab:
            st.markdown('<h2 class="section-header">⚛️ Physics Parameters</h2>', unsafe_allow_html=True)
            
            col_phys1, col_phys2 = st.columns(2)
            
            with col_phys1:
                st.markdown("#### Domain Physics")
                st.markdown(f"""
                **Grid Specifications:**
                - Domain size: 12.8 nm × 12.8 nm
                - Grid points: 128 × 128
                - Grid spacing: 0.1 nm
                - Total cells: 16,384
                ...
                **Elastic Properties (Silver):**
                - C11: 124.0 GPa
                - C12: 93.4 GPa
                - C44: 46.1 GPa
                - Poisson's ratio: {0.93/(1+0.93):.3f}
                """)
            
            with col_phys2:
                st.markdown("#### Defect Physics")
                defect_info = PhysicsParameters.get_theoretical_info(result['target_params']['defect_type'])
                
                if defect_info:
                    st.markdown(f"""
                    **{result['target_params']['defect_type']} Defect:**
                    - Eigenstrain: {defect_info['value']}
                    - Formula: {defect_info['formula']}
                    - Description: {defect_info['description']}
                    - Reference: {defect_info['reference']}
                    """)
                
                st.markdown("#### Diffusion Parameters")
                props = DiffusionPhysics.get_material_properties(diffusion_material)
                st.markdown(f"""
                **{diffusion_material} Properties:**
                - Atomic volume: {props['atomic_volume']:.2e} m³
                - Activation energy: {props['activation_energy']} eV
                - Prefactor: {props['prefactor']:.2e} m²/s
                - Melting point: {props['melting_point']} K
                - Bulk modulus: {props['bulk_modulus']/1e9:.0f} GPa
                """)
            ...
            if 'weights' in result:
                weights_df = pd.DataFrame({
                    'Source Angle (°)': result['source_theta_degrees'],
                    'Angular Distance (°)': result['source_distances'],
                    'Combined Weight': result['weights']['combined'],
                    'Spatial Kernel': result['weights']['spatial_kernel'],
                    'Defect Mask': result['weights']['defect_mask']
                })
                
                # Sort by weight
                weights_df = weights_df.sort_values('Combined Weight', ascending=False)
