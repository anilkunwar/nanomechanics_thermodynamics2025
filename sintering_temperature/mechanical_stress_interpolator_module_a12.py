        ...
        with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
            ...
            for pred_key, pred_data in multi_predictions.items():
                ...
                for field_name, field_data in stress_fields.items():
                    ...
                    npz_buffer.seek(0)
                    zip_file.writestr(f'{pred_dir}/stress_{field_name}.npz', npz_buffer.read())
...
2. attention_weights.csv - Attention weights
3. stress_*.npz - Stress fields
4. statistics.csv - Stress statistics
...
def create_attention_interface():
    """Create the attention interpolation interface with simplified download pattern"""
    ...
    # Tab 7: Export Results (SIMPLIFIED DIRECT DOWNLOAD PATTERN)
    with tab7:
        ...
        if not has_multi_predictions:
            ...
            # Multi-target predictions - DIRECT PATTERN
            if has_multi_predictions:
                ...
                with col1:
                    # Multi-target ZIP archive - DIRECT PATTERN
                    zip_buffer = st.session_state.prediction_results_manager.create_multi_prediction_archive(
                        st.session_state.multi_target_predictions,
                        st.session_state.source_simulations
                    )
                    ...
                    # Download button for multi-target ZIP
                    st.download_button(
                ...
                with col2:
                    # Summary CSV for multi-target - DIRECT PATTERN
                    summary_rows = []
            ...
            # Visualization downloads - DIRECT PATTERN
            ...
            # Numerical solutions directory files - DIRECT PATTERN
                ...
                with st.expander("📂 Files in Directory (Click to expand)", expanded=False):
                    for file_info in all_files_info[:10]: # Limit to 10 files
                        ...
                        with col_file2:
                            ...
                            # Download button (direct pattern)
                            st.download_button(
                                label="📥 Download",
                                data=file_data,
                                file_name=file_name,
