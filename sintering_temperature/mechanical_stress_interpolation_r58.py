...
class HeatMapVisualizer:
    ...
    # ENHANCEMENT: New method for summation-based radar chart
    def create_summation_radar(self, weights, source_info, target_params, title="Summation-Based Weight Distinction Radar"):
        """
        Radial radar chart showing summation distinctions of spatial weights, pre-mask and post-mask attention.
        Sources on circumference, target highlighted.
        """
        ...
        dimensions = ['Spatial Sum', 'Pre-Mask Sum', 'Post-Mask Sum']
        sums_data = [sum_spatial, sum_pre_mask, sum_post_mask]

        fig = go.Figure()
        fig.add_trace(go.Scatterpolar(
            r=sums_data + [sums_data[0]],
            theta=dimensions + [dimensions[0]],
            fill='toself',
            name='Weight Sums',
        ...
        # Add source points on circumference
        max_r = max(sums_data) * 1.2
        for i, theta in enumerate(source_info['theta_degrees']):
            fig.add_trace(go.Scatterpolar(
                r=[max_r],
                theta=[theta],
                mode='markers',
                marker=dict(size=10, symbol='circle', color='black'),
                name=f'Source {i} ({theta:.1f}°)',
                hovertemplate=f'Angle: {theta:.1f}°'
            ))
    ...
    def create_summation_sunburst(self, weights, source_info, target_params, title="Summation Distinction Sunburst"):
        ...
        # Add summation levels
        sum_levels = ['Spatial Sum', 'Pre-Mask Sum', 'Post-Mask Sum']
        sum_values = [np.sum(weights['spatial_kernel']), np.sum(weights['pre_mask']), np.sum(weights['combined'])]
        for level, val in zip(sum_levels, sum_values):
            labels.append(level)
            parents.append('Target')
            values.append(val)

        # Add sources under sums (simplified)
        for i, theta in enumerate(source_info['theta_degrees']):
            labels.append(f'Source {i} ({theta:.1f}°)')
            parents.append('Post-Mask Sum')  # Attach to post-mask as example
            values.append(weights['combined'][i])

        fig = go.Figure(go.Sunburst(
            labels=labels, parents=parents, values=values, branchvalues='total',
            maxdepth=3, insidetextorientation='radial'
        ))

        # Highlight target in hover
        fig.update_traces(hovertemplate='<b>%{label}</b><br>Value: %{value:.3f}')
    ...
    def create_chord_diagram(self, weights, source_info, target_params, title="Chord Diagram: Defect, Spatial, Attention Proximity"):
        ...
        sorted_indices = np.argsort(source_info['distances'])
        angles = np.linspace(0, 360, n_sources, endpoint=False)
        radius = 1.0

        # Node positions
        node_x = [radius * np.cos(np.radians(ang)) for ang in angles]
        node_y = [radius * np.sin(np.radians(ang)) for ang in angles]

        # Add nodes
        defect_colors = px.colors.qualitative.Set1  # Color by defect
        ...
        defect_color_map = {dt: defect_colors[i % len(defect_colors)] for i, dt in enumerate(unique_defects)}

        fig.add_trace(go.Scatter(
            x=node_x, y=node_y,
            mode='markers+text',
            marker=dict(size=15, color=[defect_color_map[dt] for dt in defect_types]),
            text=[f'{i}' for i in range(n_sources)],
            hovertemplate='Source %{text}<br>Angle: %{customdata[0]:.1f}°<br>Defect: %{customdata[1]}',
            customdata=list(zip(source_info['theta_degrees'], defect_types))
        ))

        # Add chords (Bezier curves) between every pair, but filter low attention
        for i in range(n_sources):
            for j in range(i + 1, n_sources):
                att_weight = weights['combined'][i] * weights['combined'][j]  # Proxy for pair attention
                if att_weight < 0.01: continue  # Threshold

                spatial_w = weights['spatial_kernel'][i] + weights['spatial_kernel'][j]  # Average spatial
                control_point = 0.5 + spatial_w  # Curvature by spatial weight

                # Bezier curve points
                x0, y0 = node_x[i], node_y[i]
                x2, y2 = node_x[j], node_y[j]
                x1 = (x0 + x2) / 2 + control_point * (y2 - y0) / 2  # Perpendicular offset for curve
                y1 = (y0 + y2) / 2 - control_point * (x2 - x0) / 2
                ...
                fig.add_trace(go.Scatter(
                    x=bez_x, y=bez_y,
                    mode='lines',
                    line=dict(width=att_weight * 10, color=defect_color_map[defect_types[i]]),  # Thickness by att, color by defect
                    hovertemplate=f'From {i} to {j}<br>Attention Prox: {att_weight:.3f}<br>Spatial: {spatial_w:.3f}'
                ))
    ...
    if st.session_state.interpolation_result:
        ...
        with attention_tab:
            ...
            viz_options = [
                ...
                "Radar Chart: Summation Distinctions",  # ENHANCEMENT: New option
                "Sunburst Chart: Summation Distinctions",  # ENHANCEMENT: New option
                "Chord Diagram: Defect/Spatial/Attention Proximity"  # ENHANCEMENT: New option
            ]
            ...
            try:
                ...
                # ENHANCEMENT: Handle new summation radar
                    ...
                    st.plotly_chart(fig, use_container_width=True)
                    st.markdown("""
                    **Interpretation:** Radial axes show summed weights/distinctions. Sources on circumference, target highlighted in red.
                    """)

                # ENHANCEMENT: Handle new summation sunburst
                    ...
                    st.plotly_chart(fig, use_container_width=True)
                    st.markdown("""
                    **Interpretation:** Hierarchy includes summation levels for spatial/pre/post-mask distinctions.
                    """)

                # ENHANCEMENT: Handle chord diagram
                    ...
                    st.plotly_chart(fig, use_container_width=True)
                    st.markdown("""
                    **Interpretation:** Nodes = sources (circle by proximity). Chords: thickness=attention, color=defect, curvature=spatial weight.
                    """)
