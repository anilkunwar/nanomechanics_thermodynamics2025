...
class EnhancedHeatMapVisualizer:
    ...
    def create_comparison_dashboard(self, interpolated_fields, source_fields, source_info,
        ...
        # Get statistics for each component — FIXED HANDLING FOR sigma_hydro
        stats_data = []
        if 'statistics' in source_info:
            for comp in components:
                if comp in source_info['statistics']:
                    ...
                    if comp == 'sigma_hydro':
                        # Use max of |tension| and |compression| as 'max'
                        max_val = max(
                            abs(stats.get('max_tension', 0)),
                            abs(stats.get('max_compression', 0))
                        )
                    else:
...
""", unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        with st.expander(f"ℹ️ About {method_info['name']}", expanded=False):
