...
# =============================================
# DOMAIN CONFIGURATION - 12.8 nm x 12.8 nm GRID
# =============================================
# Phase-field simulation domain parameters
# Grid: 128 x 128 points with 0.1 nm spacing
# Physical size: 128 * 0.1 nm = 12.8 nm
# Centered at origin (0, 0) with extent: -6.4 nm to +6.4 nm

DOMAIN_SIZE_NM = 12.8          # Total physical domain length (nm)
GRID_POINTS_N = 128            # Number of grid points (N)
DX_NM = 0.1                    # Grid spacing: 12.8 nm / 128 = 0.1 nm/pixel
HALF_DOMAIN = DOMAIN_SIZE_NM / 2  # 6.4 nm (half-length for coordinate bounds)

# Explicit coordinate extent: [xmin, xmax, ymin, ymax] = [-6.4, 6.4, -6.4, 6.4]
# This centers the domain at (0,0) with exactly 12.8 nm total width/height
DOMAIN_EXTENT = [-HALF_DOMAIN, HALF_DOMAIN, -HALF_DOMAIN, HALF_DOMAIN]

# Physical coordinate arrays for proper axis labeling
# X and Y range from -6.4 nm to +6.4 nm with 128 points
X_COORDS_NM = np.linspace(-HALF_DOMAIN, HALF_DOMAIN, GRID_POINTS_N)
Y_COORDS_NM = np.linspace(-HALF_DOMAIN, HALF_DOMAIN, GRID_POINTS_N)
X_MESH_NM, Y_MESH_NM = np.meshgrid(X_COORDS_NM, Y_COORDS_NM)

# Verification:
# Total length check: N * dx = 128 * 0.1 = 12.8 nm ✓
# Center check: (-6.4 + 6.4)/2 = 0 ✓
# Range check: -6.4 nm to +6.4 nm ✓
# =============================================
    ...
    def _standardize_data(self, data, file_path):
        standardized = {
            ...
            'metadata': {
                ...
                'domain_size_nm': DOMAIN_SIZE_NM,  # Explicit domain metadata
...
class TransformerSpatialInterpolator:
    def __init__(self, d_model=64, nhead=8, num_layers=3,
        ...
        # Domain configuration stored for spatial calculations
        self.domain_size = DOMAIN_SIZE_NM  # 12.8 nm
        self.grid_points = GRID_POINTS_N   # 128
        self.dx = DX_NM                    # 0.1 nm
    ...
    def interpolate_spatial_fields(self, sources, target_angle_deg, target_params):
        ...
        try:
            ...
            # Ensure all fields have the same shape (should match 128x128 domain)
            shapes = [f['von_mises'].shape for f in source_fields]
            if len(set(shapes)) > 1:
                target_shape = (GRID_POINTS_N, GRID_POINTS_N)  # Ensure 128x128
                resized_fields = []
            ...
            # Compute diffusion enhancement using physical grid spacing
            if 'sigma_hydro' in interpolated_fields:
                ...
                # Compute gradient of diffusion field (using physical dx in nm)
                grad_x, grad_y = np.gradient(D_ratio, DX_NM)  # Use 0.1 nm spacing
    ...
    def compute_von_mises(self, stress_fields):
        if all(k in stress_fields for k in ['sigma_xx', 'sigma_yy', 'sigma_zz', 'tau_xy']):
            ...
            von_mises = np.sqrt(0.5 * ((sxx-syy)**2 + (syy-szz)**2 + (szz-sxx)**2 +
                                     6*(txy**2 + tyz**2 + tzx**2)))
            return von_mises
        return np.zeros((GRID_POINTS_N, GRID_POINTS_N))  # Use domain constant
...
class HeatMapVisualizer:
    ...
    def __init__(self):
        # Store domain configuration for coordinate generation
        self.domain_size = DOMAIN_SIZE_NM      # 12.8 nm
        self.grid_points = GRID_POINTS_N       # 128
        self.dx = DX_NM                        # 0.1 nm
        self.extent = DOMAIN_EXTENT            # [-6.4, 6.4, -6.4, 6.4] nm
    ...
    def create_diffusion_heatmap(self, sigma_hydro_field, title="Diffusion Enhancement Map",
        ...
        # Use DOMAIN_EXTENT to ensure axes show -6.4 to +6.4 nm
        extent = self.extent
        ...
        if log_scale:
            ...
            im = ax.imshow(log_data, cmap=cmap, vmin=vmin, vmax=vmax,
                          ...
                          extent=extent)  # Apply physical coordinates
        ...
        # Updated labels to show domain range explicitly
        xlabel_text = label_config['xlabel'] if label_config and 'xlabel' in label_config else f"X Position (nm) [{-HALF_DOMAIN} to {HALF_DOMAIN}]"
        ylabel_text = label_config['ylabel'] if label_config and 'ylabel' in label_config else f"Y Position (nm) [{-HALF_DOMAIN} to {HALF_DOMAIN}]"
        self._apply_label(ax, f"X Position (nm)", xlabel_text, 'set_xlabel', fontsize=16, fontweight='bold')
        self._apply_label(ax, f"Y Position (nm)", ylabel_text, 'set_ylabel', fontsize=16, fontweight='bold')
        
        # Set axis limits to enforce the 12.8 nm domain visualization
        ax.set_xlim([-HALF_DOMAIN, HALF_DOMAIN])
        ax.set_ylim([-HALF_DOMAIN, HALF_DOMAIN])
        ...
        if show_stats:
            ...
            stats_text = (f"Max Enhancement: {np.max(D_ratio):.2f}x\nMin (Suppression): {np.min(D_ratio):.2f}x\n"
                         ...
                         f"Domain: {DOMAIN_SIZE_NM}×{DOMAIN_SIZE_NM} nm²")
    ...
    def create_publication_quality_diffusion_plot(self, sigma_hydro_field, title="Diffusion Enhancement",
        ...
        extent = self.extent  # Use [-6.4, 6.4, -6.4, 6.4]
    ...
    def create_interactive_diffusion_heatmap(self, sigma_hydro_field, title="Diffusion Enhancement Map",
        ...
        try:
            ...
            # Create hover text with enhanced information including physical coordinates
            hover_text = []
            for i in range(plot_data.shape[0]):
                # Map row index to physical Y coordinate (-6.4 to 6.4 nm)
                y_phys = -HALF_DOMAIN + (i / (GRID_POINTS_N - 1)) * DOMAIN_SIZE_NM if GRID_POINTS_N > 1 else 0
                row_text = []
                for j in range(plot_data.shape[1]):
                    # Map column index to physical X coordinate (-6.4 to 6.4 nm)
                    x_phys = -HALF_DOMAIN + (j / (GRID_POINTS_N - 1)) * DOMAIN_SIZE_NM if GRID_POINTS_N > 1 else 0
                    
                    base_text = f"X: {x_phys:.2f} nm, Y: {y_phys:.2f} nm<br>"
            ...
            # Update layout with publication styling and explicit axis labels showing domain
            fig.update_layout(
                title=dict(
                    text=title_str,
                ...
                height=height,
                xaxis=dict(
                    title=dict(text=f"X Position (nm) [{-HALF_DOMAIN} to {HALF_DOMAIN}]", font=dict(size=18, color="black")),
                    tickfont=dict(size=14),
                    gridcolor='rgba(150, 150, 150, 0.3)',
                    scaleanchor="y",
                    scaleratio=1,
                    range=[-HALF_DOMAIN, HALF_DOMAIN]  # Explicit axis range
                ),
                yaxis=dict(
                    title=dict(text=f"Y Position (nm) [{-HALF_DOMAIN} to {HALF_DOMAIN}]", font=dict(size=18, color="black")),
                    tickfont=dict(size=14),
                    gridcolor='rgba(150, 150, 150, 0.3)',
                    scaleanchor="x",
                    scaleratio=1,
            ...
            return fig, np.ones((GRID_POINTS_N, GRID_POINTS_N))
    ...
    def create_diffusion_3d_surface_plot(self, sigma_hydro_field, title="3D Diffusion Surface",
        ...
        # Create meshgrid using physical coordinates (nm)
        x = np.linspace(-HALF_DOMAIN, HALF_DOMAIN, GRID_POINTS_N)
        y = np.linspace(-HALF_DOMAIN, HALF_DOMAIN, GRID_POINTS_N)
        X, Y = np.meshgrid(x, y)
        ...
        ax.set_title(title_str, fontsize=20, fontweight='bold', pad=20)
        ax.set_xlabel(f'X Position (nm)', fontsize=16, fontweight='bold', labelpad=10)
        ax.set_ylabel(f'Y Position (nm)', fontsize=16, fontweight='bold', labelpad=10)
    ...
    def create_interactive_diffusion_3d(self, sigma_hydro_field, title="Interactive 3D Diffusion",
        ...
        try:
            ...
            # Create meshgrid with physical coordinates
            x = np.linspace(-HALF_DOMAIN, HALF_DOMAIN, GRID_POINTS_N)
            y = np.linspace(-HALF_DOMAIN, HALF_DOMAIN, GRID_POINTS_N)
            X, Y = np.meshgrid(x, y)
            ...
            for i in range(plot_data.shape[0]):
                ...
                for j in range(plot_data.shape[1]):
                    if log_scale:
                        value = D_ratio[i, j]
                        base_text = f"X: {X[i,j]:.2f} nm<br>Y: {Y[i,j]:.2f} nm<br>"
                        base_text += f"σₕ: {sigma_hydro_field[i, j]:.4f} GPa<br>"
                        base_text += f"D/D₀: {value:.4f}<br>"
                        base_text += f"log₁₀(D/D₀): {plot_data[i, j]:.3f}"
                    else:
            ...
            try:
                surface_trace = go.Surface(
                    z=plot_data,
                    x=X,  # Physical coordinates in nm
                    y=Y,  # Physical coordinates in nm
                    colorscale=plotly_cmap,
                    opacity=0.9,
                    contours={
                        "z": {
            ...
            # Update layout with explicit physical ranges
            fig.update_layout(
                title=dict(
                    text=title_str,
                    font=dict(size=24, color='darkblue'),
            ...
            # Add annotation for key statistics including domain size
            max_enhance = np.max(D_ratio)
            min_enhance = np.min(D_ratio)
            mean_enhance = np.mean(D_ratio)
            fig.add_annotation(
                text=(f"Max: {max_enhance:.2f}x<br>Min: {min_enhance:.2f}x<br>Mean: {mean_enhance:.2f}x<br>"
                      f"Domain: {DOMAIN_SIZE_NM}×{DOMAIN_SIZE_NM} nm²"),
    ...
    def create_diffusion_gradient_map(self, D_ratio_field, title="Diffusion Gradient Analysis",
        ...
        """Create comprehensive visualization of diffusion gradients with physical coordinates"""
        # Compute gradient magnitude and direction using physical dx (0.1 nm)
        grad_x, grad_y = np.gradient(D_ratio_field, DX_NM)  # Physical gradient (1/nm)
        grad_magnitude = np.sqrt(grad_x**2 + grad_y**2)
        
        # Create figure with 2x2 subplots
        fig, axes = plt.subplots(2, 2, figsize=figsize, dpi=dpi)
        extent = self.extent  # [-6.4, 6.4, -6.4, 6.4] nm
        
        # 1. Gradient magnitude
        ax1 = axes[0, 0]
        im1 = ax1.imshow(grad_magnitude, cmap='hot', aspect='equal',
                        interpolation='bilinear', origin='lower', extent=extent)
        plt.colorbar(im1, ax=ax1, label=f'Gradient Magnitude (nm⁻¹)')
        ax1.set_title('Diffusion Gradient Magnitude', fontsize=14, fontweight='bold')
        ax1.set_xlabel(f'X Position (nm) [{-HALF_DOMAIN} to {HALF_DOMAIN}]')
        ax1.set_ylabel(f'Y Position (nm) [{-HALF_DOMAIN} to {HALF_DOMAIN}]')
        ax1.set_xlim([-HALF_DOMAIN, HALF_DOMAIN])
        ax1.set_ylim([-HALF_DOMAIN, HALF_DOMAIN])
        
        # 2. Gradient direction (quiver plot)
        ax2 = axes[0, 1]
        stride = max(1, D_ratio_field.shape[0] // 20)
        # Create physical coordinate grids for quiver
        y_phys = np.linspace(-HALF_DOMAIN, HALF_DOMAIN, GRID_POINTS_N)
        x_phys = np.linspace(-HALF_DOMAIN, HALF_DOMAIN, GRID_POINTS_N)
        Y, X = np.meshgrid(y_phys, x_phys, indexing='ij')
        
        U = grad_x[::stride, ::stride]
        V = grad_y[::stride, ::stride]
        ax2.quiver(X[::stride, ::stride], Y[::stride, ::stride], U, V, 
                  grad_magnitude[::stride, ::stride], cmap='hot',
                  angles='xy', scale_units='xy', scale=0.5)
        ax2.set_title('Gradient Vector Field', fontsize=14, fontweight='bold')
        ax2.set_xlabel(f'X Position (nm)')
        ax2.set_ylabel(f'Y Position (nm)')
        ax2.set_xlim([-HALF_DOMAIN, HALF_DOMAIN])
        ax2.set_ylim([-HALF_DOMAIN, HALF_DOMAIN])
        ...
        im3 = ax3.imshow(D_ratio_field, cmap='RdBu_r', aspect='equal',
                        ...
                        norm=LogNorm(vmin=0.1, vmax=10), extent=extent)
        plt.colorbar(im3, ax=ax3, label='D/D₀')
        ax3.set_title(f'Diffusion Enhancement (D/D₀)\n{DOMAIN_SIZE_NM}×{DOMAIN_SIZE_NM} nm² domain', 
                     fontsize=14, fontweight='bold')
        ax3.set_xlabel(f'X Position (nm)')
        ax3.set_ylabel(f'Y Position (nm)')
        ax3.set_xlim([-HALF_DOMAIN, HALF_DOMAIN])
        ax3.set_ylim([-HALF_DOMAIN, HALF_DOMAIN])
        
        # 4. Laplacian (divergence of gradient)
        ax4 = axes[1, 1]
        # Second derivatives with physical spacing
        grad_xx = np.gradient(grad_x, DX_NM)[0]
        grad_yy = np.gradient(grad_y, DX_NM)[1]
        laplacian = grad_xx + grad_yy
        im4 = ax4.imshow(laplacian, cmap='coolwarm', aspect='equal',
                        interpolation='bilinear', origin='lower', extent=extent)
        plt.colorbar(im4, ax=ax4, label=f'Laplacian (∇²D) (nm⁻²)')
        ax4.set_title('Diffusion Field Curvature', fontsize=14, fontweight='bold')
        ax4.set_xlabel(f'X Position (nm)')
        ax4.set_ylabel(f'Y Position (nm)')
        ax4.set_xlim([-HALF_DOMAIN, HALF_DOMAIN])
        ax4.set_ylim([-HALF_DOMAIN, HALF_DOMAIN])
    ...
    def create_stress_diffusion_correlation(self, sigma_hydro_field, D_ratio_field,
        ...
        ax1.set_ylabel('D/D₀')
        ax1.set_title(f'Stress-Diffusion Correlation\n{DOMAIN_SIZE_NM}×{DOMAIN_SIZE_NM} nm² Domain', 
                     fontsize=14, fontweight='bold')
        ax1.legend()
        ...
        stats_text = (
            ...
            f"Domain Size: {DOMAIN_SIZE_NM}×{DOMAIN_SIZE_NM} nm²\n"
            f"Resolution: {DX_NM} nm/pixel"
        )
...
class ResultsManager:
    ...
    def prepare_export_data(self, interpolation_result, visualization_params):
        ...
        export_data = {
            'metadata': {
                ...
                'grid_points': GRID_POINTS_N,
                'grid_spacing_nm': DX_NM,
                'coordinate_extent': DOMAIN_EXTENT
            },
            'result': {
    ...
    def export_to_json(self, export_data, filename=None):
        if filename is None:
            ...
            filename = f"bracketing_interpolation_theta_{theta}_{defect}_{DOMAIN_SIZE_NM}nm_{timestamp}.json"
    ...
    def export_to_csv(self, interpolation_result, filename=None):
        if filename is None:
            ...
            filename = f"stress_fields_theta_{theta}_{defect}_{DOMAIN_SIZE_NM}nm_{timestamp}.csv"
            
        data_dict = {}
        
        # Add coordinate information
        x_flat = X_MESH_NM.flatten()
        y_flat = Y_MESH_NM.flatten()
        data_dict['X_nm'] = x_flat
        data_dict['Y_nm'] = y_flat
...
def main():
    ...
    with st.sidebar:
        ...
        # Display Domain Configuration prominently
        st.markdown("#### 📐 Domain Configuration")
        st.info(f"""
        **Simulation Domain:**
        - Size: {DOMAIN_SIZE_NM} nm × {DOMAIN_SIZE_NM} nm
        - Grid: {GRID_POINTS_N} × {GRID_POINTS_N} points
        - Spacing: {DX_NM} nm/pixel
        - Center: (0, 0) nm
        - Extent: [{-HALF_DOMAIN}, {HALF_DOMAIN}] nm
        """)
        st.divider()
    ...
    if st.session_state.interpolation_result:
        ...
        with diffusion_viz_tab:
            ...
            # Display theoretical background with enhanced styling and domain info
            st.markdown(f"""
            <div class="info-box">
            <strong>📚 Diffusion Enhancement Theory (Domain: {DOMAIN_SIZE_NM} nm × {DOMAIN_SIZE_NM} nm):</strong><br>
            Grid: {GRID_POINTS_N}×{GRID_POINTS_N} points, Resolution: {DX_NM} nm/pixel, Extent: [{-HALF_DOMAIN}, {HALF_DOMAIN}] nm<br>
            ...
            • <strong>Tensile stress (σ_h > 0)</strong> → Enhanced diffusion (D/D₀ > 1)<br>
            • <strong>Compressive stress (σ_h < 0)</strong> → Suppressed diffusion (D/D₀ < 1)<br>
            • <strong>Habit plane (θ ≈ 54.7°)</strong> → Maximum tensile stress → Peak enhancement at center
            </div>
            """, unsafe_allow_html=True)
            ...
            if st.checkbox("Preview Colormap", key="preview_diffusion_colormap"):
                ...
                ax_cmap.set_title(f"Colormap: {cmap_name} (Domain: {DOMAIN_SIZE_NM} nm)", fontsize=12)
                ax_cmap.set_xticks([])
                ax_cmap.set_yticks([])
                st.pyplot(fig_cmap)
            ...
            if diffusion_viz_type == 'Diffusion Enhancement (D/D₀)':
                ...
                fig_diff, D_ratio = st.session_state.heatmap_visualizer.create_diffusion_heatmap(
                    sigma_hydro_field,
                    title=f"Diffusion Enhancement Map ({DOMAIN_SIZE_NM} nm × {DOMAIN_SIZE_NM} nm)",
                    T_K=diffusion_T,
                ...
                # Show key statistics with domain info
                ...
                st.markdown(f"**Domain:** {DOMAIN_SIZE_NM} nm × {DOMAIN_SIZE_NM} nm | **Resolution:** {DX_NM} nm/pixel")
                ...
                fig_3d, D_ratio = st.session_state.heatmap_visualizer.create_diffusion_3d_surface_plot(
                    sigma_hydro_field,
                    title=f"3D Diffusion Landscape ({DOMAIN_SIZE_NM} nm × {DOMAIN_SIZE_NM} nm)",
                    T_K=diffusion_T,
                    material=diffusion_material,
                    cmap_name=cmap_name,
                    figsize=(14, 10),
                ...
                im = ax.imshow(C_ratio, cmap='plasma', aspect='equal',
                              ...
                              extent=DOMAIN_EXTENT)  # Physical coordinates
                ...
                ax.set_xlabel(f'X Position (nm) [{-HALF_DOMAIN} to {HALF_DOMAIN}]', fontsize=16, fontweight='bold')
                ax.set_ylabel(f'Y Position (nm) [{-HALF_DOMAIN} to {HALF_DOMAIN}]', fontsize=16, fontweight='bold')
                ax.set_xlim([-HALF_DOMAIN, HALF_DOMAIN])
                ax.set_ylim([-HALF_DOMAIN, HALF_DOMAIN])
                ...
                with col_grad1:
                    st.metric("Max Gradient", f"{np.max(grad_mag):.3f} nm⁻¹")
                with col_grad2:
                    st.metric("Mean Gradient", f"{np.mean(grad_mag):.3f} nm⁻¹")
                with col_grad3:
                    steep_regions = np.sum(grad_mag > np.mean(grad_mag) + np.std(grad_mag))
                    st.metric("Steep Regions", f"{steep_regions}")
                
                st.caption(f"Gradients computed with physical spacing dx = {DX_NM} nm")
                ...
                st.info(f"Current conditions: σ = {mean_stress:.2f} GPa, T = {diffusion_T} K, Domain = {DOMAIN_SIZE_NM} nm")
            ...
            if st.button("Compare Materials", use_container_width=True):
                ...
                titles = [f"{mat} at {diffusion_T}K\n{DOMAIN_SIZE_NM} nm domain" for mat in materials]
                ...
                for i, (mat, title) in enumerate(zip(materials, titles)):
                    ...
                    axes[i].set_xlim([-HALF_DOMAIN, HALF_DOMAIN])
                    axes[i].set_ylim([-HALF_DOMAIN, HALF_DOMAIN])
