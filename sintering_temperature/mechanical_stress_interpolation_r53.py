...
class HeatMapVisualizer:
    ...
    def create_diffusion_3d_surface(self, sigma_hydro_field, title="3D Diffusion Surface",
        ...
        # Create meshgrid for 12.8 nm domain
        domain_info = DomainConfiguration.get_domain_info()
        N = DomainConfiguration.N
        dx = DomainConfiguration.dx
        x = np.linspace(-domain_info['domain_half_nm'], domain_info['domain_half_nm'], N, endpoint=False)
        y = np.linspace(-domain_info['domain_half_nm'], domain_info['domain_half_nm'], N, endpoint=False)
        X, Y = np.meshgrid(x, y)
        ...
        # Add domain info
        ax.text2D(0.02, 0.98, f"Domain: {domain_info['domain_length_nm']} nm", 
    ...
    def create_interactive_diffusion_3d(self, sigma_hydro_field, title="Interactive Diffusion 3D",
        ...
        # Create hover text with domain coordinates
        hover_text = []
        for i in range(D_ratio.shape[0]):
            ...
            for j in range(D_ratio.shape[1]):
                ...
                row_text.append(
                    f"X: {x_pos:.2f} nm, Y: {y_pos:.2f} nm<br>"
        ...
        fig.update_layout(
            ...
            scene=dict(
                xaxis=dict(
                    ...
                    showbackground=True,
                    range=[-domain_info['domain_half_nm'], domain_info['domain_half_nm']]
                ),
                yaxis=dict(
        ...
        fig.add_annotation(
            text=f"Domain: {domain_info['domain_length_nm']} nm<br>Max: {max_enhance:.2f}x<br>Min: {min_enhance:.2f}x<br>Mean: {mean_enhance:.2f}x",
    ...
    def create_publication_quality_plot(self, stress_field, title="Publication Quality Plot",
        ...
        # Add scale bar for 12.8 nm domain
        scale_length = domain_info['domain_length_nm'] * 0.2  # 20% of domain
        x_start = -domain_info['domain_half_nm'] * 0.8
        y_pos = -domain_info['domain_half_nm'] * 0.9
        ax.plot([x_start, x_start + scale_length], 
               [y_pos, y_pos], 
               'k-', linewidth=3)
        ax.text(x_start + scale_length/2, y_pos - 0.5, 
               f'{scale_length:.1f} nm', ha='center', fontsize=10, fontweight='bold')
        
        # Add domain info annotation
        ax.text(0.02, 0.98, f"Domain: {domain_info['domain_length_nm']} nm",
               transform=ax.transAxes, fontsize=10, fontweight='bold',
    ...
    def create_interactive_heatmap(self, stress_field, title="Stress Heat Map",
        ...
        try:
            ...
            # Get domain coordinates for 12.8 nm domain
            domain_info = DomainConfiguration.get_domain_info()
            N = DomainConfiguration.N
            x = np.linspace(-domain_info['domain_half_nm'], domain_info['domain_half_nm'], N, endpoint=False)
            y = np.linspace(-domain_info['domain_half_nm'], domain_info['domain_half_nm'], N, endpoint=False)
            ...
            # Create heatmap trace with domain coordinates
            ...
            fig.update_layout(
                ...
                height=height,
                xaxis=dict(
                    title=dict(text="X Position (nm)", font=dict(size=18, family="Arial", color="black")),
                    tickfont=dict(size=14, family='Arial'),
                ...
                yaxis=dict(
                    title=dict(text="Y Position (nm)", font=dict(size=18, family="Arial", color="black")),
                    tickfont=dict(size=14, family='Arial'),
                    gridcolor='rgba(150, 150, 150, 0.3)',
                    scaleanchor="x",
                    scaleratio=1,
            ...
            # Add domain info annotation
            fig.add_annotation(
                text=f"Domain: {domain_info['domain_length_nm']} nm × {domain_info['domain_length_nm']} nm",
                xref="paper", yref="paper",
                x=0.02, y=0.98,
                showarrow=False,
                font=dict(size=12, color="black", family="Arial"),
    ...
    def create_comparison_dashboard(self, interpolated_fields, source_fields, source_info,
        ...
        5. 12.8 nm domain explicitly shown
        ...
        # Get domain info for 12.8 nm domain
        domain_info = DomainConfiguration.get_domain_info()
        extent = domain_info['extent']
        ...
        axis_labels = {
            'xlabel': custom_labels.get('xlabel', 'X Position (nm)'),
            'ylabel': custom_labels.get('ylabel', 'Y Position (nm)'),
        ...
        if plot6_visible and 'statistics' in source_info:
            ...
            if stats_data:
                ...
                # Add domain info
                ax6.text(0.02, 0.98, f"Domain: {domain_info['domain_length_nm']} nm",
        ...
        if plot8_visible and ground_truth_index is not None and 0 <= ground_truth_index < len(source_fields):
            ...
            if gt_field is not None and component in interpolated_fields:
                ...
                # Add domain info
                ax8.text(0.02, 0.98, f"Domain: {domain_info['domain_length_nm']} nm",
    ...
    def create_interactive_3d_surface(self, stress_field, title="3D Stress Surface",
        ...
        try:
            ...
            fig.add_annotation(
                text=f"Domain: {domain_info['domain_length_nm']} nm<br>Max: {max_stress:.2f} GPa<br>Min: {min_stress:.2f} GPa<br>Mean: {mean_stress:.2f} GPa",
                xref="paper", yref="paper",
                x=0.02, y=0.98,
                showarrow=False,
                font=dict(size=12, color="black", family="Arial"),
    ...
    def create_comparison_heatmaps(self, stress_fields_dict, cmap_name='viridis',
        ...
        for idx, ((component_name, stress_field), title) in enumerate(zip(stress_fields_dict.items(), titles)):
            ...
            # Create heatmap with equal aspect ratio and domain extent
            im = ax.imshow(stress_field, cmap=cmap, aspect='equal', 
                          interpolation='bilinear', origin='lower',
                          extent=extent)
        ...
        # Add super title with target parameters and domain info
    ...
    def create_diffusion_comparison_dashboard(self, sigma_hydro_fields, titles,
        ...
        # Add domain info
        ax_stats.text(0.02, 0.98, f"Domain: {domain_info['domain_length_nm']} nm",
        ...
        for i, D_ratio in enumerate(D_ratios):
            ...
            x_profile = np.linspace(-domain_info['domain_half_nm'], domain_info['domain_half_nm'], len(profile))
        ...
        ax_profile.set_xlabel('X Position (nm)')
    ...
    def create_phase_diagram_visualization(self, sigma_range=(-5, 5), T_range=(300, 1200),
        ...
        plt.suptitle(f'Diffusion Enhancement Phase Diagram - {material}\nDomain: 12.8 nm × 12.8 nm', 
    ...
    def create_vacancy_concentration_map(self, sigma_hydro_field, T_K=650, material='Silver',
        ...
        # Title and labels
        ax.set_title(f'Vacancy Concentration Enhancement\n{material}, T = {T_K} K, Domain: {domain_info["domain_length_nm"]} nm',
    ...
    def create_diffusion_gradient_map(self, D_ratio_field, figsize=(12, 10), dpi=300):
        ...
        # Create coordinate arrays for 12.8 nm domain
        N = DomainConfiguration.N
        x = np.linspace(-domain_info['domain_half_nm'], domain_info['domain_half_nm'], N, endpoint=False)
        y = np.linspace(-domain_info['domain_half_nm'], domain_info['domain_half_nm'], N, endpoint=False)
        X, Y = np.meshgrid(x, y)
        ...
        X_down = X[::stride, ::stride]
        Y_down = Y[::stride, ::stride]
        
        ax2.quiver(X_down, Y_down, U, V, grad_magnitude[::stride, ::stride],
        ...
        plt.suptitle(f'Diffusion Gradient Analysis\nDomain: {domain_info["domain_length_nm"]} nm', 
                    fontsize=16, fontweight='bold', y=0.98)
        plt.tight_layout()
    ...
    def create_stress_diffusion_correlation(self, sigma_hydro_field, D_ratio_field,
        ...
        plt.suptitle(f'Stress-Diffusion Enhancement Correlation Analysis\nDomain: {domain_info["domain_length_nm"]} nm', 
                    fontsize=16, fontweight='bold', y=0.98)
        plt.tight_layout()
...
def create_dashboard_configuration_interface():
    ...
    with st.expander("📝 Custom Titles & Labels", expanded=False):
        ...
        col1, col2 = st.columns(2)
        with col1:
            xlabel = st.text_input("X-axis Label", value="X Position (nm)")
            ylabel = st.text_input("Y-axis Label", value="Y Position (nm)")
...
def main():
    ...
    if st.session_state.interpolation_result:
        ...
        with tab1:
            ...
            st.markdown(f"""
            <div class="info-box">
            <strong>Domain Configuration:</strong><br>
            • <strong>Size:</strong> {domain_info['domain_length_nm']} nm × {domain_info['domain_length_nm']} nm<br>
            • <strong>Grid:</strong> {domain_info['grid_points']} × {domain_info['grid_points']} points<br>
            • <strong>Spacing:</strong> {domain_info['grid_spacing_nm']} nm<br>
            • <strong>Extent:</strong> ±{domain_info['domain_half_nm']} nm<br>
            • <strong>Area:</strong> {domain_info['area_nm2']:.1f} nm²
            </div>
            """, unsafe_allow_html=True)
        ...
        with tab2:
            ...
            # Domain info reminder
            st.info(f"**Domain:** {domain_info['domain_length_nm']} nm × {domain_info['domain_length_nm']} nm (±{domain_info['domain_half_nm']} nm)")
        ...
        with tab3:
            ...
            if 'sigma_hydro' in result['fields']:
                ...
                if diffusion_visualization_type == 'Diffusion Enhancement (D/D₀)':
                    ...
                    ax.set_title(f'Effective Diffusion Coefficient\n{material}, T = {diffusion_T} K, Domain: {domain_info["domain_length_nm"]} nm',
        ...
        with tab5:
            ...
            # Domain info reminder
            st.info(f"**Domain:** {domain_info['domain_length_nm']} nm × {domain_info['domain_length_nm']} nm (Centered at 0, ±{domain_info['domain_half_nm']} nm)")
                        ...
                        if comp in result['fields']:
                            ...
                            if comp == 'diffusion_ratio':
                                im = ax.imshow(np.log10(np.clip(field, 0.1, 10)), cmap='RdBu_r',
                                              vmin=-1, vmax=1, aspect='equal', interpolation='bilinear', 
