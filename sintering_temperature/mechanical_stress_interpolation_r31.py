...
class AdvancedPhysicsStressAnalyzer:
    ...
    def calculate_stress_intensity_factors(self, stress_field, defect_type, crack_length=1e-6):
        ...
        if hasattr(stress_field, '__len__'):
            sigma_max = np.max(stress_field) if len(stress_field) > 0 else 0
        else:
    ...
    def generate_physical_insights(self, stress_patterns, defect_type, angle_range):
        ...
        # Stress analysis - FIX: Handle empty or None stress_patterns
        if stress_patterns is None:
            stresses = np.array([])
            peak_stress = 0.0
            peak_angle = None
        elif hasattr(stress_patterns, '__len__'):
            ...
            if len(stresses) == 0:
                ...
                if angle_range is not None and len(angle_range) > peak_idx:
                    peak_angle = angle_range[peak_idx]
                else:
                    peak_angle = None
                
                # Calculate stress gradient
                if angle_range is not None and len(angle_range) > 1:
                    stress_gradient = np.gradient(stresses, np.deg2rad(angle_range))
                else:
                    stress_gradient = np.array([0.0])
                
                # Calculate stress concentration factor
                if len(stresses) > 0 and np.mean(stresses) > 0:
                    scf = peak_stress / np.mean(stresses)
                else:
                    scf = 1.0
                
                insights['stress_analysis'] = {
                    'peak_stress_gpa': float(peak_stress),
                    'peak_angle_degrees': float(peak_angle) if peak_angle is not None else None,
                    'mean_stress_gpa': float(np.mean(stresses)) if len(stresses) > 0 else 0.0,
                    'stress_amplitude_gpa': float(np.std(stresses)) if len(stresses) > 1 else 0.0,
                    'stress_concentration_factor': float(scf),
                    'max_gradient_gpa_per_degree': float(np.max(np.abs(stress_gradient))) if len(stress_gradient) > 0 else 0.0,
                    'angular_fwhm_degrees': self._calculate_fwhm(stresses, angle_range) if len(stresses) > 0 else 0.0,
                    'stress_integral_gpa_degree': float(np.trapz(stresses, angle_range)) if len(stresses) > 0 and angle_range is not None else 0.0
                }
        else:
            # Handle scalar input
            peak_stress = float(stress_patterns)
            stresses = np.array([stress_patterns])
            insights['stress_analysis'] = {
                'peak_stress_gpa': float(peak_stress),
        ...
        insights['processing_guidelines'] = {
            ...
            'optimal_orientation_degrees': float(peak_angle) if peak_angle is not None else self.crystal_geometry['habit_plane']['angle'],
    ...
    def _calculate_fwhm(self, data, angles):
        ...
        if len(data) == 0 or angles is None or len(angles) == 0:
            return 0.0
    ...
    def _optimize_thermal_cycle(self, defect_type, peak_stress):
        ...
        if peak_stress > 20:  # GPa
            ...
            return cycles.get(defect_type, cycles['No Defect'])
...
class AdvancedPhysicsInterpolator:
    ...
    def _calculate_hydrostatic_stress(self, eigen_strain, delta_theta, defect_type, shape):
        ...
        # {111} planes have different elastic response
        if isinstance(delta_theta, (int, float)):
        ...
        for defect_type, results in results_dict.items():
            ...
            if len(stresses) == 0:
                ...
                metrics[defect_type] = {
                    ...
                    'stress_integral': 0.0,
                    'peak_location': 0.0,
                    'habit_plane_stress': 0.0,
                    'stress_gradient_max': 0.0,
                    'diffusion_enhancement_peak': 1.0,
                    'sintering_temp_reduction_peak': 0.0
                }
                continue
            ...
            metrics[defect_type] = {
                ...
                'stress_amplitude': float(np.std(stresses)) if len(stresses) > 1 else 0.0,
                'angular_fwhm': self._calculate_angular_fwhm(stresses, angles),
                'stress_integral': float(np.trapz(stresses, angles)),
                'peak_location': float(angles[peak_idx]) if len(angles) > peak_idx else 0.0,
                'habit_plane_stress': float(stresses[np.argmin(np.abs(np.array(angles) - self.habit_angle))]),
                'stress_gradient_max': float(np.max(np.abs(np.gradient(stresses, angles)))) if len(stresses) > 1 else 0.0,
                'diffusion_enhancement_peak': float(results['diffusion_enhancement'][peak_idx] if len(results['diffusion_enhancement']) > peak_idx else 1.0),
                'sintering_temp_reduction_peak': float(results['sintering_temp_reduction'][peak_idx] if len(results['sintering_temp_reduction']) > peak_idx else 0.0)
            }
        ...
        for defect_type in ['ISF', 'ESF', 'No Defect']:
            if defect_type in metrics:
                ...
                advantages[defect_type] = {
                    'peak_stress_ratio': twin_metrics['peak_stress'] / defect_metrics['peak_stress'] if defect_metrics['peak_stress'] > 0 else 0,
                    'diffusion_enhancement_ratio': twin_metrics['diffusion_enhancement_peak'] / defect_metrics['diffusion_enhancement_peak'] if defect_metrics['diffusion_enhancement_peak'] > 0 else 0,
                    'sintering_advantage_k': twin_metrics['sintering_temp_reduction_peak'] - defect_metrics['sintering_temp_reduction_peak'],
                    'stress_concentration_advantage': twin_metrics['stress_integral'] / defect_metrics['stress_integral'] if defect_metrics['stress_integral'] > 0 else 0
                }
    ...
    def _calculate_angular_fwhm(self, stresses, angles):
        ...
        if len(stresses) == 0 or len(angles) == 0:
            return 0.0
    ...
    def _determine_optimal_defect(self, metrics):
        ...
        if not scores:
            return {
                'optimal_defect': 'No Defect',
                'scores': {'ISF': 0, 'ESF': 0, 'Twin': 0, 'No Defect': 0},
                'recommendation': self._generate_optimal_defect_recommendation('No Defect', metrics)
            }
...
class AdvancedDefectRadarVisualizer:
    ...
    def create_physics_insights_radar(self, defect_comparison, 
        ...
        for defect_key, data in defect_comparison.items():
            ...
            if 'diffusion_enhancement' in data and len(data['diffusion_enhancement']) > 0:
                diffusion = np.array(data['diffusion_enhancement'])
                # Scale for visualization (0-1 range)
                if len(diffusion) > 0 and np.max(diffusion) > 0:
        ...
        for data in defect_comparison.values():
            if len(data['stresses']['sigma_hydro']) > 0:
                max_stresses.append(np.max(data['stresses']['sigma_hydro']))
        
        if max_stresses:
            max_stress = max(max_stresses)
            fig.add_trace(go.Scatterpolar(
                r=[0, max_stress * 1.2],
        ...
        fig.update_layout(
            ...
            polar=dict(
                radialaxis=dict(
                    ...
                    range=[0, max_stress * 1.2] if 'max_stress' in locals() else [0, 1]
                ),
                angularaxis=dict(
                    gridcolor=self.publication_styles['grid_color'],
                    gridwidth=self.publication_styles['grid_width'],
                    ...
                    direction="clockwise",
                    tickmode='array',
                    tickvals=np.linspace(min(angles), max(angles), 7) if 'angles' in locals() and len(angles) > 0 else [0, 90, 180, 270],
                    ticktext=[f'{i:.1f}°' for i in np.linspace(min(angles), max(angles), 7)] if 'angles' in locals() and len(angles) > 0 else ['0°', '90°', '180°', '270°'],
                    tickfont=self.publication_styles['tick_font'],
                    period=360
                ),
    ...
    def _generate_physics_annotations(self, defect_comparison):
        ...
        for i, (defect_key, data) in enumerate(defect_comparison.items()):
            ...
            diffusion = data.get('diffusion_enhancement', [1])[peak_idx] if len(data.get('diffusion_enhancement', [])) > peak_idx else 1
            temp_reduction = data.get('sintering_temp_reduction', [0])[peak_idx] if len(data.get('sintering_temp_reduction', [])) > peak_idx else 0
    ...
    def create_comparative_physics_dashboard(self, comparative_results):
        ...
        for data in results.values():
            if len(data['stresses']['sigma_hydro']) > 0:
                max_stress = max(max_stress, np.max(data['stresses']['sigma_hydro']))
        ...
        for d in defect_types:
            if d in metrics and 'diffusion_enhancement_peak' in metrics[d]:
                diffusion_peaks.append(metrics[d]['diffusion_enhancement_peak'])
                valid_defect_types.append(d)
        
        if valid_defect_types and diffusion_peaks:
            fig.add_trace(
                go.Bar(
                    x=valid_defect_types,
                    y=diffusion_peaks,
                    marker_color=[self.color_schemes['defect_types'][d] for d in valid_defect_types],
        ...
        temp_reductions = []
        valid_defect_types_temp = []
        for d in defect_types:
            if d in metrics and 'sintering_temp_reduction_peak' in metrics[d]:
                temp_reductions.append(metrics[d]['sintering_temp_reduction_peak'])
                valid_defect_types_temp.append(d)
        
        if valid_defect_types_temp and temp_reductions:
            fig.add_trace(
                go.Bar(
                    x=valid_defect_types_temp,
                    y=temp_reductions,
                    marker_color=[self.color_schemes['defect_types'][d] for d in valid_defect_types_temp],
        ...
        stress_peaks = []
        valid_defect_types_stress = []
        for d in defect_types:
            if d in metrics and 'peak_stress' in metrics[d]:
                stress_peaks.append(metrics[d]['peak_stress'])
                valid_defect_types_stress.append(d)
        
        if valid_defect_types_stress and stress_peaks:
            fig.add_trace(
                go.Bar(
                    x=valid_defect_types_stress,
                    y=stress_peaks,
                    marker_color=[self.color_schemes['defect_types'][d] for d in valid_defect_types_stress],
                    name='Peak Stress',
        ...
        # Plot 5: Angular FWHM
        fwhm_values = []
        valid_defect_types_fwhm = []
        for d in defect_types:
            if d in metrics and 'angular_fwhm' in metrics[d]:
                fwhm_values.append(metrics[d]['angular_fwhm'])
                valid_defect_types_fwhm.append(d)
        
        if valid_defect_types_fwhm and fwhm_values:
            fig.add_trace(
                go.Bar(
                    x=valid_defect_types_fwhm,
                    y=fwhm_values,
                    marker_color=[self.color_schemes['defect_types'][d] for d in valid_defect_types_fwhm],
                    name='Angular FWHM',
        ...
        for defect_type in defect_types:
            if defect_type in metrics:
                table_data.append([
                    defect_type,
                    f"{metrics[defect_type].get('peak_stress', 0):.2f}",
                    f"{metrics[defect_type].get('diffusion_enhancement_peak', 1):.1f}",
                    f"{metrics[defect_type].get('sintering_temp_reduction_peak', 0):.0f}",
                    f"{metrics[defect_type].get('angular_fwhm', 0):.1f}",
                    f"{metrics[defect_type].get('stress_gradient_max', 0):.3f}"
                ])
    ...
    def create_interactive_physics_explorer(self, comparative_results):
        ...
        for defect_type in defect_types:
            ...
            # Temperature data
            if 'sintering_temp_reduction' in data and len(data['sintering_temp_reduction']) > 0:
        ...
        for i, defect_type in enumerate(defect_types):
            if i < len(metrics_data['stress']['data']):
                fig.add_trace(go.Scatterpolar(
                    r=metrics_data['stress']['data'][i],
                    theta=angles_closed if 'angles_closed' in locals() else [],
                    mode='lines+markers',
        ...
        fig.update_layout(
            ...
            polar=dict(
                radialaxis=dict(
                    title=dict(text=metrics_data['stress']['unit'] if 'stress' in metrics_data else 'GPa'),
                    gridcolor=self.publication_styles['grid_color']
                ),
                angularaxis=dict(
                    gridcolor=self.publication_styles['grid_color'],
    ...
    def create_physics_insights_report(self, comparative_results):
        ...
        report['summary'] = {
            ...
            'peak_stress_gpa': comp_analysis['individual_metrics'][optimal_defect]['peak_stress'] if optimal_defect in comp_analysis['individual_metrics'] else 0,
            'diffusion_enhancement': comp_analysis['individual_metrics'][optimal_defect]['diffusion_enhancement_peak'] if optimal_defect in comp_analysis['individual_metrics'] else 1,
            'temperature_reduction_k': comp_analysis['individual_metrics'][optimal_defect]['sintering_temp_reduction_peak'] if optimal_defect in comp_analysis['individual_metrics'] else 0,
        ...
        for defect_type, data in results.items():
            report['defect_analysis'][defect_type] = {
                ...
                'peak_stress_location': comp_analysis['individual_metrics'][defect_type]['peak_location'] if defect_type in comp_analysis['individual_metrics'] else 0,
                'angular_fwhm': comp_analysis['individual_metrics'][defect_type]['angular_fwhm'] if defect_type in comp_analysis['individual_metrics'] else 0,
                'stress_integral': comp_analysis['individual_metrics'][defect_type]['stress_integral'] if defect_type in comp_analysis['individual_metrics'] else 0,
                'physics_insights': data.get('physics_insights', {})
            }
        
        # Comparative metrics
        report['comparative_metrics'] = {
            'relative_performance': comp_analysis.get('relative_advantages', {}),
            'defect_scores': comp_analysis['optimal_defect'].get('scores', {}),
            'performance_ranking': sorted(
                comp_analysis['individual_metrics'].keys(),
                key=lambda x: comp_analysis['individual_metrics'][x]['peak_stress'],
                reverse=True
            ) if comp_analysis['individual_metrics'] else []
        }
    ...
    def _generate_processing_guidelines(self, comp_analysis):
        ...
        return {
            'recommended_method': guidelines['defect_introduction'].get(optimal, 'High-purity processing with minimal deformation'),
            'optimal_temperature': guidelines['temperature_control'].get(optimal, 'Slow heating to 800-900K, extended annealing'),
            'pressure_range': guidelines['pressure_requirements'].get(optimal, '0.1-0.3 GPa for minimal defect introduction'),
    ...
    def _define_quality_control_metrics(self, comp_analysis):
        ...
        return {
            'defect_density_target': metrics['defect_density'].get(optimal, '<10^9 m^-2 (TEM)'),
...
def main():
            ...
            with st.spinner("Performing comprehensive physics analysis..."):
                try:
                    ...
                    for idx, defect_type in enumerate(selected_defects):
                        with cols[idx]:
                            if defect_type in filtered_results['individual_results']:
                    ...
                    with col_metric1:
                        peak_stresses = [metrics.get(d, {}).get('peak_stress', 0) for d in selected_defects]
                        if peak_stresses:
                    ...
                    with col_metric2:
                        diffusion_enhancements = [metrics.get(d, {}).get('diffusion_enhancement_peak', 1) for d in selected_defects]
                        if diffusion_enhancements:
                    ...
                    with col_metric3:
                        temp_reductions = [metrics.get(d, {}).get('sintering_temp_reduction_peak', 0) for d in selected_defects]
                        if temp_reductions:
    ...
    with tab2:
        ...
        if 'comparative_results' in st.session_state:
            ...
            for defect_type in selected_defects:
                if defect_type in results['individual_results']:
    ...
    with tab3:
        ...
        if 'comparative_results' in st.session_state:
            ...
            metrics_data = []
            for defect_type in selected_defects:
                if defect_type in comp_analysis['individual_metrics']:
                    m = comp_analysis['individual_metrics'][defect_type]
                    metrics_data.append([
                        defect_type,
                        f"{m.get('peak_stress', 0):.3f}",
                        f"{m.get('mean_stress', 0):.3f}",
                        f"{m.get('diffusion_enhancement_peak', 1):.1f}",
                        f"{m.get('sintering_temp_reduction_peak', 0):.0f}",
                        f"{m.get('angular_fwhm', 0):.2f}",
                        f"{m.get('stress_gradient_max', 0):.3f}",
                        f"{m.get('stress_integral', 0):.3f}"
                    ])
            ...
            for defect_type in selected_defects:
                if defect_type in comp_analysis['individual_metrics']:
                    ...
                    for metric in metrics_to_compare:
                        max_val = max(comp_analysis['individual_metrics'][d].get(metric, 0) 
                                    for d in selected_defects if d in comp_analysis['individual_metrics'])
                        if max_val > 0 and metric in m:
            ...
            if fig_comparison.data:  # Check if we have any data
            ...
            with col_opt1:
                ...
                **Performance Score**: {optimal_info['scores'].get(optimal_info['optimal_defect'], 0):.3f}
            ...
            with col_opt2:
                ...
                **Processing Guidelines**:
                {chr(10).join(['• ' + guideline for guideline in recommendation.get('processing_guidelines', [])])}
                """)
    ...
    with tab4:
        ...
        if 'comparative_results' in st.session_state:
            ...
            with st.expander("Optimal Defect Implementation", expanded=True):
                ...
                **Key Advantages**:
                {chr(10).join(['• ' + advantage for advantage in optimal.get('advantages', [])])}
                
                **Processing Guidelines**:
                {chr(10).join(['• ' + guideline for guideline in optimal.get('processing_guidelines', [])])}
                """)
