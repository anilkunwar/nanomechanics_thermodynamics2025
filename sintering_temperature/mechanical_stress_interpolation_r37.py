...
class EnhancedSolutionLoader:
    ...
    def analyze_angle_distribution(self, solutions):
        ...
        # Create histogram with 36 bins (10-degree increments)
        ...
        return {
            ...
            'angle_histogram': hist.tolist(),  # Convert to list for JSON serialization
            'angle_bin_edges': bin_edges.tolist(),  # Convert to list for JSON serialization
            'angle_bin_centers': ((bin_edges[:-1] + bin_edges[1:]) / 2).tolist()  # Center of each bin
        }
...
class TransformerSpatialInterpolator:
    ...
    def encode_parameters(self, params_list, target_angle_deg):
        ...
        for params in params_list:
            ...
            if len(features) != 18:
                ...
                features = features[:18] + [0.0] * max(0, 18 - len(features))
    ...
    def interpolate_at_multiple_angles(self, sources, angle_range, target_params, 
        ...
        for angle in angles:
            with st.spinner(f"Interpolating at θ = {angle:.1f}°..."):
...
class HeatMapVisualizer:
    ...
    def create_angle_comparison_plot(self, results_dict, component='von_mises', 
        ...
        for idx in range(len(angles), total_plots):
            if n_rows == 1 and n_cols == 1:
                ax = axes
            elif n_rows == 1:
                ax = axes[idx]
    ...
    with st.sidebar:
        ...
        if st.session_state.angle_distribution:
            with st.expander("📊 Angle Distribution", expanded=False):
                ...
                # Show histogram - FIXED: Use bin centers instead of bin edges
                fig, ax = plt.subplots(figsize=(8, 4))
                
                # Get bin centers and histogram data
                if 'angle_bin_centers' in dist and 'angle_histogram' in dist:
                    bin_centers = dist['angle_bin_centers']
                    histogram = dist['angle_histogram']
                    
                    # Ensure arrays have same length
                    if len(bin_centers) == len(histogram):
                        ax.bar(bin_centers, histogram, width=10, alpha=0.7)
                    else:
                        # Fallback: create simple histogram
                        angles = dist['angles_deg']
                        ax.hist(angles, bins=36, range=(0, 360), alpha=0.7, edgecolor='black')
                else:
        ...
        with tab4:
            ...
            if 'source_theta_degrees' in result and result['source_theta_degrees']:
                ...
                for i, theta in enumerate(result['source_theta_degrees']):
                    ...
                    regularity = result.get('source_regularities', [0.5]*len(result['source_theta_degrees']))[i]
                    ground_truth_options.append(
                        f"Source {i}: θ={theta:.1f}° (Δ={distance:.1f}°, weight={weight:.3f}, R={regularity:.3f})"
                    )
                ...
                selected_regularity = result.get('source_regularities', [0.5]*len(result['source_theta_degrees']))[selected_index]
                ...
                with col_gt4:
                    st.metric("Regularity Score", f"{selected_regularity:.3f}")
                ...
                if st.button("🔄 Quick Multi-Angle Analysis", key="quick_multi_angle"):
                    with st.spinner("Performing quick multi-angle analysis..."):
                        ...
                        if quick_results:
                            ...
                            st.success(f"Quick multi-angle analysis completed! Generated {len(quick_results)} angles.")
                            st.rerun()
