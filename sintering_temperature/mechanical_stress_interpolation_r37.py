...
# =============================================
# ENHANCED TRANSFORMER SPATIAL INTERPOLATOR WITH CONFIGURABLE SPATIAL LOCALITY
# =============================================
class EnhancedTransformerSpatialInterpolator:
"""Transformer-inspired stress interpolator with enhanced spatial locality regularization and configurable special angles"""
def __init__(self, d_model=64, nhead=8, num_layers=3, spatial_sigma=0.2, temperature=1.0, 
             locality_weight_factor=0.7, use_habit_plane_bias=True, special_angle=54.7):
"""
...
Args:
    ...
    spatial_sigma: Standard deviation for spatial weighting
    temperature: Temperature for attention softmax
    locality_weight_factor: Weight factor between spatial and transformer weights
    use_habit_plane_bias: Whether to use bias toward special_angle
    special_angle: The "special" angle to bias toward (default 54.7° for habit plane)
"""
self.d_model = d_model
self.nhead = nhead
...
self.use_habit_plane_bias = use_habit_plane_bias
...
"""Compute spatial locality weights with configurable special angle awareness"""
weights = []
...
# Habit plane angle (54.7° for martensitic transformations) - now configurable
habit_plane_angle = self.special_angle if self.use_habit_plane_bias else None
...
# Updated to be more robust for any target angle
...
# Special handling for habit plane (configurable) proximity if enabled
habit_proximity_bonus = 0.0
if self.use_habit_plane_bias and habit_plane_angle is not None:
...
# Reduced penalty for different defect types at same orientation
# This helps recognize that different defect types at same angle have similar profiles
if src_val != tgt_val:
param_dist += 0.05  # Small penalty for different defect types
elif param == 'shape':
if src_val != tgt_val:
param_dist += 0.1  # Small penalty for different shapes
elif param == 'eps0':
eps0_diff = abs(src_val - tgt_val)
# Apply a normalized penalty based on the relative difference
param_dist += (eps0_diff / (abs(src_val) + abs(tgt_val) + 1e-8)) * 0.1
elif param == 'kappa':
kappa_diff = abs(src_val - tgt_val)
param_dist += (kappa_diff / (abs(src_val) + abs(tgt_val) + 1e-8)) * 0.1
...
# Add habit plane bonus if enabled and applicable
if self.use_habit_plane_bias:
habit_plane_angle = self.special_angle
# Distance to habit plane
to_habit = abs(angle - habit_plane_angle)
to_habit = min(to_habit, 360 - to_habit)
            
# Bonus if near habit plane
if to_habit <= 10.0:
weight = min(1.0, weight + 0.1 * (1.0 - to_habit/10.0))
...
if self.use_habit_plane_bias:
ax.axvline(x=self.special_angle, color='g', linestyle='-.', linewidth=2,
label=f'Special Angle: {self.special_angle}°')
ax.set_xlabel('Angle (degrees)', fontsize=14, fontweight='bold')
ax.set_ylabel('Spatial Weight', fontsize=14, fontweight='bold')
title = f'Enhanced Angular Weighting Function\nTarget Angle: {target_angle_deg}°'
if self.use_habit_plane_bias:
title += f"\nSpecial Angle Bias: {self.special_angle}° enabled"
...
# Habit plane proximity (1 feature) - now configurable
if self.use_habit_plane_bias:
habit_distance = abs(theta_deg - self.special_angle)
habit_distance = min(habit_distance, 360 - habit_distance)  # Handle cyclic nature
features.append(np.exp(-habit_distance / 15.0))
else:
features.append(0.0)  # No bias when not enabled
...
close_sources = sum(1 for d in angular_distances if d < 10.0)
if close_sources < 3:
st.warning(f"Only {close_sources} sources within 10° of target angle. "
...
# Compute enhanced positional weights with configurable habit plane bias
...
st.set_page_config(
page_title="Generic Transformer Stress Interpolation",
layout="wide",
page_icon="🔬",
...
# Main header
st.markdown('<h1 class="main-header">🔬 Generic Transformer Stress Field Interpolation</h1>', unsafe_allow_html=True)
    
# Description
st.markdown("""
<div class="info-box">
<strong>🔬 Physics-aware stress interpolation with configurable spatial locality.</strong><br>
• <strong>FIXED:</strong> Updated eigenstrain values to match source files (Twin: 2.12, ESF: 1.414, ISF: 0.707)<br>
• <strong>FIXED:</strong> Recognizes similar orientation profiles across defect types<br>
• <strong>NEW:</strong> Configurable special angle (default 54.7° habit plane) with optional bias<br>
• <strong>NEW:</strong> Generic interpolation works for any orientation without special angle focus<br>
• <strong>NEW:</strong> Defect type similarity weighting to recognize similar spatial patterns<br>
...
use_habit_plane_bias=True,   # Default enabled
special_angle=54.7           # Default habit plane angle
)
...
# Eigenstrain calculation - FIXED TO MATCH SOURCE FILES
st.markdown("#### 🧮 Eigenstrain Calculation")
updated_eigenstrain_map = {
'ISF': 0.707,    # Updated to match source files
'ESF': 1.414,    # Updated to match source files
'Twin': 2.12,    # Updated to match source files
'No Defect': 0.0
}
...
with col_e2:
if auto_eigen:
# Auto-calculate based on defect type using updated values
eigen_strain = updated_eigenstrain_map[defect_type]
st.metric("Eigenstrain ε₀", f"{eigen_strain:.3f}", 
delta=f"Updated from previous values")
else:
eigen_strain = st.slider(
"Eigenstrain ε₀",
min_value=0.0,
max_value=3.0,
value=updated_eigenstrain_map[defect_type],  # Start with updated default
step=0.001,
help="Updated values: Twin=2.12, ESF=1.414, ISF=0.707"
)
        
st.divider()
        
# Transformer parameters
st.markdown('<h2 class="section-header">🧠 Spatial Locality Configuration</h2>', unsafe_allow_html=True)
...
st.markdown("#### 📍 Spatial Locality Parameters")
st.info("""
...
use_habit_plane_bias=use_special_angle,
...
# Special angle configuration
st.markdown("#### 🎯 Special Angle Status")
special_col1, special_col2, special_col3 = st.columns(3)
with special_col1:
st.metric(
"Special Angle Enabled",
"✅ Yes" if st.session_state.transformer_interpolator.use_habit_plane_bias else "❌ No"
)
with special_col2:
st.metric(
"Special Angle Value",
f"{st.session_state.transformer_interpolator.special_angle:.1f}°" if 
st.session_state.transformer_interpolator.use_habit_plane_bias else "N/A"
)
with special_col3:
if st.session_state.transformer_interpolator.use_habit_plane_bias:
angular_diff = abs(result['target_angle'] - st.session_state.transformer_interpolator.special_angle)
st.metric(
"Distance to Special Angle",
f"{angular_diff:.1f}°"
)
else:
st.metric(
...
special_angle=st.session_state.transformer_interpolator.special_angle if 
st.session_state.transformer_interpolator.use_habit_plane_bias else None,
figsize=(10, 8)
)
...
special_angle_val = st.session_state.transformer_interpolator.special_angle if st.session_state.transformer_interpolator.use_habit_plane_bias else None
...
target_angle=result['target_angle'],
defect_type=result['target_params']['defect_type'],
special_angle=special_angle_val,
figsize=(12, 10)
)
st.pyplot(fig_2d)
...
defect_type=result['target_params']['defect_type'],
figsize=(10, 10),
show_special_angle=st.session_state.transformer_interpolator.use_habit_plane_bias,
special_angle=st.session_state.transformer_interpolator.special_angle
)
...
• <strong>NEW:</strong> Recognizes similar orientation patterns across different defect types
</div>
""", unsafe_allow_html=True)
...
• <strong>NEW:</strong> Includes special angle configuration in exports
</div>
""", unsafe_allow_html=True)
...
'special_angle': st.session_state.transformer_interpolator.special_angle if 
st.session_state.transformer_interpolator.use_habit_plane_bias else None,
'use_special_angle': st.session_state.transformer_interpolator.use_habit_plane_bias
}
...
<li>✅ Fixed eigenstrain values to match source files (Twin: 2.12, ESF: 1.414, ISF: 0.707)</li>
<li>✅ Recognizes similar orientation profiles across different defect types</li>
<li>✅ Configurable special angle with optional bias</li>
<li>✅ Generic interpolation works for any orientation</li>
<li>✅ Warnings when insufficient sources near target orientation</li>
</ul>
</p>
</div>
""", unsafe_allow_html=True)
...
- **Eigenstrain Updates:** Fixed values match source files
- **Defect Type Similarity:** Recognizes similar spatial patterns
- **Configurable Special Angle:** Optional bias toward any angle
...
# Physics explanation
with st.expander("🧬 Physics Background: Updated Eigenstrains and Generic Orientation Handling", expanded=True):
st.markdown("""
**Key Improvements:**

1. **Updated Eigenstrain Values:**
...
2. **Generic Orientation Handling:**
   - **Special Angle Flexibility:** The code no longer assumes 54.7° is always special
   - **Configurable Bias:** Users can enable/disable bias toward any special angle
   - **Defect Type Similarity:** Recognizes that different defect types at the same orientation have similar spatial stress patterns but different magnitudes
   - **Cyclic Distance Calculation:** Properly handles 0° = 360° equivalence for accurate angular distances

3. **Improved Weighting Scheme:**
   - **Orientation Priority:** Gives high weight to sources with similar orientation regardless of defect type
   - **Magnitude Scaling:** Different defect types at same angle get weighted highly but scaled by their eigenstrain ratio
   - **Special Angle Bonus:** Optional bonus when both source and target are near the special angle
   - **Symmetry Recognition:** Identifies angles symmetric about the special angle

4. **Robustness Enhancements:**
   - **Source Proximity Warning:** Alerts when insufficient sources near target orientation
   - **Defect Type Awareness:** Considers defect type in weighting but doesn't dominate over orientation
   - **Generic First Approach:** Works well for any orientation without requiring special angle bias

**Example Workflow for Twin @ 54.7°:**
- Sources used would be: Twin @ 60°, Twin @ 30°, Twin @ 45° (high weight due to same defect type)
- ESF @ 50°, ISF @ 55° (moderate weight due to similar orientation but different defect type)
- The model recognizes that ESF @ 60° and ISF @ 60° have similar spatial patterns to Twin @ 60° but scaled by their eigenstrain ratios
- Special angle bonus applies if 54.7° bias is enabled, giving extra weight to sources near this angle
""")
