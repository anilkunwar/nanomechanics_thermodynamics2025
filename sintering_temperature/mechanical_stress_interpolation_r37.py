...
# =============================================
# CORRECTED TRANSFORMER SPATIAL INTERPOLATOR WITH SIMPLIFIED WEIGHTING
# =============================================
class CorrectedTransformerSpatialInterpolator:
    """Transformer-inspired stress interpolator with corrected physics parameters and simplified weighting"""
    def __init__(self, d_model=64, nhead=8, num_layers=3, spatial_sigma=0.2, temperature=1.0, locality_weight_factor=0.3):
        self.d_model = d_model
        self.nhead = nhead
        self.num_layers = num_layers
        self.spatial_sigma = spatial_sigma
        self.temperature = temperature
        self.locality_weight_factor = locality_weight_factor  # Lower value gives more weight to spatial weights
    ...
    def compute_positional_weights(self, source_params, target_params):
        """Compute spatial locality weights with simplified angular distance weighting"""
        weights = []
        ...
        for src in source_params:
            ...
            # Simplified angular distance weighting - pure proximity based
            if angular_diff <= 1.0:  # Very very close (within 1 degree)
                angular_weight = 1.0
            elif angular_diff <= 2.0:  # Very close (within 2 degrees)
                angular_weight = 0.95
            elif angular_diff <= 3.0:  # Close (within 3 degrees)
                angular_weight = 0.9
            elif angular_diff <= 5.0:  # Moderately close (within 5 degrees)
                angular_weight = 0.8
            elif angular_diff <= 7.0:  # Somewhat close (within 7 degrees)
                angular_weight = 0.6
            elif angular_diff <= 10.0:  # Medium distance
                angular_weight = 0.4
            elif angular_diff <= 15.0:  # Far
                angular_weight = 0.2
            elif angular_diff <= 20.0:  # Very far
                angular_weight = 0.1
            elif angular_diff <= 30.0:  # Extremely far
                angular_weight = 0.05
            else:  # Beyond 30 degrees
                angular_weight = 0.01
            
            # Other parameter distances (with much reduced importance)
            param_dist = 0.0
            ...
            for param in key_params:
                ...
                if src_val is not None and tgt_val is not None:
                    if param == 'defect_type':
                        if src_val != tgt_val:
                            param_dist += 0.05  # Very small penalty for different defect types
                    elif param == 'shape':
                        if src_val != tgt_val:
                            param_dist += 0.05  # Very small penalty for different shapes
                    elif param == 'eps0':
                        eps0_diff = abs(src_val - tgt_val)
                        param_dist += eps0_diff / 3.0 * 0.05  # Reduced weight
                    elif param == 'kappa':
                        kappa_diff = abs(src_val - tgt_val)
                        param_dist += kappa_diff / 2.0 * 0.05  # Reduced weight
            
            # Combine with Gaussian kernel for smooth transitions
            param_weight = np.exp(-0.5 * (param_dist / self.spatial_sigma) ** 2)
            
            # Final combination: Angular weight dominates (95%), parameters contribute (5%)
            combined_weight = 0.95 * angular_weight + 0.05 * param_weight
    ...
    def visualize_angular_weighting(self, target_angle_deg=54.7, figsize=(12, 8)):
        """Visualize the simplified angular weighting function"""
        ...
        for angle in angles:
            ...
            # Apply simplified weighting function
            if angular_diff <= 1.0:
                weight = 1.0
            elif angular_diff <= 2.0:
                weight = 0.95
            elif angular_diff <= 3.0:
                weight = 0.9
            elif angular_diff <= 5.0:
                weight = 0.8
            elif angular_diff <= 7.0:
                weight = 0.6
        ...
        ax.set_title(f'Angular Weighting Function (Simplified)\nTarget Angle: {target_angle_deg}°',
        ...
        regions = [
            (1.0, 1.0, 'Very Very Close (≤1°)'),
            (2.0, 0.95, 'Very Close (≤2°)'),
            (3.0, 0.9, 'Close (≤3°)'),
            (5.0, 0.8, 'Moderate (≤5°)'),
            (10.0, 0.4, 'Far (≤10°)'),
            (20.0, 0.1, 'Very Far (≤20°)')
        ]
    ...
    def encode_parameters(self, params_list, target_angle_deg):
        """Encode parameters into transformer input with simplified features"""
        encoded = []
    ...
    def interpolate_spatial_fields(self, sources, target_angle_deg, target_params):
        """Interpolate full spatial stress fields using transformer attention with simplified spatial locality"""
        if not sources:
            st.warning("No sources provided for interpolation.")
            return None
        
        # Pre-check: warn if few sources are close to target angle
        source_angles = []
        for src in sources:
            if 'params' in src:
                theta = src['params'].get('theta', 0.0)
                source_angles.append(np.degrees(theta) % 360)
        
        if source_angles:
            angular_distances = [min(abs(a - target_angle_deg), 360 - abs(a - target_angle_deg)) for a in source_angles]
            close_sources = sum(1 for d in angular_distances if d < 5.0)
            if close_sources < 2:
                st.warning(f"Only {close_sources} sources within 5° of target angle ({target_angle_deg}°). "
                          "Interpolation may be less accurate. Consider adding more sources near target orientation.")
        ...
        try:
            ...
            # Compute simplified positional weights with pure angular proximity
            ...
            # With locality_weight_factor=0.3, we get 30% transformer weights, 70% spatial weights
            combined_weights = (
    ...
    def create_comparison_dashboard(self, interpolated_fields, source_fields, source_info,
        ...
        if ground_truth_index is not None and 0 <= ground_truth_index < len(source_fields):
            ...
            if gt_field is not None:
                ...
                gt_defect = source_info['defect_types'][ground_truth_index]  # Added defect type
                ...
                ax2.set_title(f'Ground Truth\nθ = {gt_theta:.1f}° ({gt_defect}), Δ={gt_distance:.1f}°', fontsize=16, fontweight='bold')
        ...
        if ground_truth_index is not None and 0 <= ground_truth_index < len(source_fields):
            ...
            if gt_field is not None:
                ...
                ax3.set_title(f'Difference\nMax Abs Error: {max_diff:.3f} GPa', fontsize=16, fontweight='bold')
        ...
        if 'weights' in source_info:
            ...
            # Add theta and defect type labels to bars
            for i, bar in enumerate(bars):
                if i < len(source_info['theta_degrees']) and i < len(source_info['defect_types']):
                    theta = source_info['theta_degrees'][i]
                    defect = source_info['defect_types'][i]
                    height = bar.get_height()
                    if height > 0.01:  # Only label significant bars
                        ax4.text(bar.get_x() + bar.get_width()/2., height + 0.005,
                                f'{defect}\nθ={theta:.0f}°', ha='center', va='bottom',
        ...
        if 'theta_degrees' in source_info and 'distances' in source_info:
            ...
            defect_colors = {'Twin': 'red', 'ESF': 'blue', 'ISF': 'green', 'No Defect': 'gray'}
            colors = [defect_colors.get(dt, 'purple') for dt in source_info['defect_types']]
            
            scatter = ax5.scatter(angles_rad, distances,
                                 s=sizes, alpha=0.7, c=colors, edgecolors='black')
            
            # Plot target angle
            target_rad = np.radians(target_angle)
            ax5.scatter(target_rad, 0, s=200, c='black', marker='*', edgecolors='white', label='Target')
            
            # Plot special angle (if enabled)
            if special_angle is not None:
                special_rad = np.radians(special_angle)
                ax5.axvline(special_rad, color='orange', alpha=0.7, linestyle='--', linewidth=2, 
                          label=f'Special Angle ({special_angle}°)')
            ...
            # Create legend for defect types
            from matplotlib.lines import Line2D
            legend_elements = [Line2D([0], [0], marker='o', color='w', markerfacecolor=defect_colors[dt], 
                                     markersize=10, label=dt) for dt in defect_colors.keys()]
    ...
    def prepare_export_data(self, interpolation_result, visualization_params):
        ...
        export_data = {
            ...
            'result': {
                ...
                'special_angle': visualization_params.get('special_angle', None),
...
# =============================================
# MAIN APPLICATION WITH CORRECTED PHYSICS
# =============================================
def main():
    # Configure Streamlit page
    st.set_page_config(
        page_title="Corrected Transformer Stress Interpolation",
        layout="wide",
        page_icon="🔬",
    ...
    # Main header
    st.markdown('<h1 class="main-header">🔬 Corrected Transformer Stress Field Interpolation</h1>', unsafe_allow_html=True)
    
    # Description
    st.markdown("""
    <div class="info-box">
    <strong>🔬 Physics-corrected stress interpolation with simplified spatial locality.</strong><br>
    • <strong>FIXED:</strong> Corrected eigenstrain values to match source files (Twin: 2.12, ESF: 1.414, ISF: 0.707)<br>
    • <strong>FIXED:</strong> Simplified angular weighting that prioritizes numerical proximity<br>
    • <strong>FIXED:</strong> 60° now properly weighted more heavily than 90° for 54.7° targets<br>
    • <strong>NEW:</strong> Configurable special angle with minimal bias<br>
    • <strong>NEW:</strong> Pure numerical proximity for synthetic datasets<br>
    • <strong>NEW:</strong> Warnings when insufficient sources near target orientation<br>
    ...
    if 'transformer_interpolator' not in st.session_state:
        # Initialize with corrected parameters and simplified weighting
        st.session_state.transformer_interpolator = CorrectedTransformerSpatialInterpolator(
            spatial_sigma=0.2,
            locality_weight_factor=0.3,  # 30% transformer weights, 70% spatial weights
        )
    ...
    with st.sidebar:
        ...
        st.markdown('<h2 class="section-header">🧠 Simplified Spatial Locality</h2>', unsafe_allow_html=True)
        
        # Spatial locality parameters
        st.markdown("#### 📍 Angular Distance Weighting")
        st.info("""
        **Simplified Weighting Scheme (Pure Numerical Proximity):**
        - ≤1°: 1.0 weight (highest priority)
        - ≤2°: 0.95 weight  
        - ≤3°: 0.9 weight
        - ≤5°: 0.8 weight
        - ≤7°: 0.6 weight
        - ≤10°: 0.4 weight
        - ≤15°: 0.2 weight
        - >30°: 0.01 weight (lowest priority)
        
        For synthetic datasets around 54.7°, 60° will always be weighted more heavily than 90°.
        """)
        ...
        # KEY FEATURE: Adjustable spatial locality weight factor (now favoring spatial weights)
        spatial_weight_factor = st.slider(
            ...
            value=0.3,
            step=0.01,
            help="Balance between transformer attention and spatial locality (0 = pure spatial, 1 = pure transformer). Lower values give more weight to angular proximity."
        )
        ...
        if st.button("🚀 Perform Transformer Interpolation", type="primary", use_container_width=True):
            if not st.session_state.solutions:
                ...
                with st.spinner("Performing interpolation with corrected physics..."):
    ...
    if st.session_state.solutions:
        ...
        if st.session_state.solutions:
            ...
            if source_thetas:
                ...
                # Show proximity to target angle
                target_angle = custom_theta
                closest_angle = min(source_thetas, key=lambda x: min(abs(x-target_angle), 360-abs(x-target_angle)))
                closest_distance = min(abs(closest_angle-target_angle), 360-abs(closest_angle-target_angle))
                st.markdown(f"**Closest Source to Target:** {closest_angle:.1f}° (Δ={closest_distance:.1f}°)")
    ...
    if st.session_state.interpolation_result:
        ...
        with tab1:
            ...
            # Simplified spatial locality configuration
            st.markdown("#### 📍 Simplified Spatial Locality Configuration")
            ...
            with locality_col1:
                st.metric(
                    ...
                    help="0 = pure spatial weights, 1 = pure transformer weights. Lower values prioritize angular proximity."
                )
            with locality_col2:
                st.metric(
                    "Spatial Sigma",
        ...
        with tab3:
            # Simplified Weights analysis tab
            st.markdown('<h2 class="section-header">⚖️ Simplified Weight Distribution Analysis</h2>', unsafe_allow_html=True)
            if 'weights' in result:
                ...
                with col_w4:
                    ...
                    max_defect = st.session_state.solutions[result['source_indices'][max_weight_idx]]['params']['defect_type'] if max_weight_idx < len(result['source_indices']) and 'defect_type' in st.session_state.solutions[result['source_indices'][max_weight_idx]]['params'] else 'Unknown'
                ...
                # Add theta and defect type labels
                for i in range(len(weights['combined'])):
                    if i < len(result['source_indices']):
                        source_idx = result['source_indices'][i]
                        if source_idx < len(st.session_state.solutions):
                            defect_type = st.session_state.solutions[source_idx]['params'].get('defect_type', 'Unknown')
                            theta = result['source_theta_degrees'][i] if i < len(result['source_theta_degrees']) else 0.0
                            dist = result['source_distances'][i] if i < len(result['source_distances']) else 0.0
                ...
                for i in range(len(weights['combined'])):
                    if i < len(result['source_indices']):
                        source_idx = result['source_indices'][i]
                        if source_idx < len(st.session_state.solutions):
                            source_params = st.session_state.solutions[source_idx]['params']
                            ...
                            if angular_diff <= 1.0:
                                ...
                                angular_weight_category = "Very Close (≤2°)"
                            elif angular_diff <= 3.0:
                                angular_weight_category = "Close (≤3°)"
                            elif angular_diff <= 5.0:
                                angular_weight_category = "Moderate (≤5°)"
                            elif angular_diff <= 10.0:
                                angular_weight_category = "Far (≤10°)"
                            elif angular_diff <= 20.0:
                                angular_weight_category = "Very Far (≤20°)"
                            else:
                                angular_weight_category = "Extremely Far (>20°)"
        ...
        with tab4:
            ...
            • For synthetic datasets, 60° sources should be more similar to 54.7° targets than 90° sources
            </div>
            """, unsafe_allow_html=True)
            ...
            if 'source_theta_degrees' in result and result['source_theta_degrees']:
                # Create dropdown options with defect type information
                    ...
                    if i < len(result['source_indices']):
                        ...
                        if source_idx < len(st.session_state.solutions):
                            ...
                            ground_truth_options.append(
                                f"Source {source_idx}: {defect_type} @ θ={theta:.1f}° (Δ={distance:.1f}°, weight={weight:.3f})"
                            )
                ...
                selected_source_idx = int(selected_option.split(":")[0].split(" ")[1])
                st.session_state.selected_ground_truth = selected_source_idx
                
                # Display selected source info
                selected_theta = result['source_theta_degrees'][result['source_indices'].index(selected_source_idx)] if selected_source_idx in result['source_indices'] else 0.0
                selected_distance = result['source_distances'][result['source_indices'].index(selected_source_idx)] if selected_source_idx in result['source_indices'] else 0.0
                selected_weight = result['weights']['combined'][result['source_indices'].index(selected_source_idx)] if selected_source_idx in result['source_indices'] else 0.0
                selected_defect = st.session_state.solutions[selected_source_idx]['params'].get('defect_type', 'Unknown') if selected_source_idx < len(st.session_state.solutions) else 'Unknown'
                
                col_gt1, col_gt2, col_gt3, col_gt4 = st.columns(4)
                with col_gt1:
                    st.metric("Selected Source", selected_source_idx)
                with col_gt2:
                ...
                if comp_component in result['fields']:
                    # Prepare source info for dashboard - include defect types
                    source_info = {
                        'theta_degrees': result['source_theta_degrees'],
                        'distances': result['source_distances'],
                        'weights': result['weights'],
                        'defect_types': []  # Add defect types to source info
                    }
                    
                    # Extract defect types for all sources
                    for source_idx in result['source_indices']:
                        if source_idx < len(st.session_state.solutions):
                            source_info['defect_types'].append(st.session_state.solutions[source_idx]['params'].get('defect_type', 'Unknown'))
                        else:
                            source_info['defect_types'].append('Unknown')
                    ...
                    fig_comparison = st.session_state.heatmap_visualizer.create_comparison_dashboard(
                        ...
                        cmap_name=comp_cmap,
                        figsize=(20, 15),
                        ground_truth_index=result['source_indices'].index(selected_source_idx),
                        special_angle=54.7
                    )
                    st.pyplot(fig_comparison)
                    
                    # Calculate and display detailed error metrics
                    if result['source_indices'].index(selected_source_idx) < len(source_fields_list):
                        ground_truth_field = source_fields_list[result['source_indices'].index(selected_source_idx)][comp_component]
        ...
        with tab5:
            ...
            • Includes corrected eigenstrain values and simplified angular weighting
            </div>
            """, unsafe_allow_html=True)
                    ...
                    with zipfile.ZipFile(zip_buffer, 'w') as zip_file:
                        ...
                        if "Comparison Dashboard" in export_plots and st.session_state.selected_ground_truth is not None:
                            ...
                            fig = st.session_state.heatmap_visualizer.create_comparison_dashboard(
                                ...
                                component='von_mises',
                                cmap_name='viridis',
                                ground_truth_index=result['source_indices'].index(st.session_state.selected_ground_truth),
                                special_angle=54.7
                            )
                            buf = BytesIO()
                            fig.savefig(buf, format='png', dpi=300, bbox_inches='tight')
        ...
        Follow these steps to start interpolating stress fields with corrected physics:
        </p>
        ...
        <li>Adjust simplified spatial locality parameters</li>
        ...
        <li>✅ Fixed eigenstrain values: Twin=2.12, ESF=1.414, ISF=0.707</li>
        <li>✅ Simplified angular weighting for pure numerical proximity</li>
        <li>✅ 60° now properly weighted more heavily than 90° for 54.7° targets</li>
        <li>✅ Removed unnecessary crystal symmetry complexity for synthetic data</li>
        <li>✅ Added proximity warnings for better interpolation accuracy</li>
        </ul>
        </p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_guide2:
        ...
        - **Eigenstrain Values Fixed:**
          - Twin boundary: **2.12** (was 0.707)
          - ESF (Extrinsic Stacking Fault): **1.414** (was 0.333) 
          - ISF (Intrinsic Stacking Fault): **0.707** (was 0.289)
        - **Simplified Weighting:**
          - Pure numerical proximity (60° > 90° for 54.7° target)
          - Minimal defect type influence
          - 70% spatial weights, 30% transformer weights
        """)
    with col_guide3:
        st.markdown("""
        #### 🔍 Advanced Features
        - Visualize simplified angular weighting function
    ...
    # Physics explanation
    with st.expander("🧬 Physics Background: Corrected Eigenstrains and Pure Angular Proximity", expanded=True):
        st.markdown("""
        **Key Corrections for Synthetic Dataset Interpolation:**
        ...
        1. **Corrected Eigenstrain Values:**
           ...
           - These values now correctly reflect the physical magnitudes for FCC crystal defects in the synthetic dataset

        2. **Pure Angular Proximity for Synthetic Data:**
           - **Removed crystal symmetry complexity** - synthetic datasets are designed with simple numerical relationships
           - **60° is properly weighted higher than 90°** for 54.7° targets (Δ=5.3° vs Δ=35.3°)
           - **Simplified weighting scheme** prioritizes numerical proximity over other features
           - **No special angle bias** unless explicitly enabled - interpolation respects pure distance relationships

        3. **Why 54.7° Looks More Similar to 60° Than 90°:**
           - In synthetic datasets, stress fields change **gradually** with orientation
           - The **angular distance** is the dominant factor: 54.7°→60° = 5.3° difference vs 54.7°→90° = 35.3° difference
           - **Defect type matters less** than orientation proximity in these controlled synthetic examples
           - **Linear interpolation** between nearby orientations produces physically reasonable results

        4. **Why Previous Implementation Failed:**
           - Overly complex weighting scheme tried to account for crystal symmetry that doesn't apply to synthetic data
           - Transform attention mechanism was overriding spatial proximity relationships
           - Incorrect eigenstrain values distorted the entire stress field magnitude relationships
           - Habit plane bonuses and symmetry considerations created artificial weight distributions

        **Example Weight Distribution for 54.7° Target:**
        - Twin @ 60°: **0.8** weight (Δ=5.3°)
        - ESF @ 55°: **0.9** weight (Δ=0.3°) 
        - ISF @ 50°: **0.8** weight (Δ=4.7°)
        - Twin @ 90°: **0.05** weight (Δ=35.3°)
        - ESF @ 30°: **0.01** weight (Δ=24.7°)

        This corrected implementation ensures that **numerical proximity dominates** the interpolation, producing results that match the synthetic dataset design principles.
        """)
